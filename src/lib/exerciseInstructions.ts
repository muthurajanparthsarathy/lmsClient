...
export type InstructionResolution = {
  html: string
  /**
   * The same instructions split into one HTML fragment per point, so callers
   * can render them as a numbered checklist. Empty when the authored copy is a
   * single block that can't be split — render `html` instead.
   */
  items: string[]
...
/**
 * Split authored rich text into one fragment per point — `<li>` items first,
 * falling back to `<p>` blocks. Inner markup (bold, links) is preserved so the
 * author's emphasis survives; blocks with no visible text are dropped.
 */
function splitAuthored(html: string): string[] {
  const collect = (tag: 'li' | 'p'): string[] => {
    const matches = html.match(new RegExp(`<${tag}\\b[^>]*>([\\s\\S]*?)<\\/${tag}>`, 'gi')) || []
    return matches
      .map((m) => m.replace(new RegExp(`^<${tag}\\b[^>]*>|<\\/${tag}>$`, 'gi'), '').trim())
      .filter((inner) => stripHtml(inner).length > 0)
  }
  const items = collect('li')
  if (items.length > 0) return items
  const paras = collect('p')
  // A single paragraph is the author writing one continuous note — keep it as
  // prose rather than dressing one sentence up as a one-item checklist.
  return paras.length > 1 ? paras : []
}
...
export function resolveExerciseInstructions(exercise: any): InstructionResolution {
  ...
  if (stripHtml(authored).length > 0) {
    return { html: authored, items: splitAuthored(authored), source: 'authored' }
  }
