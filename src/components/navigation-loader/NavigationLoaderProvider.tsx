...
import { usesClientWorkspaceSkeleton } from '@/lib/clientWorkspaceRoutes';
import { isPersistentDashboardRoute } from '@/lib/dashboardRoutes';
...
interface Ctx {
  ...
  stop: () => void;
  /** A persistent layout handles navigation loading inside its content panel. */
...
export function NavigationLoaderProvider({ children }: { children: React.ReactNode }) {
  ...
  const skeletonOnly = usesClientWorkspaceSkeleton(pathname) || contentOnly;
  ...
  const start = useCallback((targetPath?: string) => {
    const nextPath = targetPath ?? window.location.pathname;
    if (usesClientWorkspaceSkeleton(nextPath) || (contentOnly && isPersistentDashboardRoute(nextPath))) {
      scheduleHide();
      return;
    }
  ...
  }, [scheduleHide, installDomObserver, contentOnly]);
  ...
  const onLocationChange = useCallback(() => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (usesClientWorkspaceSkeleton(window.location.pathname) || (contentOnly && isPersistentDashboardRoute(window.location.pathname))) {
        scheduleHide();
        return;
      }
    }
    ...
    void raf1; void raf2;
  }, [scheduleHide, queryClient, scanDomForLoading, contentOnly]);
  ...
  useEffect(() => {
    ...
    if (usesClientWorkspaceSkeleton(window.location.pathname)) return;
  ...
  const value = useMemo<Ctx>(() => ({
    isNavigating: isVisible && !skeletonOnly,
    start,
    stop,
    setContentOnly,
  }), [isVisible, skeletonOnly, start, stop]);
  ...
  return (
    <NavigationLoaderContext.Provider value={value}>
      ...
      {children}
      {!skeletonOnly && <PageLoader visible={isVisible} />}
