...
export interface ContactPerson {
  name: string;
  // Primary email/phone — required on write. The two "secondary" columns
  // below are an ADDITIONAL email + phone on the SAME person (not a second
  // contact person), so one contact record can carry two ways to reach
  // them. Both are optional and never blocked by required-field checks.
  email: string;
  phoneNumber: string;
  secondaryEmail?: string;
  secondaryPhoneNumber?: string;
  isPrimary: boolean;
  // Free-text role for the contact — Primary / Secondary / Finance / HR /
  // Technical / Management / Other. Optional and never required by the
  // backend: existing records without one keep working, and the form's
  // suggestions can grow without a type change.
  contactType?: string;
  designation?: string;
  _id?: string;
}
...
export interface Client {
  _id: string;
  // Human-readable Client ID like "CLT-000023" — allocated by the server on
  // create (see server/utils/clientCode.js) and stable for the client's life.
  // Optional at the type level because records created before the field
  // existed carry "" until the read-side backfill fills them in.
  clientId?: string;
  clientCompany: string;
  // Organization / office phone (switchboard) — NOT the primary contact's
  // mobile. Free-text so a legacy record's single string keeps working.
  clientPhone?: string;
  description?: string;
  clientAddress?: string;
  clientLogo?: string;
  // Where the uploaded logo sits inside its circular frame. CSS
  // object-position string (e.g. "50% 50%", "30% 70%"). Empty on legacy
  // records; renderers fall back to "50% 50%".
...
// clientId is intentionally NOT on this shape — it is allocated by the server
// on create and immutable on update.
export interface ClientInput {
  clientCompany: string;
  clientPhone?: string;
  /** 4-digit reference year the client was created. Optional at the type
   *  level for callers that just want to update other fields; the form
   *  always sends it, and the server may use it to override `createdAt`
   *  (Jan 1 of that year) so the "Created Year" column matches the pick. */
  createdYear?: string;
  description?: string;
  clientAddress?: string;
  clientLogo?: string;
...
export interface ClientPageFilters {
  search?: string;
  status?: string;
  /** Single value ("B2B") for legacy callers, or a list (["B2B","B2I"]) for the
   *  toolbar's multi-select. Empty list reads as "no filter". */
