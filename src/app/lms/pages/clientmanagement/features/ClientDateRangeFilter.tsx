...
export default function ClientDateRangeFilter({ value, onChange, earliestCreatedAt, years: explicitYears, className = '' }: {
    ...
    /** Explicit list of years to offer. When supplied, this overrides the
     *  `earliestCreatedAt → now` range — the caller has narrowed the set
     *  (e.g. only years where clients matching the OTHER active filters
     *  exist), and offering years outside that set would be a dead end. */
    years?: string[]
    className?: string
}) {
    const years = explicitYears && explicitYears.length ? explicitYears : yearsBack(earliestCreatedAt)
