...
import type { Client } from '@/app/lms/pages/clientmanagement/api/clientManagementService'
import { TYPE_LABEL, avatarTone, businessModelDisplayName, fmtDate, orderedContacts } from './lib'
            ...
            title={
                <span className="flex items-center gap-3 min-w-0">
                    {shown?.clientLogo ? (
                        ...
                        <img
                            src={shown.clientLogo}
                            alt=""
                            className="w-10 h-10 rounded-tile object-cover flex-shrink-0 border border-hairline"
                            style={{ objectPosition: shown.clientLogoPosition || '50% 50%' }}
                        />
                    ) : (
                        ...
                        <div className="rounded-tile border border-hairline bg-surface overflow-hidden shadow-xs">
                            <DetailRow
                                ...
                                value={shown.clientId ? (
                                    <span className="font-mono text-body tabular-nums">{shown.clientId}</span>
                                ) : (
                                    <span className="text-faint text-xs italic">Not yet assigned</span>
                                )}
                            />
                            <DetailRow label="Client name" value={shown.clientCompany || '—'} />
                            <DetailRow label="Business model" value={businessModelDisplayName(shown.businessModel)} />
                            <DetailRow label="Office phone" value={shown.clientPhone || '—'} />
                            <DetailRow label="Status" value={isActive ? 'Active' : 'Inactive'} />
                            <DetailRow
                                label="Address"
                                value={shown.clientAddress && shown.clientAddress.replace(/<[^>]*>/g, '').trim() ? (
                                    // Address is authored in TipTap now, so
                                    // the stored value carries HTML. Render
                                    // it the same way Description does one
                                    // block below — trailing whitespace-check
                                    // avoids a bare "<p></p>" showing as a
                                    // blank line.
                                    <div
                                        className="prose prose-sm max-w-none [&_p]:my-0.5 [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5"
                                        dangerouslySetInnerHTML={{ __html: shown.clientAddress }}
                                    />
                                ) : '—'}
                            />
