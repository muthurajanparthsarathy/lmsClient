...
export function ClientManagementView({ embedded = false }: { embedded?: boolean }) {
    ...
    // Initial page size is deliberately small (5). On first render, before
    // the auto-fit effect has measured the viewport, this is the number of
    // rows the server is asked for — a conservative value that fits inside
    // any usable viewport (even 1366×768 with a maximised sidebar). The
    // effect then expands it to the real fit on the next tick. Setting a
    // large initial value would fetch (and briefly render) more rows than
    // the container can hold, producing a one-frame internal scrollbar on
    // small screens. Undershooting is invisible; overshooting is visible.
...
<body>
    ...
    const columns: Column<Client>[] = [
        ...
        },
        {
            ...
            className: 'no-print w-[8%] pl-2 pr-4 sm:pr-5 text-right whitespace-nowrap align-middle',
            skeletonWidth: '20px',
            render: (client) => {
        ...
        <Shell>
            ...
            >
                ...
                <div ref={tableCardRef} className="mt-2 flex flex-1 min-h-0 flex-col overflow-hidden">
                    ...
                    <div ref={tableFooterRef} className="no-print">
                    <TableFooter
