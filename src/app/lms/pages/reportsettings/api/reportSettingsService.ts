...
export const letterheadElements = (): ReportElement[] => [
    ...
    { ...newElement('text', 'reportTitle'), text: '{title}', x: 4.5, y: 20.8, w: 58, h: 3.4, fontSize: 12.5, bold: true, color: BRAND_INK },
    { ...newElement('text', 'reportMeta'), text: '{scope}  ·  Generated {generated}', x: 57.5, y: 21.0, w: 38, h: 3.4, fontSize: 8.5, align: 'right', color: BRAND_MUTED },

    /* The scope the reader narrowed to, in words — sitting directly under
       the report title, which is where someone reading the printed sheet
       looks to answer "what is this a report OF?". `{filters}` resolves
       to nothing when the report covers everything, and an element with
       no text renders nothing, so an unfiltered report simply doesn't
       carry the line. */
    { ...newElement('text', 'reportFilters'), text: '{filters}', x: 4.5, y: 24.4, w: 91, h: 3, fontSize: 7.5, color: BRAND_MUTED },

    // ── The data ─────────────────────────────────────────────────────────────
    { ...newElement('table', 'table'), x: 4.5, y: 28, w: 91, h: 54 },
