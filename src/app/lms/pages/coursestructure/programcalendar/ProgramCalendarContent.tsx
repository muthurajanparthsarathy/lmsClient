...
import { phaseOptionsFor } from '@/app/lms/pages/coursestructure/components/coursePhases'
import { businessModelDisplayName } from '@/app/lms/pages/clientmanagement/features/lib'
import ProgramCalendarReportDialog from './reports/ProgramCalendarReportDialog'
import type { CalendarHierarchy, CalendarReportSource } from './reports/programCalendarReport'
import type { CalendarReportStructure } from './reports/calendarReportTable'
import styles from './ProgramCalendar.module.css'
import ProgramCalendarToolbar from './ProgramCalendarToolbar'
...
// Resolves WHICH course record the screen works on, and nothing else.
//
// A phased course is several course records — one per phase — so choosing a
// phase means working on that phase's record. The screen below is KEYED on the
// resolved id: switching phase unmounts it and mounts a fresh one.
//
// That key is load-bearing, not tidiness. The screen latches its saved calendar
// into local state behind a `dataLoaded` flag — start date, session slots,
// deviations, the generated schedule — and none of it is derived from
// `courseId`. Swapping the id underneath a live screen left the previous
// phase's calendar on display and, worse, in the save payload: the writes take
// the CURRENT courseId with whatever body is in state, and the server $sets the
// whole document. Remounting is the only way to be certain nothing survives.
export default function ProgramCalendarContent(
    ...
    const baseCourseId = courseIdProp ?? searchParams.get('courseId')
    // Dropped whenever the host points at a different course, so a phase chosen
    // for one course never leaks into the next.
    const [phaseCourseId, setPhaseCourseId] = useState<string | null>(null)
    // The chosen phase is part of the key for the same reason the course is:
    // each phase has its OWN calendar, and the screen latches one into local
    // state. Switching phase remounts rather than trying to reset piece by
    // piece — the pieces are what got missed last time.
    const [phase, setPhase] = useState('')
    useEffect(() => { setPhaseCourseId(null); setPhase('') }, [baseCourseId])
...
function ProgramCalendarScreen(
    { courseId, phase, embedded, onPhaseChange }: {
        courseId: string | null
        /** "" until the user picks one, then whichever phase they are working in. */
        phase: string
        embedded: boolean
        onPhaseChange: (name: string, courseId: string) => void
    }
) {
    ...
    // Fetched as soon as the course resolves, not only when the report opens:
    // the Phase control needs it to know whether this course is phased at all.
    // Same ['service-mapping','detail',id] cache entry the Business Management
    // pages use, so this is usually already warm.
    const { data: reportMapping, isLoading: reportMetadataLoading, isError: reportMetadataError, refetch: refetchReportMetadata } = useServiceMapping(
        course?.mappingId || batchHierarchy?.mappingId || undefined
    )

    // Empty for every course that does not run in phases — which is most of
    // them — and the toolbar then renders no Phase control at all.
    const phaseOptions = useMemo(
        () => phaseOptionsFor(reportMapping, allCourses as any[], course as any),
        [reportMapping, allCourses, course]
    )
    // The phase being worked on. One course covers all of its phases, so this
    // is a view choice, not a different course — it only reopens a record when
    // the mapping is an older one that kept a record per phase.
    const currentPhase = useMemo(() => {
        if (phase && phaseOptions.some(p => p.name === phase)) return phase
        // Falls back to the phase this record was filed under, then to the first
        // configured phase, so the control is never blank while options exist.
        const path = String((course as any)?.coursePath || '').trim()
    ...
    // Which modules this phase plans from. A phased course builds its structure
    // per phase, so the calendar must schedule THAT phase's modules — planning
    // Phase II from Phase I's content would invent a schedule for work that
    // phase does not do.
    //
    // A module saved before phases has no phase and belongs to the FIRST one —
    // the same rule Course Structure applies, because the two screens must
    // agree about what Phase I contains.
    //
    // The mapping is what says whether this course has phases, and it arrives a
    // moment after the course does. Treating "not loaded yet" as "no phases"
    // would let EVERY phase's modules through for that moment — the calendar
    // would paint both phases and then narrow, which reads as the filter not
    // working at all. A course filed under a one-part path is the phased shape
    // (a degree placement is " ▸ "-joined), so only those hold, and only until
    // the mapping resolves. Every other course is unaffected.
    const phasesUnknown = useMemo(() => {
        const path = String((course as any)?.coursePath || '').trim()
        const looksPhased = Boolean(path) && !path.includes(' ▸ ')
        return looksPhased && !phaseOptions.length && (reportMetadataLoading || !reportMapping)
    }, [course, phaseOptions, reportMetadataLoading, reportMapping])

    const inCurrentPhase = useMemo(() => {
        const fallback = phaseOptions[0]?.name || ''
        return (m: { phase?: string }) => {
            if (phasesUnknown) return false
            if (!phaseOptions.length) return true
            const tagged = String(m?.phase || '').trim()
            return tagged ? tagged === currentPhase : fallback === currentPhase
        }
    }, [phaseOptions, currentPhase, phasesUnknown])

    const { data: rawModules = [] } = useQuery({ ...moduleApi.getAll(), enabled: !!course, select: (d: any[]) => d.filter(m => (m.courses?.includes?.(courseId)||m.courses===courseId) && inCurrentPhase(m)).sort((a: any,b: any)=>(a.index||0)-(b.index||0)) })
    ...
    const { data: savedCalendar, isSuccess: calendarFetched, isError: calendarFetchFailed } = useQuery({
        ...programCalendarApi.getByCourse(courseId || '', currentPhase, isFirstPhase),
    ...
    useEffect(() => {
        ...
        if (d?.startDate) { setStartDate(d.startDate); setSavedStartDate(d.startDate); setActiveTab('calendar') }
    ...
    const savePayload = () => ({
        ...
        sessions: daySlots,
        deviations,
        // Each phase keeps its own calendar. adoptLegacy lets the FIRST phase
        // claim a calendar saved before phases existed, so it is carried
        // forward rather than left orphaned beside a new one.
        phase: currentPhase,
        adoptLegacy: isFirstPhase,
    })
    ...
    function handleSaveStartDate() {
        ...
        saveCalendar(savePayload(), {
            onSuccess: () => {
                ...
                setDataLoaded(true)
                queryClient.invalidateQueries({ queryKey: ['programCalendar', courseId, currentPhase] })
    ...
    function handleSaveSession() {
        if (!courseId) return
        saveCalendar(savePayload(), { onSuccess: () => { setSessionDirty(false); setDataLoaded(true); queryClient.invalidateQueries({ queryKey: ['programCalendar', courseId, currentPhase] }); showSaveToast('Session details saved successfully!') } })
    ...
    const reportBatchId = realBatches.some(batch => batch._id === reportBatch) ? reportBatch : realBatches[0]?._id || 'all'

    // The report needs this table's MERGE GEOMETRY, and the generated schedule
    // cannot carry it: an activity merged across h1..h3 is scheduled as ONE
    // content item, so the schedule has no way to say that rows 2 and 3 are
    // continuations of a merge rather than rows with nothing in them. Rebuilding
    // it from the same isCellMerged the table renders with is what keeps the
    // exported tables identical to what is on screen. Hours are the configured
    // pedagogy durations; the report prefers schedule-derived hours and only
    // falls back to these when an activity never made it onto the calendar.
    const reportStructure = useMemo<CalendarReportStructure>(() => ({
        rows: tableRows.map((row, ridx) => {
            const activities: CalendarReportStructure['rows'][number]['activities'] = []
            ;(['iDo', 'weDo', 'youDo'] as const).forEach(type => {
                ;(activityTypes[type] as string[]).forEach(activity => {
                    // pedagogyViews is already any[] from its query select, so no cast.
                    const mi = isCellMerged(ridx, type, activity, pedagogyViews, tableRows)
                    if (mi.isMerged) {
                        activities.push({ type, activity, hours: mi.value, rowSpan: mi.rowSpan, continuation: !mi.isStart })
                        return
                    }
                    const hours = getSingleHour(row, type, activity)
                    if (hours > 0) activities.push({ type, activity, hours, rowSpan: 1, continuation: false })
                })
            })
            return {
                module: row.moduleName, subModule: row.subModuleName,
                topic: row.topicName, subTopic: row.subtopicName,
                activities,
            }
        }),
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }), [tableRows, activityTypes, pedagogyViews, courseHours])

    const reportSource = useMemo<CalendarReportSource>(() => {
        const scopedDeviations = deviations.filter(deviation => !deviation.appliesTo?.length || deviation.appliesTo.includes(reportBatchId))
        const mappedClient = typeof reportMapping?.client === 'object' ? reportMapping.client : undefined
        const reportHierarchy = (course?.courseHierarchy || []).filter((level): level is CalendarHierarchy =>
            ['Module', 'Sub Module', 'Topic', 'Sub Topic'].includes(level))
        return {
            planned: generated || [],
            actual: reportOpen && startDate && dailyHours > 0 && contentItems.length
                ? generateCalendar(daySlots, contentItems, { startDate, workingDays }, [
                    ...holidays,
                    ...scopedDeviations.map(deviation => ({ id: deviation.id, name: deviation.reason, date: deviation.date, duration: 'full' as const })),
                ]) : [],
            hierarchy: reportHierarchy,
            details: {
                courseName: course?.courseName || 'Not specified',
                courseId: course?.courseCode || course?._id || 'Not specified',
                clientName: mappedClient?.clientCompany || course?.clientName || course?.clientData?.clientCompany || 'Not specified',
                businessModel: businessModelDisplayName(mappedClient?.businessModel || course?.serviceType),
                serviceModel: course?.serviceModal || reportMapping?.serviceModels?.join(', ') || 'Not specified',
                providingYear: reportMapping?.year || 'Not specified',
            },
            deviations: scopedDeviations.map(({ date, reason }) => ({ date, reason })),
            batchName: realBatches.find(batch => batch._id === reportBatchId)?.batchName,
            structure: reportStructure,
        }
    }, [reportOpen, reportBatchId, realBatches, generated, deviations, reportMapping, course, startDate, dailyHours, contentItems, daySlots, workingDays, holidays, reportStructure])

    const openReport = () => {
        setReportBatch(realBatches.some(batch => batch._id === selectedActualBatch) ? selectedActualBatch : realBatches[0]?._id || 'all')
        setReportOpen(true)
    }
    ...
    function deleteSlotAndSave(id: string) {
        ...
        setSessionDirty(false)
        if (courseId) {
            saveCalendar({ ...savePayload(), sessions: next }, { onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['programCalendar', courseId, currentPhase] }); showSaveToast('Session deleted and saved.') } })
        }
    }
    ...
    function clearAllSlotsAndSave() {
        ...
        setSessionDirty(false)
        if (courseId) {
            saveCalendar({ ...savePayload(), sessions: [] }, { onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['programCalendar', courseId, currentPhase] }); showSaveToast('All sessions cleared.') } })
        }
    }
    ...
    function handleReset() {
        if (!courseId) return
        deleteCalendar({ courseId, phase: currentPhase }, {
            onSuccess: () => {
                queryClient.invalidateQueries({ queryKey: ['programCalendar', courseId, currentPhase] })
                setStartDate('')
                setDaySlots([])
    ...
    return (
        <Shell embedded={embedded}>
            ...
            <div className="min-h-screen">
                <div className="max-w-full mx-auto rounded-2xl border border-hairline bg-surface px-4 py-4 md:px-5 space-y-4">
                    ...
                    <div className="flex flex-wrap items-center justify-between gap-2 border-b border-hairline">
                        ...
                        {activeTab === 'calendar' && <Button size="sm" variant="outline" onClick={openReport} disabled={!generated?.length} className="text-xs"><BarChart3 className="size-3.5" />Report</Button>}
                        {/* Reset button — only show when there is saved data */}
                        {(savedCalendar as any)?._id && (
                            <button onClick={()=>setResetConfirm(true)}
                                className="flex h-8 items-center gap-1.5 px-2.5 rounded-control text-subtle hover:bg-row-hover hover:text-danger-700 text-xs font-medium transition-colors">
                    ...
                    {activeTab === 'calendar' && (
                        <motion.div initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} className="space-y-4">
                            ...
                            <div ref={tourStartRef}>
                                <ProgramCalendarToolbar
                                    startDate={startDate}
                                    onStartDateChange={value => { setStartDate(value); setStartDateDirty(true) }}
                                    editable={startDateEditable}
                                    saving={isSaving}
                                    dirty={startDateDirty}
                                    canSave={Boolean(courseId && startDate && startDateDirty)}
                                    onSave={handleSaveStartDate}
                                    endDate={summary?.endDate || estimatedEndDate ? fmtDate((summary?.endDate || estimatedEndDate)!) : undefined}
                                    workingDays={estimatedDays ?? 0}
                                    holidays={displayHolidays.length}
                                    onWorkingDays={() => setShowWorkingDaysModal(true)}
                                    onHolidays={() => setShowHolidaysModal(true)}
                                    restriction={evidenceLock
                                        ? hasAttendance ? 'Program calendar locked due to attendance marked' : `Locked — ${deviations.length} deviations recorded`
                                        : startPassed ? `Started ${savedStartDate} — ${isAdmin ? 'admin correction only' : 'start date locked'}` : undefined}
                                    holidayWarning={!holidayScopeId ? 'Institution not resolved — holidays not applied yet' : undefined}
                                    mode={calendarMode}
                                    onModeChange={mode => { setCalendarMode(mode); if (mode !== 'planned') setScheduleView('table') }}
                                    batches={realBatches.map(batch => ({ value: batch._id, label: batch.batchName }))}
                                    batch={selectedActualBatch}
                                    onBatchChange={setSelectedActualBatch}
                            ...
                            return (
                                <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{duration:0.2}}
                                    className={`${styles.calendarTable} ${tableFullscreen
                                        ? 'fixed inset-0 z-modal bg-surface flex flex-col'
                                        : 'overflow-auto max-h-[75vh] rounded-xl border border-hairline'}`}>
                                    {tableFullscreen && (
                                        <div className="flex items-center justify-between border-b border-hairline px-4 py-2 bg-surface shrink-0">
                                            <span className="text-sm font-semibold text-heading">Program Calendar</span>
                                            <button onClick={()=>setTableFullscreen(false)}
                                                className="flex items-center gap-1.5 text-xs font-medium text-body hover:bg-row-hover border border-hairline-strong rounded-control px-3 py-1.5 transition-colors">
                                    ...
                                    <table className="w-full text-[11px] border-collapse">
                                        <thead className="sticky top-0 z-20">
                                            {/* Row 1: group headers */}
                                            <tr>
                                                {hasModule    && <th rowSpan={2} style={{background:'var(--calendar-hierarchy-header)',color:'white'}} className="border border-ink-700 px-2.5 py-1.5 text-center font-bold align-middle min-w-[120px]">Module</th>}
                                                {hasSubModule && <th rowSpan={2} style={{background:'var(--calendar-hierarchy-header)',color:'white'}} className="border border-ink-700 px-2.5 py-1.5 text-center font-bold align-middle min-w-[110px]">Sub Module</th>}
                                                {hasTopic     && <th rowSpan={2} style={{background:'var(--calendar-hierarchy-header)',color:'white'}} className="border border-ink-700 px-2.5 py-1.5 text-center font-bold align-middle min-w-[130px]">Topic</th>}
                                                {hasSubTopic  && <th rowSpan={2} style={{background:'var(--calendar-hierarchy-header)',color:'white'}} className="border border-ink-700 px-2.5 py-1.5 text-center font-bold align-middle min-w-[110px]">Sub Topic</th>}
                                                {!hideIDo && activityTypes.iDo.length>0 && (
                                                    collapseIDo
                                                        ? <th rowSpan={2} style={{background:'var(--calendar-ido-header)',color:'white'}} className="border border-brand-900 px-2.5 py-1.5 text-center font-bold align-middle">I Do</th>
                                                        : <th colSpan={activityTypes.iDo.length} style={{background:'var(--calendar-ido-header)',color:'white'}} className="border border-brand-900 px-2 py-1.5 text-center font-bold">I Do</th>
                                                )}
                                                {!hideWeDo && activityTypes.weDo.length>0 && (
                                                    collapseWeDo
                                                        ? <th rowSpan={2} style={{background:'var(--calendar-wedo-header)',color:'white'}} className="border border-brand-900 px-2.5 py-1.5 text-center font-bold align-middle">We Do</th>
                                                        : <th colSpan={activityTypes.weDo.length} style={{background:'var(--calendar-wedo-header)',color:'white'}} className="border border-brand-900 px-2 py-1.5 text-center font-bold">We Do</th>
                                                )}
                                                <th rowSpan={2} style={{background:'var(--calendar-hours-header)',color:'white'}} className="border border-warn-700 px-2.5 py-1.5 text-center font-bold align-middle whitespace-nowrap">Total Hours</th>
                                                ...
                                                <th rowSpan={2} style={{background:'var(--calendar-start-header)',color:'white'}} className="border border-success-700 px-2.5 py-1.5 text-center font-bold align-middle whitespace-nowrap">Start Date</th>
                                                <th rowSpan={2} style={{background:'var(--calendar-end-header)',color:'white'}} className="border border-brand-900 px-2.5 py-1.5 text-center font-bold align-middle whitespace-nowrap">End Date</th>
                                                {isActual && <th rowSpan={2} style={{background:'var(--calendar-deviation-header)',color:'white'}} className="border border-danger-700 px-2.5 py-1.5 text-center font-bold align-middle whitespace-nowrap min-w-[120px]">Deviation</th>}
                                            </tr>
                                        ...
                                        <tbody>
                                                ...
                                                return tableRows.map((row, ridx) => {
                                                                                ...
                                                                                {groupDevs.map(dv => (
                                                                                    <div key={dv.id} className="flex items-start gap-1">
                                                                                        ...
                                                                                        <button onClick={()=>{
                                                                                            ...
                                                                                            if (courseId) saveCalendar({ ...savePayload(), deviations: nextDevs }, { onSuccess: ()=>{ queryClient.invalidateQueries({ queryKey: ['programCalendar', courseId, currentPhase] }); showSaveToast('Deviation removed.') } })
                                                    ...
                                                    return (
                                                        <Fragment key={row.rowId}>
                                                            {mainRow}
                                                            <tr className={styles.assessmentRow} style={{background:'#ECFDF5'}}>
                                            ...
                                            <tr className={styles.totalRow} style={{background:'#F1F5F9'}}>
                            ...
                            {calendarMode === 'comparison' && (
                                <motion.div initial={{opacity:0}} animate={{opacity:1}} className="space-y-3">
                                    ...
                                    {(!generated || !actualGenerated) ? (
                                        ...
                                        <div className={`${styles.calendarTable} overflow-auto max-h-[75vh] rounded-xl border border-hairline`}>
                                        {(() => {
                                            ...
                                            return (
                                                <table className="w-full text-[11px] border-collapse">
                                                    <thead className="sticky top-0 z-10">
                                                        ...
                                                        <tr>
                                                            <th rowSpan={2} style={{background:'var(--calendar-hierarchy-header)',color:'white'}} className="border border-ink-700 px-2.5 py-1.5 text-center font-bold align-middle whitespace-nowrap min-w-[40px]">#</th>
                                                            <th rowSpan={2} style={{background:'var(--calendar-hierarchy-header)',color:'white'}} className="border border-ink-700 px-2.5 py-1.5 text-center font-bold align-middle min-w-[130px]">Module</th>
                                                            <th rowSpan={2} style={{background:'var(--calendar-hierarchy-header)',color:'white'}} className="border border-ink-700 px-2.5 py-1.5 text-center font-bold align-middle min-w-[140px]">Topic</th>
                                                            <th rowSpan={2} style={{background:'var(--calendar-hours-header)',color:'white'}} className="border border-warn-700 px-2.5 py-1.5 text-center font-bold align-middle whitespace-nowrap">Total Hours</th>
                                                            <th colSpan={2} style={{background:'var(--calendar-planned-header)',color:'white'}} className="border border-info-700 px-2.5 py-1.5 text-center font-bold whitespace-nowrap">Planned</th>
                                                            <th colSpan={2} style={{background:'var(--calendar-start-header)',color:'white'}} className="border border-success-700 px-2.5 py-1.5 text-center font-bold whitespace-nowrap">Actual</th>
                                                            <th rowSpan={2} style={{background:'var(--calendar-deviation-header)',color:'white'}} className="border border-danger-700 px-2.5 py-1.5 text-center font-bold align-middle whitespace-nowrap min-w-[120px]">Deviation</th>
                                                        </tr>
                            ...
                            <div className="p-5 space-y-4">
                                    ...
                                    <button onClick={()=>{
                                        ...
                                        if (courseId) {
                                            saveCalendar({ ...savePayload(), deviations: nextDevs }, {
                                                onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['programCalendar', courseId, currentPhase] }); showSaveToast('Deviation saved.') }
                                            })
                                        }
                                    }}
                                ...
                                {existing && (
                                    <button onClick={()=>{
                                        ...
                                        if (courseId) {
                                            saveCalendar({ ...savePayload(), deviations: nextDevs }, {
                                                onSuccess: () => { queryClient.invalidateQueries({ queryKey: ['programCalendar', courseId, currentPhase] }); showSaveToast('Cancellation removed.') }
                                            })
                                        }
                                    }}
            ...
            {reportOpen && <ProgramCalendarReportDialog
                source={reportSource}
                initialView={calendarMode}
                onClose={() => setReportOpen(false)}
                batches={realBatches.map(batch => ({ value: batch._id, label: batch.batchName }))}
                selectedBatch={reportBatchId}
                onBatchChange={setReportBatch}
                loading={reportMetadataLoading}
                metadataError={reportMetadataError ? 'Linked service details could not be loaded. Retry to include the providing year and client details.' : undefined}
                onRetryMetadata={() => { void refetchReportMetadata() }}
            />}
        </Shell>
    )
}
