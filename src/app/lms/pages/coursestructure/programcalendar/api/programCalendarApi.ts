...
export const deleteProgramCalendar = async (courseId: string, phase = ''): Promise<any> => {
    // Scoped to the phase: resetting Phase II must not wipe Phase I.
...
export const programCalendarApi = {
    ...
    getByCourse: (courseId: string, phase = '', adoptLegacy = false) => ({
        queryKey: ['programCalendar', courseId, phase],
        queryFn: () => fetchProgramCalendarByCourse(courseId, phase, adoptLegacy),
        enabled: !!courseId,
        staleTime: 0,
        refetchOnMount: true,
    ...
    delete: () => ({
        ...
        mutationFn: (vars: string | { courseId: string; phase?: string }) =>
            typeof vars === 'string'
                ? deleteProgramCalendar(vars)
                : deleteProgramCalendar(vars.courseId, vars.phase || ''),
    }),
};
