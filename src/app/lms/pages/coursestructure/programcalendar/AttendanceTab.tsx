...
import {
  ...
  type HalfPeriod,
} from "@/apiServices/attendanceApi";
...
export default function AttendanceTab({ courseId }: AttendanceTabProps) {
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Single-row toolbar: search + batch on the left; date/day + download
          + remarks flush on the right. Blocking-state pills (weekend, missing
          window, etc.) sit between so they're impossible to miss. */}
      <div className="mb-1.5 flex items-center gap-2 flex-wrap">
        ...
        <div className="relative w-64 shrink-0">
          ...
          <input
            ...
            className="w-full h-9 pl-8 pr-7 rounded-md border border-hairline bg-surface text-xs text-body placeholder:text-faint focus:outline-none focus:ring-2 focus:ring-brand/30 focus:border-brand-500 transition-colors"
          />
          {studentSearch && (
            <button
              type="button"
              ...
              className="absolute right-2 top-1/2 -translate-y-1/2 text-faint hover:text-body transition-colors"
            >
              <X className="h-3.5 w-3.5" />
            </button>
          )}
        </div>
        {/* Batch selector — portalled listbox with the L&D floating-label
            chrome; keeps the dirty-cell guard so unsaved marks still route
            through the discard dialog. */}
        ...
        {/* Blocking-state pills — explain why the marking circles are locked.
            L&D reviewers don't mark, so the pills are noise for them and stay
            hidden in the fromLdc flow. */}
        {!fromLdc && isWeekend(selectedDay) && <StatusPill tone="danger">Weekend</StatusPill>}
        {!fromLdc && future && <StatusPill tone="neutral">Future date — marking disabled</StatusPill>}
        {!fromLdc && !future && trainingWindow === undefined && !windowError && (
          <StatusPill tone="neutral">Checking training window…</StatusPill>
        )}
        {!fromLdc && !future && windowError && (
          <StatusPill tone="warn">
            Couldn&apos;t check the training window — marking disabled, reload to retry
          </StatusPill>
        )}
        {!fromLdc && !future && windowMissing && (
          <StatusPill tone="warn">
            Set the Program Calendar first — attendance needs a training schedule
          </StatusPill>
        )}
        {!fromLdc && !future && beforeStart && (
          <StatusPill tone="warn">
            Training starts {fmtWindowDay(trainingWindow?.startDate)} — marking disabled
          </StatusPill>
        )}
        {!fromLdc && !future && afterEnd && (
          <StatusPill tone="neutral">
            Training ended {fmtWindowDay(trainingWindow?.endDate)} — marking disabled
          </StatusPill>
        )}
        {/* Right cluster. L&D Head sees only the status tally here (single row);
            the shell header already owns date + Detailed report + Remarks.
            Everyone else keeps the date badge + inline download shortcuts. */}
        {fromLdc ? (
          students.length > 0 && (
            <div className="ml-auto flex items-center gap-1.5 shrink-0 flex-wrap">
              {[
                { label: "Present",        value: sumP,   dot: "bg-success-500" },
                { label: "Absent", value: sumA,   dot: "bg-danger-500" },
                { label: "Half-day",       value: sumH,   dot: "bg-warn-500" },
                { label: "Not marked",     value: sumNot, dot: "bg-ink-400" },
              ].map((c) => (
                <span key={c.label} className={`inline-flex items-center gap-1.5 ${D.chip} font-medium text-body`}>
          ...
          <div className="ml-auto flex items-center gap-2 shrink-0">
            <span className="inline-flex items-center gap-1.5 h-9 rounded-md border border-hairline bg-surface-sunken/60 px-2.5 text-xs text-body tabular-nums">
              <span className="font-semibold text-heading">{fmt(selectedDay)}</span>
              <span className="text-subtle">· {fmtWeekday(selectedDay)}</span>
            </span>
            <button
              type="button"
              onClick={() => setReportPicker("attendance")}
              disabled={students.length === 0}
              className="inline-flex items-center gap-1.5 h-9 px-3 rounded-md border border-brand-200 bg-surface hover:bg-brand-wash text-brand-strong text-xs font-semibold disabled:opacity-50 transition-colors"
            >
            ...
            <button
              type="button"
              onClick={() => setReportPicker("remarks")}
              disabled={students.length === 0}
              className="inline-flex items-center gap-1.5 h-9 px-3 rounded-md border border-warn-500/30 bg-surface hover:bg-warn-50 text-warn-700 text-xs font-semibold disabled:opacity-50 transition-colors"
            >
      ...
      {!me.admin && !me.viewer && visibleBatches.length > 0 && (
        <p className="mb-1 text-2xs text-faint">Showing the batches you belong to.</p>
      )}

      {/* Summary chips — the selected day's tally at a glance. Hidden in the
          L&D Head flow: the same chips already live in the toolbar row. */}
      {!fromLdc && students.length > 0 && (
        <div className="mb-2 flex items-center gap-2 flex-wrap">
          {[
            ...
            <span
              key={c.label}
              className={`inline-flex items-center gap-1.5 ${D.chip} font-medium text-body`}
            >
              <span className={`h-1.5 w-1.5 rounded-full ${c.dot}`} />
              {c.label}
              <span className="tabular-nums font-semibold text-heading">{c.value}</span>
      ...
      {/* Grid */}
      <div className="flex-1 min-h-0 flex flex-col rounded-tile border border-hairline bg-surface shadow-xs overflow-hidden">
        {loadingStudents || loadingAttendance ? (
          <div className="flex-1 overflow-hidden">
            <div className="flex h-10 items-center gap-4 border-b border-hairline bg-canvas px-4">
              {["w-6", "w-40", "w-24", "w-12", "w-12", "w-12", "w-32"].map((w, i) => (
          ...
          <div ref={attTableWrapRef} className="flex-1 min-h-0 overflow-hidden">
            {/* table-fixed: cell CONTENT must never drive column widths — the
                Remark cell swaps between "—" and a sentence, and the All pill
                gains a check icon when everyone is marked; under auto layout
                each of those reflowed every column on click. */}
            <table className="w-full border-collapse text-sm table-fixed">
              <thead className="bg-canvas sticky top-0 z-sticky">
                <tr>
                  <th className={`w-12 ${D.theadPad} text-left ${D.theadText} font-semibold uppercase tracking-wider text-subtle border-b border-hairline`}>
                    #
                  </th>
                  <th className={`min-w-[16rem] ${D.theadPad} text-left ${D.theadText} font-semibold uppercase tracking-wider text-subtle border-b border-hairline`}>
                    Student
                  </th>
                  <th className={`w-28 ${D.theadPad} text-left ${D.theadText} font-semibold uppercase tracking-wider text-subtle border-b border-hairline`}>
                    Roll No.
                  </th>
                  {STATUS_COLUMNS.map(({ key, label }) => {
                    // Marked for all when every student already carries this
                    // status — one click marks (or clears) the whole class.
                    const allMarked =
                      students.length > 0 &&
                    ...
                    return (
                      <th
                        key={key}
                        className="w-24 px-2 py-2 text-center border-b border-hairline"
                      >
                        <div className={`text-2xs font-semibold uppercase tracking-wider ${statusHeadText[key]}`}>
                          {label}
                        </div>
                        {!locked && (
                          <button
                            ...
                            className={cn(
                              "mt-1 inline-flex items-center gap-1 rounded-chip border px-1.5 h-5 text-[10px] font-semibold transition-colors",
                              allMarked
                                ? toneAllActive[key]
                                : "border-hairline-strong bg-surface text-subtle hover:bg-row-hover"
                            )}
                          >
                            {/* A verb, not a bare "All" — the pill MARKS every
                                student with this status; once everyone has it,
                                it reports that state and a click clears it. */}
                            {allMarked && <Check className="h-3 w-3" strokeWidth={3} />}
                  ...
                  <th className={`w-40 ${D.theadPad} text-left ${D.theadText} font-semibold uppercase tracking-wider text-subtle border-b border-hairline`}>
                    Remark
                  </th>
                </tr>
              </thead>
              <tbody>
                {pagedStudents.map((s, idx) => {
                  ...
                  return (
                    <tr key={s._id} className="hover:bg-row-hover transition-colors">
                      <td className="px-3 py-1.5 text-faint text-[10px] tabular-nums border-b border-hairline">
                        {String(i + 1).padStart(2, "0")}
                      </td>
                      <td className={`${D.cellPad} border-b border-hairline`}>
                        <div className="flex items-center gap-2.5">
                          <div className={`${D.avatarSize} rounded-full bg-brand-wash text-brand-strong flex items-center justify-center font-semibold flex-shrink-0`}>
                            {(s.firstName?.[0] || "?").toUpperCase()}
                            {(s.lastName?.[0] || "").toUpperCase()}
                          </div>
                          <div className="min-w-0">
                            <div className={`${D.nameText} font-semibold text-heading truncate leading-tight`}>
                              {`${s.firstName} ${s.lastName}`.trim() || "—"}
                            </div>
                            {s.email && (
                              <div className={`${D.emailText} text-subtle truncate leading-tight`}>
                                {s.email}
                              </div>
                            )}
                          </div>
                      ...
                      <td className="px-3 py-1.5 text-body text-[11px] border-b border-hairline whitespace-nowrap">
                        {s.userId || "—"}
                      </td>
                      {STATUS_COLUMNS.map(({ key, label }) => {
                        const on = status === key;
                        return (
                          <td
                            key={key}
                            className={`${D.statusCellPad} text-center border-b border-hairline`}
                          >
                            <button
                              type="button"
                              disabled={locked}
                              ...
                              className={cn(
                                `inline-flex ${D.circleSize} items-center justify-center rounded-full border transition-all`,
                                on
                                  ? statusOnClasses[key]
                                  : "border-hairline-strong bg-surface text-transparent hover:border-brand-300 hover:bg-row-hover",
                                locked && "opacity-40 cursor-not-allowed",
                                cell?.dirty && on && "ring-2 ring-brand/30 ring-offset-1"
                              )}
                            >
                      ...
                      <td className={`${D.cellPad} border-b border-hairline`}>
                        {status === "A" || status === "H" ? (
                          <div className="text-xs text-body max-w-[260px]">
                            ...
                            {cell?.reason ? (
                              ...
                              <span className="italic text-faint">
                                Reason is asked on Save changes
                              </span>
                            )}
                          </div>
                        ) : (
        ...
        {/* Pagination footer — matches the L&D portfolio table footer. */}
          ...
          <div className="flex flex-wrap items-center justify-between gap-3 border-t border-hairline bg-surface-sunken/30 px-4 py-1.5">
            ...
            {attTotalPages > 1 && (
              <div className="flex items-center gap-1">
                <button type="button" onClick={() => setAttPage((p) => Math.max(1, p - 1))} disabled={attCurrent <= 1} aria-label="Previous page" className="inline-flex h-7 w-7 items-center justify-center rounded-control border border-hairline-strong text-subtle transition-colors hover:bg-row-hover disabled:cursor-not-allowed disabled:opacity-40">
                  <ChevronLeft className="h-3.5 w-3.5" />
                </button>
                {Array.from({ length: attTotalPages }, (_, idx) => idx + 1).slice(0, 5).map((p) => (
                  <button key={p} type="button" onClick={() => setAttPage(p)} aria-current={attCurrent === p ? "page" : undefined} className={`inline-flex h-7 min-w-[28px] items-center justify-center rounded-control px-2 text-xs font-semibold tabular-nums transition-colors ${attCurrent === p ? "bg-brand-100 text-brand-700 dark:bg-brand-500/15 dark:text-brand-400" : "text-subtle hover:bg-row-hover hover:text-body"}`}>
                    {p}
                  </button>
                ))}
                <button type="button" onClick={() => setAttPage((p) => Math.min(attTotalPages, p + 1))} disabled={attCurrent >= attTotalPages} aria-label="Next page" className="inline-flex h-7 w-7 items-center justify-center rounded-control border border-hairline-strong text-subtle transition-colors hover:bg-row-hover disabled:cursor-not-allowed disabled:opacity-40">
