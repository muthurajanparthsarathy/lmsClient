...
export interface SubmitDeps {
    ...
    /** The phase being worked on, stamped onto anything created under it. */
    currentPhase?: string;
            ...
            if (!addOnlyPedagogyLevel) {
                if (editMode?.type === 'module') {
                    ...
                    newModule = await createModuleMutation.mutateAsync({
                        ...
                        index: nextIndex,
                        courses: selectedCourse._id,
                        // Which phase this module belongs to. "" for a course
                        // with no phases, which is every non-placement course.
                        phase: currentPhase || "",
                        institution: "",
                        testConfiguration: moduleTestConfig
                    });
