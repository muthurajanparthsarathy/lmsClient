...
function PedagogyScreen(
    ...
    } = usePedagogyManagement(courseId, phase, onPhaseChange)
...
// Resolves WHICH course record the builder works on, and nothing else.
//
// A phased course is several course records — one per phase. The screen above
// is KEYED on the resolved id so switching phase remounts it, because its
// transient modes hold RECORD IDS: an armed delete selection, merge
// selections, expanded rows. Swapping the course in place left those pointing
// at the phase no longer on screen — confirming a delete then removed modules
// the user could not see. It also left `selected` disagreeing with
// `selectedCourse`, and the resync effect snapped the screen back to the old
// phase on the next refetch of the course list.
export default function PedagogyManagementContent(
    { courseId, embedded = false }: { courseId?: string; embedded?: boolean } = {}
) {
    const searchParams = useSearchParams()
    // The standalone route carries ?courseId=; the L&D console is hash-routed
    // and hands the id down instead.
    const baseCourseId = courseId ?? searchParams.get('courseId') ?? undefined
    const [phaseCourseId, setPhaseCourseId] = React.useState<string | null>(null)
    // Keyed on the phase as well as the course: Phase I's modules are not Phase
    // II's, and the builder's transient modes hold MODULE IDS — an armed delete
    // selection, merge selections, expanded rows. Remounting is what guarantees
    // none of them survives a phase change.
    const [phase, setPhase] = React.useState('')
    React.useEffect(() => { setPhaseCourseId(null); setPhase('') }, [baseCourseId])
    const active = phaseCourseId || baseCourseId
    return <PedagogyScreen
        key={`${active || ''}::${phase}`}
        courseId={active}
        phase={phase}
        embedded={embedded}
        onPhaseChange={(name, nextId) => {
            setPhase(name)
            if (nextId && nextId !== active) setPhaseCourseId(nextId)
        }}
    />
}
