 ...
 * @param phaseOverride the phase being worked on, owned by the wrapper so a
 * change remounts this screen — see the note there.
 * @param onPhaseChange hands a phase choice back to that wrapper. Deliberately
 * NOT a local setSelectedCourse.
 */
export function usePedagogyManagement(
    ...
    // ── Phase ───────────────────────────────────────────────────────────────
    // A phased course (Placement Training) is several course records, one per
    // phase, so choosing a phase means switching to that phase's record. The
    // mapping is the only place that knows whether this course is phased at
    // all; it shares the ['service-mapping','detail',id] cache entry with the
    // Business Management pages, so it is usually already warm.
    const { data: courseMapping } = useServiceMapping(selectedCourse?.mappingId || undefined)
    const phaseOptions = useMemo(
        () => phaseOptionsFor(courseMapping, courses as Course[], selectedCourse),
        [courseMapping, courses, selectedCourse]
    )
    // The phase being worked on. One course covers all of its phases, so this is
    // a view choice, not a different course — it only reopens a record when the
    // mapping is an older one that kept a record per phase.
    const currentPhase = useMemo(() => {
        if (phaseOverride && phaseOptions.some((p) => p.name === phaseOverride)) return phaseOverride
        ...
        const path = String(selectedCourse?.coursePath || '').trim()
        return phaseOptions.find((p) => p.name.toLowerCase() === path.toLowerCase())?.name
            || phaseOptions[0]?.name || ''
    }, [phaseOverride, phaseOptions, selectedCourse])
    const handlePhaseSelect = useCallback((name: string) => {
    ...
    const {
        ...
        select: (data) => {
            ...
            const mine = data.filter(module => module.courses.includes(selectedCourse._id));
            // …and, for a phased course, to the phase being worked on. Phase I's
            // structure is not Phase II's; each is built and saved separately.
            // A module with no phase predates this, so it belongs to the FIRST
            // phase — otherwise everything built before would vanish.
            if (!phaseOptions.length) return mine;
            const fallback = phaseOptions[0]?.name || '';
            return mine.filter((module) =>
                ((module as { phase?: string }).phase || '').trim() === currentPhase
                || (!((module as { phase?: string }).phase || '').trim() && fallback === currentPhase));
        }
    });
    ...
    phaseOptions, currentPhase, handlePhaseSelect,
