...
export interface PedagogyMainViewDeps {
    ...
    embedded?: boolean;
    /** The phases this course runs in. Empty for every course that is not
     *  phased, and then no Phase control renders — see coursePhases.ts. */
    phaseOptions?: { name: string; courseId: string }[];
    currentPhase?: string;
    handlePhaseSelect?: (name: string) => void;
    ...
    return (
            <div className={embedded ? "bg-gray-50 px-2 py-2" : "bg-gray-50 px-4 sm:px-6 py-4"}>
                <div className="space-y-2">
                    ...
                    {/* Phase — only for a course that actually runs in phases.
                        Its own card rather than a slot in the breadcrumb line,
                        because that line is hidden when embedded and the phase
                        still has to be switchable there. */}
                    {phaseOptions.length > 0 && (
                        <div className="mt-2.5 flex items-center gap-2 bg-white rounded-xl border border-[#F0EFEE] shadow-[0_1px_2px_rgba(16,24,40,0.04)] px-4 py-2.5">
                            <label htmlFor="pedagogy-phase" className="text-xs font-medium text-slate-600">Phase</label>
                            <Select value={currentPhase || undefined} onValueChange={(value) => handlePhaseSelect?.(value)}>
                                <SelectTrigger id="pedagogy-phase" className="h-8 w-44 text-xs">
