...
const ClientInfoCard: React.FC<{ course: any }> = ({ course }) => (
    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-4">
        ...
        <div className="space-y-2">
            ...
            {course.clientData?.clientAddress && stripHtml(course.clientData.clientAddress) && (
                <div>
                    <p className="text-xs font-medium text-gray-500 dark:text-gray-400">Address</p>
                    {/* Client Address is TipTap HTML (see Client
                          Management); strip tags here so this
                          plain-text card shows a readable address
                          instead of raw markup. `whitespace-pre-line`
                          keeps the paragraph breaks from the
                          extracted text. */}
                    <p className="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-line">{stripHtml(course.clientData.clientAddress)}</p>
                </div>
            )}
