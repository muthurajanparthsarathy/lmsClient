...
interface ResourceTypeSectionProps {
  ...
  // Notes / AI, the "Student Used Feature" pair. Course Setup offers them; the
  // Map Service wizard does not, because the mapping is choosing which resource
  // TYPES its courses start with and these two are per-course student features
  // rather than defaults worth carrying at mapping level. Default true so every
...
const ResourceTypeSection: React.FC<ResourceTypeSectionProps> = ({
  ...
  // Hidden wholesale where the caller has no use for them. Kept as an empty
  // list rather than a flag at each site, so the section's own "is there
  // anything to show" check stays the single source of that answer.
  const STUDENT_FEATURE_ROWS = showStudentFeatures ? I_DO_STUDENT_FEATURE_ROWS : [];
      ...
      <div style={{ padding: 14 }}>
        ...
        {activeTab === 'iDo' && (
          <>
            ...
            ) : I_DO_ROWS.length === 0 && STUDENT_FEATURE_ROWS.length === 0 ? (
              <div style={{ padding: '24px 0', textAlign: 'center', color: 'var(--color-text-secondary)', fontSize: 13 }}>
