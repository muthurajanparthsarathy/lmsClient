...
/* Course Setup's filters, in the same shape Service Mapping uses: multi-select
   where the API takes a list, a range for years, and single-choice for the
   three it only takes one of. Applied live — there is no Apply button. */
...
export const EMPTY_FILTERS: MappingFilters = {
    clients: [], services: [], models: [], course: '', from: '', to: '',
}
import {
    STATUS_META,
    type MappingRowVM,
...
export default function MappingList({
    ...
    const serverFilters: MappingPageFilters = useMemo(() => {
        return {
            ...
            serviceModels: filters.models.length ? filters.models : undefined,
            course: filters.course || undefined,
            yearFrom: filters.from || undefined,
            yearTo: filters.to || undefined,
    ...
    const { data, isLoading, isFetching, isError } = useServiceMappingsPage(
        serverFilters,
        currentPage,
        pageSize,
        { setup: true },
    ...
    /* Picking clients narrows everything downstream of them. The server's
       byClient facet says what each client actually has — its services, service
       models, years and course names — so choosing two clients leaves the
       Service dropdown showing only the services those two run, and so on down
       the row. Null means "do not narrow": no client picked, or a backend
       without the facet. */
    const clientScope = useMemo(() => scopeByClients(facets, filters.clients), [facets, filters.clients])

    const options = useMemo(() => {
        const keep = (list: string[], allowed: string[] | undefined) =>
            allowed ? list.filter((value) => allowed.includes(value)) : list
        // Course values are lower-cased so the filter compares like with like
        // against the server's anchored, case-insensitive course match.
        const courseNames = keep(facets?.courses || [], clientScope?.courses)
        return {
            courses: courseNames.map((s) => ({ value: s.toLowerCase(), label: s })),
            clients: (facets?.clients || []).map(([value, label]) => ({ value, label })),
            // Value stays raw for the query; the label carries the acronym.
            services: keep(facets?.services || [], clientScope?.services).map((s) => ({ value: s, label: serviceLabel(s) })),
            models: keep(facets?.serviceModels || [], clientScope?.serviceModels).map((s) => ({ value: s, label: s })),
            years: keep(facets?.years || [], clientScope?.years).map((s) => ({ value: s, label: s })),
        }
    }, [facets, clientScope])

    /* A selection the new client scope no longer offers would stay active while
       vanishing from its own dropdown — an invisible predicate that empties the
       table with nothing left on screen to un-tick. pruneToScope returns the
       same array when there is nothing to drop, so this settles in one pass. */
    useEffect(() => {
        if (!clientScope) return
        setFilters((current) => {
            const services = pruneToScope(current.services, clientScope.services)
            const models = pruneToScope(current.models, clientScope.serviceModels)
            const course = current.course && !clientScope.courses.some((name) => name.toLowerCase() === current.course)
                ? '' : current.course
            ...
            return services === current.services && models === current.models
                && course === current.course && from === current.from && to === current.to
                ? current
                : { ...current, services, models, course, from, to }
        })
    }, [clientScope])
    ...
    const activeFilterCount =
        (filters.clients.length ? 1 : 0) + (filters.services.length ? 1 : 0) + (filters.models.length ? 1 : 0) +
        (filters.course ? 1 : 0) +
    ...
    const listChip = (count: number, one: string, many: string) => count === 1 ? one : `${count} ${many}`
    const filterChips: { key: string; label: string; onRemove: () => void }[] = [
        ...(filters.clients.length ? [{ key: 'clients', label: listChip(filters.clients.length, options.clients.find((o) => o.value === filters.clients[0])?.label || 'Client', 'clients'), onRemove: () => setFilters((f) => ({ ...f, clients: [] })) }] : []),
        ...(filters.services.length ? [{ key: 'services', label: listChip(filters.services.length, filters.services[0], 'services'), onRemove: () => setFilters((f) => ({ ...f, services: [] })) }] : []),
        ...(filters.models.length ? [{ key: 'models', label: listChip(filters.models.length, filters.models[0], 'service models'), onRemove: () => setFilters((f) => ({ ...f, models: [] })) }] : []),
        ...
        ...(filters.from || filters.to ? [{ key: 'year', label: `${filters.from || 'Any'} – ${filters.to || 'Any'}`, onRemove: () => setFilters((f) => ({ ...f, from: '', to: '' })) }] : []),
    ]
...
<body>${clientPages}</body>
    ...
    return (
        <div className="flex flex-1 min-h-0 flex-col px-4 sm:px-6 lg:px-8 pt-3 pb-3">
            {/* Heading dropped — the "Courses" tab in the top strip is
                already the section label, so a second "Course Setup" H1
                right beneath it repeats what the tab has said. */}

            {/* Four tiles over the WHOLE book of work, same treatment as
                Service Mapping's overview. */}
            <div>
                <CourseSetupOverview stats={stats} isError={isError} />
            </div>
            ...
            <div className="no-print mt-3 flex items-center gap-2 flex-wrap min-w-0">
                ...
                {/* Every action here is the shared Button, so the toolbar picks
                    up one focus ring, one disabled treatment and one hover
                ...
                <div className="ml-auto flex items-center gap-1.5 flex-wrap">
                    <Button
                        ...
                        aria-controls="course-setup-filters"
            ...
            {showFilters && <div id="course-setup-filters" className="no-print mt-3 shrink-0 rounded-xl border border-hairline bg-canvas/40 p-3">
                <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                    <div className="min-w-0"><p className="mb-1.5 text-xs font-medium text-subtle">Client</p><MappingMultiFilter label="Clients" options={options.clients} value={filters.clients} onChange={(clients) => setFilters((f) => ({ ...f, clients }))} placeholder="Select client" /></div>
                    <div className="min-w-0"><p className="mb-1.5 text-xs font-medium text-subtle">Service</p><MappingMultiFilter label="Services" options={options.services} value={filters.services} onChange={(services) => setFilters((f) => ({ ...f, services }))} placeholder="Select service" emptyLabel={clientScope ? 'No services for the selected clients' : undefined} /></div>
                    <div className="min-w-0"><p className="mb-1.5 text-xs font-medium text-subtle">Service model</p><MappingMultiFilter label="Service models" options={options.models} value={filters.models} onChange={(models) => setFilters((f) => ({ ...f, models }))} placeholder="Select service model" emptyLabel={clientScope ? 'No service models for the selected clients' : undefined} /></div>
                    <div className="min-w-0"><p className="mb-1.5 text-xs font-medium text-subtle">Course</p><MappingSingleFilter label="Courses" options={options.courses} value={filters.course} onChange={(course) => setFilters((f) => ({ ...f, course }))} placeholder="Select course" /></div>
                    <div className="min-w-0 [&>button]:w-full"><p className="mb-1.5 text-xs font-medium text-subtle">Year range</p><MappingYearRange from={filters.from} to={filters.to} years={(options.years || []).map((option) => option.value)} onChange={(from, to) => setFilters((f) => ({ ...f, from, to }))} placeholder="Select year range" /></div>
                </div>
