...
/** One mapping as the row view-model the list, the cards and the report all
 *  read. It lives here rather than in MappingList because the report needs the
 *  identical derivation — courses come from groupCourses (the mapping's own
 *  course array, deduped by place+name), and the status from the server's
 *  setupProgress. Reading those fields by hand anywhere else is how a report
 *  ends up insisting a client has no courses. */
export const toRowVM = (m: ServiceMapping): MappingRowVM => {
