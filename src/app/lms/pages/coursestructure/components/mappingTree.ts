...
export const buildPhaseTree = (
    ...
    // The group holding THIS mapping record. A group is identified by course
    // name AND path together (see `groupCourses`), so both have to be matched:
    // the same course name under two phases is two groups, and looking it up by
    // name alone would be ambiguous even if it resolved. Matching the composite
    // the same way `groupCourses` builds it keeps the two in step.
    const groupFor = (c: MappedCourse | undefined) => {
        if (!c) return null
        const key = `${norm(c.path)}::${norm(c.courseName)}`
        return groups.find((g) => `${norm(g.path)}::${norm(g.courseName)}` === key) || null
    }
