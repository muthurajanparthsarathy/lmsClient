...
export function TabCard({
    ...
    return (
        <div className={`bg-surface border border-hairline rounded-lg shadow-xs overflow-hidden flex flex-col ${className}`}>
            {children}
        </div>
    )
}
...
export function TabCardHeader({
    ...
    return (
        <div className="flex flex-wrap items-center justify-between gap-2 border-b border-hairline px-4 py-3 flex-shrink-0">
            <div className="flex items-center gap-2.5 min-w-0">
                <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-lg bg-brand-wash">
...
export function CountPill({ value, label }: { value: number; label: string }) {
    return (
        <span className="inline-flex h-6 items-center gap-1 rounded-md bg-ink-100 px-2 text-xs font-medium text-ink-700 whitespace-nowrap">
...
export const TH_CLASS =
    'h-9 bg-canvas border-b border-hairline px-3 text-left align-middle text-2xs font-semibold uppercase tracking-wider text-subtle'

export const TD_CLASS = 'h-11 px-3 align-middle text-xs text-body'
