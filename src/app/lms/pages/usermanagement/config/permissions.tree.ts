...
export const PERMISSION_TREE: PermissionNode[] = [
  ...
  container("admin", "Admin", { icon: "ShieldCheck", children: [
    ...
    // External — assessments and events run for people who are NOT LMS users.
    //
    // A CONTAINER with two grantable pages, same shape as Question Bank above,
    // so the modal reads exactly like the rail:
    //
    //   External
    //     Assessment  → View Details · Create Assessment · Edit · Delete ·
    //                   Questions · Participants · Add Participant · Bulk Upload
    //     Event       (no functions yet — Coming Soon)
    //
    // The functions here gate the External Assessment screen's own actions.
    // They are deliberately NOT shared with the You_Do assessment permission
    // set: External Assessment is a separate module over separate data
    // (external participants never become LMS users), so an admin granting
    // one must never implicitly grant the other.
    container("admin-external", "External", {
      icon: "Globe", children: [
        page("admin-external-assessment", "externalassessment", "Assessment", {
          icon: "ClipboardList", color: "violet",
          description: "Assessments for external participants — separate from LMS users",
          children: [
            fn("View Details", { defaultSelected: true }),
            fn("Create Assessment"),
            fn("Edit"),
            fn("Delete"),
            fn("Questions"),
            fn("Participants", { defaultSelected: true }),
            fn("Add Participant"),
            fn("Bulk Upload"),
          ],
        }),
        // No functions yet — the page is a Coming Soon placeholder. The
        // permission exists now so it can be granted ahead of the build and
        // the rail entry appears the moment the screen lands.
        page("admin-external-event", "externalevent", "Event", {
    ...
    container("admin-systemsettings", "System Settings", {
      icon: "Settings", children: [
        ...
        // Report Settings — the letterhead for everything the Reports tab
        // exports: header, footer, watermark, logo, signature, and the order
        // the blocks appear in. A separate grant because it changes what
        // leaves the building on a client-facing PDF; holding "can read the
        // report" should not imply "can restyle it for everyone".
        page("admin-report-settings", "reportsettings", "Report Settings",
          { icon: "FileCog", color: "slate" }),
      ],
    }),
