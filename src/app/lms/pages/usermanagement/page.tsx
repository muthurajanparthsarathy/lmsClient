...
export default function UserManagementPage() {
  ...
  const pageContent = (
    <div className="min-h-full h-full flex flex-col">
      ...
      >
        ...
        <div className="no-print mt-3 flex items-center gap-2 flex-wrap min-w-0">
          ...
          <div className="ml-auto flex items-center gap-1.5 flex-wrap">
            ...
            {/* Export used to sit here as its own dropdown. Excel / PDF /
                Print all live inside the Reports dialog now, which already
                knows what is on screen, so the toolbar does not repeat them. */}
        ...
        {/* ── Inline filter panel — expands on the same screen under the toolbar ──
            Same five multi-selects Service Mapping and Course Setup use, so the
            three admin lists filter identically. Role, Client, Service, Service
            model, Status; each applies as you tick, with no Apply button. */}
        {showFilters && (
          <div id="user-management-filters" className="no-print mt-3 shrink-0 rounded-xl border border-hairline bg-canvas/40 p-3">
            <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
              <div className="min-w-0">
                ...
                <MappingMultiFilter label="Roles" options={filterOptions.roles} value={selectedRoles} onChange={setSelectedRoles} placeholder="Select role" />
              </div>
              <div className="min-w-0">
                <p className="mb-1.5 text-xs font-medium text-subtle">Client</p>
                <MappingMultiFilter label="Clients" options={filterOptions.clients} value={selectedClients} onChange={setSelectedClients} placeholder="Select client" />
              </div>
              <div className="min-w-0">
                <p className="mb-1.5 text-xs font-medium text-subtle">Service</p>
                <MappingMultiFilter label="Services" options={filterOptions.services} value={selectedServices} onChange={setSelectedServices} placeholder="Select service" />
              </div>
              <div className="min-w-0">
                <p className="mb-1.5 text-xs font-medium text-subtle">Service model</p>
                <MappingMultiFilter label="Service models" options={filterOptions.serviceModels} value={selectedServiceModels} onChange={setSelectedServiceModels} placeholder="Select service model" />
              </div>
              <div className="min-w-0">
      ...
      {/* The report. Mounted only while open so it starts from a clean draft
          each time, and so its own fetch never runs behind a closed dialog.
          `baseParams` hands it the search that is on screen; the report's own
          controls supply the rest. */}
      {reportOpen && institutionId && token && (
        <UserReportDialog
          institutionId={institutionId}
          token={token}
          baseParams={{ search: debouncedSearchTerm, searchField }}
