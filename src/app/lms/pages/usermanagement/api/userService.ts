...
export type UsersPageParams = {
  ...
  roles?: string[];
  status?: string;
  /** Multi-select filters for the User Management filter bar. `statuses`
   *  supersedes the single `status` above when present. */
  clients?: string[];
  services?: string[];
...
export type UsersPageResponse = {
  ...
  limit: number;
  totalPages: number;
  /** Institution-wide, so the dropdowns list every value rather than the
   *  current page's. `clients` is [id, name] pairs, already sorted by name. */
  facets?: {
    batches?: string[];
  ...
  /** The four header tiles, counted over the whole institution. */
...
const buildUsersPageParams = (p: UsersPageParams, isExport = false) => {
  ...
  if (p.clients?.length) q.clients = p.clients.join(",");
  if (p.services?.length) q.services = p.services.join(",");
  if (p.serviceModels?.length) q.serviceModels = p.serviceModels.join(",");
  if (p.statuses?.length) q.statuses = p.statuses.join(",");
 ...
 */
/** One service mapping, as the export chunk ships it: enough to answer
 *  "which service does this client's <service model> belong to?" for users
 *  whose record predates `serviceMappingId`. */
export type ServiceIndexEntry = { _id: string; client?: string; service?: string; serviceModels?: string[] };

/** The export's rows, with the service index riding along on the array. Every
 *  existing caller just iterates the rows, so widening the return type this way
 *  leaves them untouched. */
export type UsersExportRows = UsersPageResponse["Users"] & { serviceIndex?: ServiceIndexEntry[] };
...
export const fetchUsersForExport = async (
  ...
  while (rows.length < total && page <= 1000) {
    ...
    // Same on every chunk; keep the first one and stop re-reading it.
    if (!serviceIndex.length && Array.isArray(response.data?.serviceIndex)) {
      serviceIndex = response.data.serviceIndex;
    }
    rows.push(...batch);
  ...
  // Carried on the array rather than in a wrapper object: every existing
  // caller destructures nothing and just iterates the rows.
  const result: UsersExportRows = rows;
  result.serviceIndex = serviceIndex;
  return result;
};
