...
import { fetchUsers, fetchUsersPage, type UsersPageParams } from "@/app/lms/pages/usermanagement/api/userService";
...
export type UsersPageResult = {
  ...
  batches: string[];
  /** Filter-bar options and the four header counts. Both describe the whole
   *  institution, not this page and not the current filter, so the dropdowns
   *  keep every choice while a filter is narrowing the rows. */
  facets: {
    ...
    queryFn: async () => {
      ...
      return {
        ...
        facets: {
          ...
          serviceModels: data.facets?.serviceModels ?? [],
        },
        stats: data.stats,
      };
    },
