...
import {
    ...
    Printer, FileSpreadsheet, FileText, CalendarDays, CircleCheck, Users, BarChart3,
    CheckCircle2,
} from 'lucide-react'
...
import ServiceReportDialog from './components/ServiceReportDialog'
import { MappingYearRange, MappingMultiFilter } from './components/MappingReportFilters'
...
function ClientCombobox({
    ...
    return (
        <div ref={boxRef} className="relative">
            ...
            >
                {selected ? (
                    // No business-model chip on the trigger. It is the very next
                    // field on this step ("Business model", filled from this same
                    // client) and a chip in the header summary above it, so on the
                    // trigger it was the third copy of one fact — and the one with
                    // least room. The list rows below still carry it, where it
                    // helps tell two similarly named clients apart.
                    <>
...
function MapServiceWizard({
    ...
    const headerContext = [
        ...
        {
            label: 'Service',
            value: serviceModels.map((m) => titleCaseName(serviceModelDisplayName(m))).join(' · '),
        },
                        ...
                        {phase === 'preview' ? (
                            <div className="flex-1 min-h-0 bg-surface flex flex-col">
                                <MappingPreview
                                    ...
                                    service={service}
                                    modelName={serviceModels.map(serviceModelDisplayName).join(' · ')}
                                    year={year}
                                            ...
                                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                                <div className="space-y-1" data-tour="sm-service">
                                                    ...
                                                    >
                                                        ...
                                                        {/* Title-cased for display only — the value
                                                            saved and matched on is still the raw
                                                            string from the client record. Same
                                                            treatment as the header chips, so the
                                                            field and the summary above it cannot
                                                            spell the same model two ways. */}
                                                        <span className={`truncate ${service ? 'text-heading' : 'text-faint'}`}>
                                                            {service
                                                                ? titleCaseName(businessModelDisplayName(service))
                                                                : (!clientId
                                                ...
                                                <div className="space-y-1" data-tour="sm-year">
                                                    ...
                                                    {(() => {
                                                        ...
                                                        const options = [
                                                            ...(stored ? [{
                                                                value: stored,
                                                                label: storedIsPast ? `${stored} — past year, pick again` : stored,
                                                                disabled: storedIsPast,
                                                            }] : []),
                                                        ...
                                                        return (
                                                            <ListSelect
                                                                value={year}
                                                                options={options}
                                                                onChange={(next) => { clearFieldError('year'); setYear(next) }}
                                                                placeholder="Select a year"
                                                                emptyLabel="No years available"
                                                                ariaLabel="Service providing year"
                                                    ...
                                                    <ListSelect
                                                        ...
                                                        options={[
                                                            ...
                                                            ...serviceModels
                                                                ...
                                                                .map((m) => ({ value: m, label: titleCaseName(serviceModelDisplayName(m)) })),
                                                            ...
                                                            ...availableModels.map((modal) => ({
                                                                value: modal.name,
                                                                label: titleCaseName(serviceModelDisplayName(modal.name)),
                                                            })),
                                                        ]}
...
<body>${clientPages}</body>
    ...
    return (
        <Shell>
            {reportOpen && <ServiceReportDialog catalogue={reportCatalogue} facets={facets} initialFilters={pageFilters} onClose={() => setReportOpen(false)} />}
            ...
            <motion.div variants={pageEnter} initial="hidden" animate="visible" className="flex h-full flex-col min-h-0 min-w-0">
                <div className="flex flex-1 min-h-0 flex-col px-4 sm:px-6 md:px-8 pt-3 pb-3">
                    ...
                    <div className="no-print mt-3 flex shrink-0 items-center gap-2 flex-wrap min-w-0">
                        ...
                        {/* Shared Button throughout, matching Course Setup:
                            outline for the supporting action, solid brand for
                            the two that lead somewhere. */}
                        ...
                        {canMap && (
                            <>
                                ...
                                <Button type="button" size="sm" className="text-xs" onClick={() => openCreate()}>
                    ...
                    {showReportFilters && <div id="service-mapping-filters" className="no-print mt-3 shrink-0 rounded-xl border border-hairline bg-canvas/40 p-3">
                        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                            ...
                            <div className="min-w-0"><p className="mb-1.5 text-xs font-medium text-subtle">Service models</p><MappingMultiFilter label="Service models" options={availableServiceModels.map((value) => ({ value, label: value }))} value={serviceModelsFilter} onChange={setServiceModelsFilter} emptyLabel={clientScope ? 'No service models for the selected clients' : undefined} /></div>
                            <div className="min-w-0 [&>button]:w-full"><p className="mb-1.5 text-xs font-medium text-subtle">Year range</p><MappingYearRange from={yearFilter} to={yearToFilter} years={availableYears} onChange={(from, to) => { setYearFilter(from); setYearToFilter(to) }} /></div>
                        </div>
                            ...
                            <DataTable<ClientRow>
                                ...
                                columns={[
                                    ...
                                    },
                                    {
                                        ...
                                        label: 'Client',
                                        sortKey: 'client',
                                        className: 'w-[29%] px-3 text-left align-middle',
                                        skeletonWidth: '60%',
                                    ...
                                    },
                                    {
                                        key: 'actions',
                                        label: 'Actions',
                                        className: 'w-[27%] no-print px-3 text-center align-middle',
                                        skeletonWidth: '85%',
                                        render: (row) => {
                                            if (!canMap && !canView) {
                                            ...
                                            return (
                                                <div className="flex flex-wrap items-center justify-center gap-1.5 py-1">
                                                    {canView && (
                                                        <button
                                                            type="button"
                                                            aria-label={`Manage services for ${row.client.clientCompany}`}
                                                            onClick={(e) => { e.stopPropagation(); setDetailsClientId(row._id) }}
                                                            className="inline-flex min-h-8 max-w-full items-center justify-center rounded-control border border-hairline-strong bg-surface px-2.5 py-1.5 text-xs font-medium leading-4 text-body transition-colors hover:border-line-hover hover:bg-row-hover hover:text-heading focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/30"
                                                        >
                                                            Manage Services
                                                        </button>
                                                    )}
                                                    {canMap && (
                                                        <button
                                                            type="button"
                                                            aria-label={`Add services for ${row.client.clientCompany}`}
                                                            onClick={(e) => { e.stopPropagation(); openCreate(row._id) }}
                                                            className="inline-flex min-h-8 max-w-full items-center justify-center rounded-control border border-brand-500/20 bg-brand-wash px-2.5 py-1.5 text-xs font-medium leading-4 text-brand-strong transition-colors hover:border-brand-500/40 hover:bg-brand-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/30"
                                                        >
                                                            Add Services
                                                        </button>
                                                    )}
