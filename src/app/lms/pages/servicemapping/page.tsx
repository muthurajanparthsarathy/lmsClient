...
import {
    ...
    MoreVertical, ChevronLeft, ChevronRight, Eye,
    Printer, FileSpreadsheet, FileText, CalendarDays, CircleCheck, Users,
    CheckCircle2,
} from 'lucide-react'
...
import PlacementStructureForm from './components/PlacementStructureForm'
// DEFAULT_BATCH_COUNT is no longer imported here: the wizard opens with batches
// OFF, and the only thing that writes that number is the checkbox inside the
// form itself.
...
import { EmptyState, pageEnter, useModalBlink } from '../../shared/ui'
import { buildRowVM, extractHierarchy, ClientAvatar, formatServiceModel, isB2BService, type MappingRowVM } from './components/workspaceShared'
import MappingHierarchyCards from './components/MappingHierarchyCards'
import ServiceMappingOverview from './components/ServiceMappingOverview'
import { MappingYearFilter, MappingMultiFilter } from './components/MappingReportFilters'
import ClientToolbarFilter from '../clientmanagement/features/ClientToolbarFilter'
...
import { scopeByClients, pruneToScope, collapseAll } from './components/filterScope'
...
const serviceModelDisplayName = (value: string): string =>
    value.trim().toLowerCase().replace(/\s+/g, ' ').replace(/(^|[\s-])\S/g, (word) => word.toUpperCase())
...
const PHASE_NUMERALS = ['I', 'II', 'III', 'IV', 'V', 'VI', 'VII', 'VIII', 'IX', 'X']

const isPlacementTraining = (serviceModels: string[]): boolean =>
    serviceModels.some((m) => /placement\s*training/i.test(m))

// The model's own name, spelled the way it is configured. Step 2 falls back to
// it when no course category matches, so the empty state can still name what it
// found nothing under.
const placementTrainingName = (serviceModels: string[]): string =>
    serviceModels.find((m) => /placement\s*training/i.test(m)) || 'Placement training'

// Category and model names are compared through this, never as raw text. The
// same category is spelled "Placement Training", "placement training" and
// "Placement  Training" across tenants, and a case- or whitespace-sensitive
// compare would resolve to no category at all — which the UI can only show as
// an empty course list, indistinguishable from a category with nothing in it.
const normaliseName = (s: string): string => s.trim().toLowerCase().replace(/\s+/g, ' ')

// The key two names are compared ON. Case, spacing and punctuation all vary
// between how a service model is typed in Dynamic field settings and how the
// matching category is typed in Course Management — "Placement Training",
// "placement-training", "Placement_Training" are one category to a human and
// three different strings to ===. Reducing both sides to letters and digits is
// what makes them meet.
const matchKey = (s: string): string => (s || '').toLowerCase().replace(/[^a-z0-9]+/g, '')

// Service and model names arrive spelled however they were typed into Dynamic
// field settings — "placement training", "PLACEMENT TRAINING", "Business to
// institution" — and the wizard shows all three side by side, where the
// disagreement is the first thing you notice. Title case fixes the display only;
// nothing is written back, and matching still runs through normaliseName.
//
// Two kinds of word survive untouched: anything holding a digit (B2B, B2I) and
// anything already fully capitalised (CSR, COE, HTD), because "B2b" and "Csr"
// would be a worse answer than the problem. Joining words stay lowercase unless
// they lead, which is what makes it "Business to Institution".
const TITLE_MINOR_WORDS = new Set(['a', 'an', 'and', 'at', 'by', 'for', 'in', 'of', 'on', 'or', 'the', 'to', 'with'])
const titleCaseName = (s: string): string =>
    s.trim().split(/\s+/).map((word, i) => {
...
// A service model's label in the wizard. B2B's models are acronyms (TD, HTD,
// CSR, COE), so under a B2B service they read in full capitals however they
// were typed into the master data; every other model keeps the title case.
// Display only — the stored name is still what is saved and matched on.
const serviceModelLabel = (name: string, service: string): string =>
    isBusinessToBusiness(service) ? name.trim().toUpperCase() : titleCaseName(serviceModelDisplayName(name))
...
type SortKey = 'clientId' | 'client' | 'service' | 'model' | 'year' | 'status'
...
// The Services count + its service-list card — the same disclosure Course
// Setup's "Available Courses" count uses: a blue link-like chip that opens a
// white card, one line per item. The client's mappings are not shipped with
// the row, so the card fetches them when it opens (the same query Manage
// Services uses, so the two share a cache). Positioned with CSS like
// RowActionsMenu below — the page zoom throws measured poppers off — and rows
// in the lower half pass `openUp` so the table's scroll box cannot clip it.
function ServicesCountPopover({ clientId, total, active, openUp }: {
    clientId: string
    total: number
    active: number
    openUp?: boolean
    ...
    const { data: mappings = [], isLoading } = useMappingsByClient(open ? clientId : null)
    useEffect(() => {
        if (!open) return
    ...
    return (
        <div ref={boxRef} className="relative inline-flex">
            ...
            {open && (
                <div
                    role="dialog"
                    aria-label={`Services (${total})`}
                    // Right-aligned to the chip, so the card grows back over the
                    // Business Model column and never past the table's right edge.
                    className={`absolute right-0 z-dropdown w-72 rounded-tile border border-hairline bg-surface text-left shadow-lg ${openUp ? 'bottom-full mb-1.5' : 'top-full mt-1.5'}`}
                >
                    <p className="px-3 pt-2.5 pb-2 text-2xs font-semibold uppercase tracking-wider text-subtle border-b border-hairline">
                        Services ({total})
                    </p>
                    {isLoading ? (
                        <p className="px-3 py-3 text-xs text-subtle">Loading services…</p>
                    ) : mappings.length === 0 ? (
                        <p className="px-3 py-3 text-xs text-subtle">No services mapped.</p>
                    ) : (
                        <ul className="max-h-52 overflow-y-auto py-1.5">
                            {mappings.map((m) => {
                                const isActive = m.status === 'active'
                                const models = (m.serviceModels || []).filter(Boolean)
                                    .map((sm) => formatServiceModel(sm, m.service)).join(', ') || m.service || '—'
                                return (
                                    <li key={m._id} className="flex items-center gap-2 px-3 py-1.5 text-[12px] text-body">
                                        <span className={`size-1.5 rounded-full flex-shrink-0 ${isActive ? 'bg-success-500' : 'bg-ink-300'}`} aria-hidden />
                                        <span className="min-w-0 flex-1 truncate">
                                            {models}
                                            <span className="text-subtle"> — {m.year || '—'}</span>
                                        </span>
                                        <span className={`flex-shrink-0 text-2xs font-medium ${isActive ? 'text-success-700' : 'text-subtle'}`}>
...
// The listing's row ⋮ menu. Positioned with CSS against its own button rather
// than measured and placed by JS: the app runs under `html { zoom: .95 }`
// (globals.css), and a measured popper lands ~5% short of its trigger — in the
// far-right Actions column that put the menu a long way from the dots. Rows
// near the bottom pass `openUp`, so the table's scroll box cannot clip it.
function RowActionsMenu({ label, openUp, items }: {
    label: string
    openUp?: boolean
    ...
    const [open, setOpen] = useState(false)
    const boxRef = useRef<HTMLDivElement>(null)
    // Close on outside click / Escape, like the other menus on this page.
    useEffect(() => {
        if (!open) return
        const onDown = (e: MouseEvent) => {
            if (boxRef.current && !boxRef.current.contains(e.target as Node)) setOpen(false)
    ...
    return (
        <div ref={boxRef} className="relative inline-flex">
            <button
                ...
                aria-expanded={open}
                onClick={() => setOpen((v) => !v)}
                className={`inline-flex size-7 items-center justify-center rounded-chip text-subtle hover:bg-ink-100 hover:text-heading transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/25 ${open ? 'bg-ink-100 text-heading' : ''}`}
            >
                ...
                >
                    {items.map((item) => (
                        <button
                            ...
                            className="flex w-full items-center gap-2 rounded-chip px-2 py-1.5 text-xs text-body transition-colors hover:bg-row-hover hover:text-heading focus-visible:bg-row-hover focus-visible:outline-none"
                        >
                            {item.icon}
                            {item.label}
                        </button>
...
function ClientCombobox({
    ...
    placeholder?: string
    /**
     * The list is already narrowed to ONE business model. Its business-model
     * filter and the model badge on every row could only repeat that one
     * answer, so both are hidden.
     */
    scopedToModel?: boolean
}) {
    const [open, setOpen] = useState(false)
    const [query, setQuery] = useState('')
    const [pickerYear, setPickerYear] = useState('')
    const [pickerModel, setPickerModel] = useState('')
    const pickerYears = useMemo(() => Array.from(new Set(clients.map((client) => client.createdAt ? new Date(client.createdAt).getFullYear() : NaN).filter(Number.isFinite))).sort((a, b) => b - a), [clients])
    ...
    const openMenu = () => {
        if (!open && boxRef.current) {
            ...
            setDropUp(up)
            const room = (up ? spaceAbove : spaceBelow) - 145 // search box + margins
    ...
    const filtered = useMemo(() => {
        ...
        return clients.filter((client) =>
            (!q || (client.clientCompany || '').toLowerCase().includes(q)) &&
            (!pickerModel || client.businessModel === pickerModel) &&
            (!pickerYear || (client.createdAt && String(new Date(client.createdAt).getFullYear()) === pickerYear))
        )
    }, [clients, query, pickerModel, pickerYear])
    ...
    return (
        <div ref={boxRef} className="relative">
            <button
                ...
                disabled={disabled}
                onClick={openMenu}
                // h-9, the height of every other Step 1 control — the picker
                // shares a row with the locked Business model box, and at h-10
                // it sat 4px lower than its neighbour.
                className={`w-full h-9 flex items-center gap-2 px-3 rounded-control border bg-surface text-left transition-colors disabled:bg-surface-sunken disabled:cursor-not-allowed ${
                    open ? 'border-brand-500 ring-2 ring-brand-500/15' : 'border-hairline-strong hover:border-line-hover'
                }`}
            >
                {selected ? (
                    // No business-model chip on the trigger. It is the field
                    // right beside this one on Step 1 ("Business model", picked
                    // first), so on the trigger it was a second copy of one fact —
                    // and the one with least room. The list rows carry it only
                    // when the list mixes models (see `scopedToModel`).
                    <>
                ...
                {open && (
                    ...
                    >
                        <div className="p-2 border-b border-hairline">
                            ...
                            <div className={`mt-2 grid gap-2 ${scopedToModel ? 'grid-cols-1' : 'grid-cols-2'}`}>
                                {!scopedToModel && (
                                    <label className="min-w-0 text-[10px] font-medium text-subtle">Business model
                                        <select aria-label="Filter clients by business model" value={pickerModel} onChange={(event) => setPickerModel(event.target.value)} className="mt-1 h-8 w-full rounded-lg border border-hairline-strong bg-surface px-2 text-xs text-body focus:outline-none focus:ring-2 focus:ring-brand/20">
                                            <option value="">All models</option>
                                            {['B2B', 'B2I', 'B2C'].map((model) => <option key={model} value={model}>{businessModelDisplayName(model)}</option>)}
                                        </select>
                                    </label>
                                )}
                                <label className="min-w-0 text-[10px] font-medium text-subtle">Created year
                                    <select aria-label="Filter clients by created year" value={pickerYear} onChange={(event) => setPickerYear(event.target.value)} className="mt-1 h-8 w-full rounded-lg border border-hairline-strong bg-surface px-2 text-xs text-body focus:outline-none focus:ring-2 focus:ring-brand/20">
                                        <option value="">All years</option>
                                        {pickerYears.map((year) => <option key={year} value={String(year)}>{year}</option>)}
                                    </select>
                                </label>
                            </div>
                            {(pickerModel || pickerYear) && <button type="button" onClick={() => { setPickerModel(''); setPickerYear('') }} className="mt-2 text-xs font-medium text-brand-strong">Clear filters</button>}
                        </div>
                                ...
                                filtered.map((c) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            {scopedToModel ? null : c.businessModel ? (
...
function ResourceDefaultsStep({
    ...
    // No heading and no blurb: the modal header already says "3. Resources",
    // and the paragraph under it explained the step to someone who had just read
    // the step's name — three lines of chrome above the only thing on screen.
    return (
        <div>
            <ResourceTypeSection
...
function MapServiceWizard({
    ...
    // Overlay clicks blink these two panels rather than closing them — a
    // half-filled wizard is too expensive to lose to a stray click. The header
    // X and Cancel still route through requestClose, which is where the
    // unsaved-changes prompt lives.
    const { ref: wizardPanelRef, blink: blinkWizard } = useModalBlink<HTMLDivElement>()
    const { ref: confirmPanelRef, blink: blinkConfirm } = useModalBlink<HTMLDivElement>()
    const [stepIndex, setStepIndex] = useState(0)
    // The step body is the scroll container, and it is the SAME element across
    // steps — so a long Step 2 left Step 3 opening halfway down itself, with its
    // first field above the fold. Reset on every step change (and on entering the
    // preview, which reuses the same pane).
    //
    // Instant, not smooth: the step is sliding in at the same moment, and two
    // animations racing over the same pixels reads as a stutter.
    ...
    // The business model CODE (B2B / B2I / B2C) picked first on Step 1 —
    // `service` above is the service name derived from it.
    ...
    //
    // It starts at ZERO — batches off — so a new mapping never renders a tick and
    // two empty required name boxes before the open effect has run. Ticking the
    // box writes DEFAULT_BATCH_COUNT.
    ...
    // ── The placement configuration's identity ───────────────────────────────
    // One course, one intake year, one semester, for the whole configuration.
    // The course used to be asked per phase (a phase's name doubled as its
    // course); it is asked once now, and a phase carries only its batches.
    //
    // `configAcademicBatch` is an intake YEAR ("2026"). It is NOT a training
    // batch — those are the named groups under a phase or the course — and the
    // two never share a field, a payload key or a column.
    const [configCourseName, setConfigCourseName] = useState('')
    const [configAcademicBatch, setConfigAcademicBatch] = useState('')
    ...
    // Read only by the LOAD path now, to size the store it rebuilds from an older
    // document. The wizard edits one course when phases are off, so nothing else
    // consults this — see placementPhaseKeys.
    const [, setNoPhaseCourseCount] = useState(1)
    // Per-field validation messages, keyed by field name ('client', 'courseName',
    // 'batch-0', 'sem:<path>:<i>:courseName'…) so a failed Next points at the
    ...
    useEffect(() => {
        ...
        if (editingMapping) {
            ...
            // The select shows the code. A saved service that is none of the
            // three (a legacy CSR mapping) matches no pattern, leaves this blank,
            // and the select shows the service's own name instead.
            setBusinessModel(BUSINESS_MODELS[ALLOWED_SERVICE_PATTERNS.findIndex((p) => p.test(editingMapping.service || ''))] || '')
            // The saved year. Legacy mappings without one fall back to the
            // current year so the locked field is never empty.
            ...
            if (!loadedPlacement) {
                ...
                // Read from courseConfigurations, which the server always sends:
                // a mapping saved before they existed has one derived from its
                // old masterData + courses on the way out, so this branch does
                // not need to know which era the document is from.
                const savedConfig = (editingMapping.courseConfigurations || [])[0]
                if (savedConfig) {
                    setConfigCourseName(savedConfig.courseName || '')
                    setConfigAcademicBatch(savedConfig.academicBatch || editingMapping.year || '')
                    setConfigSemester(savedConfig.semester || '')
                } else {
                    // Not a placement mapping (or nothing saved yet): fall back to
                    // the mapping's own first course, so the field is never blank
                    // while the document plainly holds a course.
                    setConfigCourseName(editingMapping.courseName || '')
                    setConfigAcademicBatch(editingMapping.year || '')
                    setConfigSemester('')
                }
            ...
            setService('')
            setBusinessModel('')
            // Current year for a new mapping. Step 1 shows it locked, so this is
            // the year the mapping is saved with.
            ...
            setSemesterCourses({})
            // Batches start OFF on a new mapping. The count IS the enabled state,
            // so seeding it at DEFAULT_BATCH_COUNT opened every new mapping with
            // batches already ticked and two empty name boxes demanding to be
            // filled — a required field nobody asked for, on a step where most
            // services run no batches at all. Ticking the box still writes
            // DEFAULT_BATCH_COUNT, so opting IN lands on two as before.
            setBatchCount(0)
            setBatchNames([])
            setBatchPhases([])
            setBatchPhasesEnabled([])
            setPhaseNames([])
            setConfigCourseName('')
            // Defaults to the year the mapping is being created for; the picker
            // can move it, since an intake is not always the current year.
            setConfigAcademicBatch(String(new Date().getFullYear()))
            setConfigSemester('')
    ...
    // The business model is picked on its own select, BEFORE the client. A
    // preset client (the wizard opened from a client's row) arrives with no
    // model picked yet, so the model is filled in from that client instead.
    useEffect(() => {
        if (!open || editingMapping || businessModel) return
        const bm = clients.find((c) => c._id === clientId)?.businessModel || ''
        if (BUSINESS_MODELS.includes(bm)) setBusinessModel(bm)
    }, [open, editingMapping, businessModel, clientId, clients])

    // The mapping's service follows the picked business model — the servicesList
    // option at the same index (real service id + models when the master list
    // has it). Re-runs when that list loads, so a model picked before it arrived
    // still lands on the real service. Editing keeps the saved service.
    useEffect(() => {
        if (!open || editingMapping) return
        const idx = BUSINESS_MODELS.indexOf(businessModel)
        const next = idx >= 0 ? servicesList[idx]?.name || businessModel : ''
        ...
        setPrtMode('')
        setPartnerInstitutionIds([])
    }, [open, editingMapping, businessModel, servicesList, service])
    ...
    // One model per mapping. The state is an ARRAY because that is the shape the
    // payload and the legacy read models want, not because several can be picked
    // — choosing always replaces, and the blank option clears.
    // Everything Step 2 holds. The service model decides what that step even ASKS
    // — a degree hierarchy, phases and their courses, or one flat course with
    // batches — so structure entered under one model is not partial work under
    // another, it is data for a question that is no longer being asked. Worse, it
    // is invisible: the new model's form renders none of it, but the old values
    // stay in the store and go out in the payload at save time.
    const clearStructureStep = () => {
        setCourses([blankCourse()])
        setSemesterCourses({})
        setBatchCount(0)
        setBatchNames([])
        ...
        setConfigSemester('')
        setNoPhaseCourseCount(1)
        // The degree hierarchy and its dates — degrees, departments, sections,
        // semesters and every window set against them.
        setMasterData({})
        setGroupedData({})
        setDegreeStartYears({})
        setDegreeEndYears({})
        setSemesterDates({})
        // Messages for fields that no longer exist. Step 1's own keys survive:
        // the user is standing on that step and may be looking at one.
        setFieldErrors((prev) => {
            const next: Record<string, string> = {}
            Object.entries(prev).forEach(([k, v]) => {
                const isStructureKey = k.startsWith('sem:')
                    || k.startsWith('semdate:')
                    || k.startsWith('phase-')
                    || k.startsWith('batch-')
                    || k.startsWith('courseName-')
                    || k.startsWith('courseCategory-')
                    || k === 'hierarchy'
                    || k === 'degreeStartYear'
                if (!isStructureKey) next[k] = v
            })
            return next
        })
    }
    ...
    const chooseModel = (name: string) => {
        ...
        setServiceModels(name ? [name] : [])
        // The structure step is model-driven — reset the per-model choices…
        setPrtMode('')
        setDivDegreeEnabled(false)
        // …and everything those choices shaped. Only reachable on an UNSAVED
        // mapping: `modelLocked` returns above once a mapping exists, so this can
        // never throw away structure that has already been saved.
        clearStructureStep()
    }

    // Business model comes first and the client list is narrowed to it, so a
    // client picked under a different model no longer belongs there — drop it.
    // Closed once the service model built on it is final (editing, or a
    // partial save), for the same reason the model is.
    const chooseBusinessModel = (code: string) => {
        if (isEditing || modelLocked) return
        clearFieldError('service')
        setBusinessModel(code)
        const current = clients.find((c) => c._id === clientId)
        if (current && current.businessModel !== code) setClientId(null)
    }
    const clientsForModel = businessModel
        ? clients.filter((c) => c.businessModel === businessModel)
        : []

    // B2C mappings have no service model: the field is not rendered, not
    // validated, and Step 2 falls through to its plain course + batches form.
    const isB2C = businessModel === 'B2C'

    // Service offering date — the day the mapping was made: its saved createdAt
    // when editing, today for a new one. Date only ("15 Sep 2026", the listing's
    // own format), no time of day. Only the YEAR is stored on the mapping, so a
    // legacy mapping whose saved year differs from the year it was created in
    // (the old picker allowed future years) shows that saved year alone rather
    // than a date that contradicts it.
    const offeringDate = (() => {
        const made = editingMapping?.createdAt ? new Date(editingMapping.createdAt) : new Date()
        if (isNaN(made.getTime()) || (year && String(made.getFullYear()) !== year)) return year || '—'
        return made.toLocaleDateString('en-GB', { day: '2-digit', month: 'short', year: 'numeric' })
    })()
    ...
    const { categoriesData, isLoadingCategories } = useCourseData({
        isOpen: open,
        isEditMode: false,
        formData: courseForm,
    ...
    // Every course Course Management holds, flattened across ALL categories.
    // The degree tree's rows have no category of their own to filter by, so
    // scoping this list to one would hide most of the catalogue from them.
    // Deduped because the same course name can be filed under two categories,
    // and two identical rows in a picker are indistinguishable to whoever is
    // choosing; sorted because a flat list assembled in category order reads as
    // no order at all.
    const allCourseNames = useMemo(() => {
        const seen = new Set<string>()
        categoryOptions.forEach((c) => (c.courseNames || []).forEach((n) => {
            const v = (n || '').trim()
            if (v) seen.add(v)
        }))
    ...
    // The course category this mapping's service offering already owns — the one
    // Step 2's course picker lists from. The mapping never creates a category or
    // a course; it only points at ones Course Management has.
    const serviceCourseCategory = useMemo(() => {
        // A mapping can carry several models (Placement Training alongside CSR,
        // say). On a placement mapping the courses come from the PLACEMENT
        // category — matching against the whole list would resolve to whichever
        // category happened to sit earlier in Course Management.
        const preferred = isPlacement ? [placementTrainingName(serviceModels)] : serviceModels
        const wanted = preferred.map(matchKey).filter(Boolean)
        if (!wanted.length) return undefined

        // Four passes, each looser than the last. Anything stricter left the
        // picker empty with no way to tell a missing category from an empty one.
        //   1. the category's NAME
        //   2. its CODE — some tenants name the category loosely ("Placement")
        //      and carry the exact model string in the code instead
        //   3. either name CONTAINING the other, which is what matches
        //      "Placement Training Programme" to the "Placement Training" model
        //   4. the same pattern that identified the flow in the first place
        const byField = (pick: (c: CourseCategory) => string | undefined) =>
            categoryOptions.find((c) => {
                const key = matchKey(pick(c) || '')
                return Boolean(key) && wanted.includes(key)
            })
        return byField((c) => c.categoryName)
            || byField((c) => c.categoryCode)
            || categoryOptions.find((c) => {
                const key = matchKey(c.categoryName)
                return Boolean(key) && wanted.some((w) => key.includes(w) || w.includes(key))
            })
            || (isPlacement
                ? categoryOptions.find((c) => /placement\s*training/i.test(c.categoryName))
                : undefined)
    ...
    // With phases off the mapping carries exactly ONE course — the wizard has no
    // control that adds a second any more. It used to derive this list from
    // `noPhaseCourseCount`, which the load path still restores from older
    // documents: those extra 'phd<i>' keys were then seeded, VALIDATED and
    // serialised while having no field on screen, so a batch error on one of them
    // failed Next with nothing highlighted and nothing to scroll to.
    //
    // Their saved data is not discarded — see the mappedCourses builder, which
    // passes any surviving 'phd<i>' course through untouched.
    ...
    const applyPhaseCount = (n: number) => {
        ...
        //
        // The ONE exception is Phase 1 when phases are switched on, which is a
        // move rather than an addition: the course the mapping already carried
        // becomes the first phase's course. Its entry migrates from 'ph' to 'ph0'
        // below — but a direct course keeps its NAME on the entry while a phase
        // keeps it in phaseNames, so without this the user ticks the box and
        // finds the course they picked (and saved) replaced by an empty picker,
        // with only its batches still there.
        // Phases are named "Phase I", "Phase II", … A phase's name used to be
        // the course it delivered, which is why it arrived empty for the user to
        // fill; the course is now a field of its own above, so a phase can carry
        // the only name it ever really had — its position.
        setPhaseNames((prev) => Array.from(
            { length: count },
            (_, i) => prev[i] ?? `Phase ${PHASE_NUMERALS[i] || i + 1}`
        ))
        ...
        setSemesterCourses((prev) => {
            ...
            if (prevCount === 0 && count > 0 && next['ph']) {
                // Phase 1 inherits the unphased course's batches — but ONLY if it
                // had any. The root entry starts batch-less (batches are opt-in
                // without phases), so migrating it wholesale gave Phase 1 an
                // unticked Batches box and no fields while Phase 2, freshly
                // seeded below, got the promised two. A phase always arrives with
                // batches; that is what the screen shows and what the seed for
                // every OTHER phase does.
                const carried = (next['ph'] || [])[0]
                const carriedHasBatches = Boolean(carried && carried.batchesEnabled && carried.batches.length)
                next['ph0'] = next['ph0'] || [carriedHasBatches
                    ? carried
                    : { ...blankCourse(), batchesEnabled: true, batches: ['Batch I', 'Batch II'] }]
            }
            if (prevCount > 0 && count === 0 && next['ph0']) {
                // Phase 1's batches become the course's batches. The phase's NAME
                // is not carried across: it used to be written onto the entry as
                // its course name, from the model where a phase's name WAS the
                // course it delivered. Under the current model that stamped the
                // literal "Phase I" onto the course — which then showed up in the
                // preview, in the saved course name, and as a "custom" course
                // nobody had typed.
    ...
    // unseeded key would swallow the first edit.
    //
    // A PHASE is seeded with its batches already on and two of them named, which
    // is the state the screens show: a phase exists to be delivered in groups, so
    // arriving at an unticked box and no fields would mean two clicks before the
    // common case. The checkbox is still there to say otherwise. The root key —
    // the mapping with no phases — keeps the plain opt-out seed, because plenty
    // of courses run undivided.
    useEffect(() => {
    ...
    const validateStep = (step: number): boolean => {
        ...
        if (step === 0) {
            ...
            if (!service.trim()) errs.service = 'Please select a business model'
            ...
            if (!isB2C && !serviceModels.length) errs.serviceModel = 'Please select a service model'
        ...
        if (step === 1) {
            ...
            if (isDegreeFlow) {
                ...
                Object.entries(semesterCourses).filter(([path]) => path.includes(PATH_SEP)).forEach(([path, list]) => {
                    ...
                    list.forEach((c, i) => {
                        // No category to check — the row asks for a typed name
                        // and nothing else. Two courses are the same course
                        // when they are spelled the same, so the name alone is
                        // the duplicate key; it used to be prefixed with the
                        // category, which let one semester hold the same name
                ...
                // The rules are unchanged; only the wording follows the control.
                // A phase's name is no longer typed — it is the course picked for
                // that phase — so "Name this phase" would point at a field that
                // does not exist.
                // The course and semester belong to the CONFIGURATION now, so
                // they are validated once rather than per phase.
                if (!configCourseName.trim()) errs.configCourseName = 'Select a course'
                if (!configSemester.trim()) errs.configSemester = 'Select a semester'
                ...
                placementPhaseKeys().forEach((key) => {
                    ...
                    // Nothing to check per course any more: the single course
                    // is the configuration's, checked once above.
                    if (!c) return
                ...
                if (Object.keys(errs).some((k) => k.startsWith('sem:') || k.startsWith('phase-') || k.startsWith('config'))) {
    ...
    const handleSave = async (uptoStep: number = steps.length - 1) => {
        ...
        const mappedCourses = isDegreeFlow
            ...
            ? Object.entries(semesterCourses).filter(([path]) => path.includes(PATH_SEP)).flatMap(([path, list]) =>
                list
                    // A typed name is the whole answer, so it is the whole
                    // test. Requiring a category here would silently drop every
                    // course entered since the category field was removed.
                    // `category` is still written through: rows loaded from a
                    // mapping made under the old shape keep what they recorded.
            ...
            ? placementPhaseKeys().flatMap((key, i) => {
                ...
                // The COURSE is the configuration's; the phase name is only
                // the path that says where under it this entry sits. They used to
                // be the same string, which is why `courseName` read a phase name
                // for every phased mapping.
                const phaseName = (phaseNames[i] || '').trim()
                const name = configCourseName.trim() || (c?.courseName || '').trim()
                if (!name) return []
                if (phaseNames.length && !phaseName) return []
                return [{
                    category: serviceCourseCategory?.categoryName || '',
                    courseName: name,
                    path: phaseNames.length ? phaseName : '',
        ...
        // A placement mapping saved before this step carried ONE course could
        // hold several path-less courses. The wizard no longer edits them, so
        // they are written back exactly as they were loaded rather than dropped —
        // removing saved data is not something a save the user did not ask for
        // should do.
        const legacyExtraPlacementCourses = isPlacementCourses && !phaseNames.length
            ? Object.entries(semesterCourses)
                .filter(([key]) => /^phd[0-9]+$/.test(key))
                .flatMap(([, list]) => (list || []).slice(0, 1))
                .filter((c) => c && c.courseName.trim())
                .map((c) => ({
                    category: c.category || '',
                    courseName: c.courseName.trim(),
                    path: '',
        ...
        // The structure in the shape the server stores it: one configuration for
        // the course, its phases, and its training batches with the phase each
        // belongs to. Client-side phase ids are positional and temporary — the
        // server re-mints them and re-points the batches through the ids it
        // issues, which is what stops a batch ever referencing a phase from
        // another configuration.
        //
        // The legacy `mappedCourses` / masterData above are still written beside
        // this: Course Setup and the embedded client.services read model both
        // consume them, so dropping them here would break readers that have
        // nothing to do with this wizard.
        const courseConfigurations = isPlacementCourses && configCourseName.trim()
            ? [{
                courseName: configCourseName.trim(),
                courseCategory: serviceCourseCategory?.categoryName || '',
                academicBatch: configAcademicBatch.trim(),
                semester: configSemester.trim(),
                phaseConfigEnabled: phaseNames.length > 0,
                batchesEnabled: phaseNames.length > 0
                    ? true
                    : Boolean((semesterCourses['ph'] || [])[0]?.batchesEnabled),
                phases: phaseNames.map((name, i) => ({
                    _id: `phase-${i}`,
                    name: (name || '').trim(),
                })),
                trainingBatches: phaseNames.length
                    ? phaseNames.flatMap((_, i) => {
                        const entry = (semesterCourses[`ph${i}`] || [])[0]
                        // A phase that unticked Batches contributes none. The
                        // names stay in the store so reticking brings them back,
                        // but they are not what the mapping delivers.
                        if (!entry?.batchesEnabled) return []
                        return (entry?.batches || [])
                            .map((b) => (b || '').trim())
                            .filter(Boolean)
                            .map((name) => ({ name, phaseId: `phase-${i}` }))
                    })
                    : (((semesterCourses['ph'] || [])[0]?.batchesEnabled
                        ? (semesterCourses['ph'] || [])[0]?.batches || []
                        : []) as string[])
                        .map((b) => (b || '').trim())
                        .filter(Boolean)
                        // No phase id at all, rather than a placeholder phase —
                        // absence is what the unphased shape means.
                        .map((name) => ({ name, phaseId: null })),
            }]
            : []
        ...
        const result = await onSave(
            {
                ...
                // was entered under.
                courses: [...mappedCourses, ...legacyExtraPlacementCourses],
                ...
                batchConfigs,
                // Placement's structure in the shape the server stores it —
                // phases and training batches as records with their own ids and
                // order, rather than names buried in masterData.
    ...
    const buildPreviewModel = (): Pick<MappingPreviewProps, 'degree' | 'batches' | 'flatCourses' | 'phasesTree'> => {
        const completeAt = (path: string) =>
            (semesterCourses[path] || []).filter((c) => c.courseName.trim())
        if (isDegreeFlow) {
        ...
        if (isPlacementCourses) {
            ...
            const entryAt = (key: string) => {
                ...
                // configuration's own course, whether or not phases are on. It
                // used to read the phase name for a phased mapping and the
                // entry's `courseName` otherwise — and since the rewrite moved
                // the course to its own field, nothing writes that any more, so
                // the preview said "No course" for every unphased mapping and
                // named the PHASE for every phased one.
                const name = (configCourseName.trim() || c?.courseName || '').trim()
                return {
            ...
            return {
                phasesTree: phaseNames.length
                    ? phaseNames.map((p, i) => ({ name: p.trim() || `Phase ${i + 1}`, ...entryAt(`ph${i}`) }))
    ...
    // The heading now carries the position itself ("2. Structure & course"), so
    // the form needs no line under it at all — a "Step 2 of 3" underneath was
    // saying the same thing twice, and the rail on the left says it a third
    // time. Only the preview keeps a subtitle, and it reports state rather than
    // position.
    const subtitle = phase === 'preview' ? 'Saved — review the structure below' : ''
    ...
    //
    // Three labelled facts rather than the sentence this used to be ("Service
    // offering business to institution · placement training for Demo client").
    // The sentence read as prose and buried the three answers the user is
    // actually checking, and it could only ever be scanned by reading it whole.
    // Empty entries drop out, so Step 1 fills the row in as it is answered.
    const headerClientName = clients.find((c) => c._id === clientId)?.clientCompany
    const headerContext = [
        // Same order as the Step 1 fields: business model, client, service.
        //
        // `service` is the BUSINESS model (B2B / B2I / B2C); the service models
        // are the offerings under it (Placement Training, Degree Program, …).
        //
        // Each goes through this app's display-name helper FIRST — that is what
        // turns a stored value into the label configured for it — and is title
        // cased on top, so the three chips agree on capitalisation without
        // overriding what the settings table says a model is called.
        { label: 'Business model', value: service && titleCaseName(businessModelDisplayName(service)) },
        // The client's name is left exactly as it was entered — a company name is
        // whatever its owner spells it, not a phrase to normalise.
        { label: 'Client', value: headerClientName },
        {
            label: 'Service',
            value: serviceModels.map((m) => serviceModelLabel(m, service)).join(' · '),
        },
    ...
    const courseApi: CourseApi = {
        ...
        filledCountUnder: (prefix) => {
            ...
            Object.entries(semesterCourses).forEach(([path, list]) => {
                ...
                total += list.length
                filled += list.filter((c) => c.courseName.trim()).length
            })
            return { filled, total }
        },
    ...
    return (
        ...
        <AnimatePresence>
            {open && (
                <motion.div
                    ...
                    onClick={(e) => { if (e.target === e.currentTarget && !isSaving) blinkWizard() }}
                >
                    <motion.div
                        ref={wizardPanelRef}
                    ...
                    >
                        ...
                        <div className="px-5 py-3 border-b border-hairline flex items-center justify-between gap-3 flex-shrink-0 bg-surface">
                            <div className="min-w-0 flex-1">
                                <h2 className="text-base font-semibold text-heading truncate">
                                    ...
                                    {phase === 'preview' ? 'Preview' : `${stepIndex + 1}. ${steps[stepIndex].label}`}
                                </h2>
                                {subtitle && <p className="text-2xs text-faint truncate">{subtitle}</p>}
                                {/* Context — the Step 1 answers this mapping is
                                    built on. Label in the muted voice, value in a
                                    chip, so the three facts can be picked out at a
                                    glance instead of read. Hidden on every form
                                    step: Step 1 IS those fields, Step 2 opens with
                                    them as locked inputs, and Step 3 needs the room
                                    more than the reminder — so only the preview
                                    keeps them. */}
                                {headerContext.length > 0 && phase !== 'form' && (
                                    <dl className="mt-1.5 flex flex-wrap items-center gap-x-4 gap-y-1">
                                        {headerContext.map(({ label, value }) => (
                                            <div key={label} className="flex min-w-0 items-center gap-1.5">
                                                <dt className="text-2xs text-faint flex-shrink-0">{label}</dt>
                                                <dd
                                                    title={value}
                                                    className="max-w-[220px] truncate rounded-chip border border-brand-200 bg-brand-wash px-1.5 py-0.5 text-2xs font-semibold text-brand-strong"
                                                >
                                                    {value}
                                                </dd>
                                            </div>
                            ...
                            <button
                                ...
                                disabled={isSaving}
                                className="w-7 h-7 rounded-full flex items-center justify-center bg-danger-500 text-white hover:bg-danger-700 transition-colors flex-shrink-0 disabled:opacity-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-danger-500/40"
                                aria-label="Close"
                            >
                                <X size={15} />
                            </button>
                        ...
                        {phase === 'preview' ? (
                            <div className="flex-1 min-h-0 bg-surface flex flex-col">
                                <MappingPreview
                                    ...
                                    service={service}
                                    modelName={serviceModels.map((m) => serviceModelLabel(m, service)).join(' · ')}
                                    year={year}
                        ...
                        {/* scrollbar-gutter:stable — the gutter is reserved whether
                            or not the step currently scrolls, so the content width
                            is the SAME either way. Without it, anything that makes
                            the body taller (ticking Phase configuration, adding a
                            semester's courses) summons a scrollbar and silently
                            narrows every row above it, which is how a field ends
                            up jumping to the next line because of an edit made
                            somewhere else on the form. */}
                        <div ref={bodyRef} className="flex-1 min-h-0 px-5 py-4 bg-surface overflow-y-auto [scrollbar-gutter:stable]">
                            <AnimatePresence mode="wait">
                                ...
                                >
                                    ...
                                    {stepIndex === 0 && (
                                        // One two-column grid, read left to right:
                                        //   Business model · Client
                                        //   Service model  · Service offering year
                                        // then the partner list (CSR/COE) across both columns.
                                        // gap-y rather than space-y: margins on grid items would
                                        // knock the second column out of line with the first.
                                        <div className="grid grid-cols-1 sm:grid-cols-2 items-start gap-x-3 gap-y-4 max-w-2xl">
                                            <div className="space-y-1 min-w-0" data-tour="sm-service">
                                                <FieldLabel required hint="Pick this first; the client list and the service models follow it">Business model</FieldLabel>
                                                {/* A real choice, made BEFORE the client: each row shows
                                                    the code with its full name under it. Closed only when
                                                    the model built on it is final (editing, or a partial
                                                    save). A legacy saved service that is none of the three
                                                    still shows as itself rather than reading blank. */}
                                                <ListSelect
                                                    value={businessModel || service}
                                                    options={[
                                                        ...(service && !businessModel ? [{ value: service, label: titleCaseName(service) }] : []),
                                                        ...BUSINESS_MODELS.map((code) => ({
                                                            value: code,
                                                            label: businessModelDisplayName(code),
                                                        })),
                                                    ]}
                                                    onChange={chooseBusinessModel}
                                                    disabled={isEditing || modelLocked}
                                                    title={isEditing || modelLocked ? 'Fixed once the mapping is saved' : undefined}
                                                    placeholder="Select a business model"
                                                    emptyLabel="No business models available"
                                            ...
                                            <div className="space-y-1 min-w-0" data-tour="sm-client">
                                                <FieldLabel required hint="Only clients with the chosen business model are listed">Client</FieldLabel>
                                                {/* Narrowed to the business model beside it, so the picker's
                                                    own business-model filter and the badge on every row could
                                                    only repeat that choice — both are hidden. Editing passes
                                                    the full list: the saved client must still be found to be
                                                    shown. */}
                                                <ClientCombobox
                                                    clients={isEditing ? clients : clientsForModel}
                                                    value={clientId}
                                                    onChange={(id) => { clearFieldError('client'); setClientId(id) }}
                                                    disabled={isEditing || !businessModel}
                                                    placeholder={businessModel ? 'Select a client…' : 'Select a business model first'}
                                                    scopedToModel
                                                />
                                                {businessModel && !isEditing && clientsForModel.length === 0 && (
                                                    <p className="text-2xs text-warn-700">
                                                        No clients with this business model yet — add one in the Clients tab.
                                                    </p>
                                                )}
                                            ...
                                            {/* Service models — CSR and COE are picked here (under B2B),
                                                ...
                                                Not rendered at all for B2C, which has no service model. */}
                                            {!isB2C && (
                                            <div className="space-y-1.5 min-w-0" data-tour="sm-model">
                                                ...
                                                {!service ? (
                                                    <p className="text-xs text-subtle">Select a business model to see its service models.</p>
                                                ) : availableModels.length === 0 && serviceModels.length === 0 ? (
                                                    // A business model picked before the services list has
                                                    // arrived has no models YET — saying "none configured"
                                                    // then would send the user off to fix a problem that
                                                    // is not there.
                                                    <p className="text-xs text-subtle">
                                                        {isLoadingServices ? 'Loading service models…' : 'No service models configured for this service.'}
                                                    </p>
                                                ) : (
                                                    // A list of tiles for a one-of-N answer made the
                                                    // choice hard to read back: with everything the same
                                                    // size, "which one is selected" came down to spotting
                                                    // a tick. A select states it in words. Once saved the
                                                    // field is simply DISABLED — the greyed control says
                                                    // "not yours to change" without a lock icon on every
                                                    // tile and a paragraph explaining them.
                                                    <ListSelect
                                                        value={serviceModels[0] || ''}
                                                        options={[
                                                            // A model picked before it left the master
                                                            // list still has to appear, or the field
                                                            // would read blank while holding a value.
                                                            ...serviceModels
                                                                .filter((m) => !availableModels.some((am) => am.name === m))
                                                                .map((m) => ({ value: m, label: serviceModelLabel(m, service) })),
                                                            // Display only — the value carried is the
                                                            // raw name, which is what the mapping stores
                                                            // and matches on. The label goes through the
                                                            // app's own display-name helper first, then
                                                            // title case.
                                                            ...availableModels.map((modal) => ({
                                                                value: modal.name,
                                                                label: serviceModelLabel(modal.name, service),
                                                            })),
                                                        ]}
                                                        onChange={chooseModel}
                                                        disabled={modelLocked}
                                                        title={modelLocked ? 'Fixed once the mapping is saved' : undefined}
                                                        placeholder="Select a service model"
                                                        emptyLabel="No service models configured for this service."
                                                        ariaLabel="Service model"
                                                        searchPlaceholder="Search service models…"
                                                        invalid={Boolean(fieldErrors.serviceModel)}
                                                        className="w-full"
                                                    />
                                                )}
                                                {/* The guidance line doubles as the error slot — showing both
                                                    would say the same thing twice in two colours. Once saved
                                                    there is no line at all: the disabled control already says
                                                    the field is closed, and a banner repeating it was three
                                                    sentences the user had read on the way in. */}
                                            ...
                                            <div className="space-y-1 min-w-0" data-tour="sm-year">
                                                <FieldLabel required hint="Set automatically: today's date for a new mapping, or the date a saved mapping was made">Service offering date &amp; year</FieldLabel>
                                                {/* Read-only. A new mapping shows today and an edited one the
                                                    day it was made, so there is nothing here to pick. The
                                                    mapping itself still stores the year. Same locked look as
                                                    the Step 2 summary row. */}
                                                <div
                                                    ...
                                                    title="Set automatically; it can't be changed here"
                                                    aria-disabled="true"
                                                >
                                                    <Lock size={11} className="text-faint flex-shrink-0" />
                                                    <span className="truncate tabular-nums text-heading">
                                                        {offeringDate}
                                                    </span>
                                                </div>
                                            ...
                                            {/* Partner institutions — CSR (business model) or COE (service model
                                                ...
                                                Client Management records. Spans both columns. */}
                                            {isPartnerFlow && (
                                                <div className="space-y-1 sm:col-span-2" data-tour="sm-partner">
                                                                ...
                                                                <div key={idx} className="flex items-center gap-2">
                                                                    <div className="flex-1 min-w-0">
                                                                        <ClientCombobox
                                                                            ...
                                                                            // B2I clients only (rowClients above).
                                                                            scopedToModel
                                                                        />
                                                                    </div>
                                                                    {showRemove && (
                                    ...
                                    {stepIndex === 1 && (
                                        ...
                                        // Every non-degree flow now lays Step 2 out the same
                                        // way — course on the left, batches and their names
                                        // on the right. That pair needs room: the course
                                        // label, its picker, the Batches checkbox, the count
                                        // label and the count box all sit on ONE line, and
                                        // anything narrower than 4xl pushed the checkbox onto
                                        // a line of its own. Still a form, not the degree
                                        // flow's full-width workbench.
                                        <div className={isDegreeFlow ? 'w-full' : 'max-w-4xl'}>
                                            <div className={isDegreeFlow ? 'space-y-2' : 'space-y-4'}>
                                                {/* Step 1's answers, repeated as one locked row so the
                                                    structure below is built with them in view. Read-only —
                                                    change them by going Back to Step 1. Same locked-field
                                                    look as Step 1's Business model. */}
                                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 px-1">
                                                    {[
                                                        { label: 'Business model', value: service ? titleCaseName(businessModelDisplayName(service)) : '' },
                                                        { label: 'Client', value: headerClientName || '' },
                                                        ...(isB2C ? [] : [{ label: 'Service model', value: serviceModels.map((m) => serviceModelLabel(m, service)).join(' · ') }]),
                                                        { label: 'Service offering date & year', value: offeringDate },
                                                    ].map(({ label, value }) => (
                                                        <div key={label} className="space-y-1 min-w-0">
                                                            <FieldLabel>{label}</FieldLabel>
                                                            <div
                                                                className="w-full h-9 text-sm border border-hairline-strong rounded-control px-2.5 bg-surface-sunken flex items-center gap-1.5 cursor-not-allowed select-none"
                                                                title={value ? `${value} — change it in Step 1` : 'Set in Step 1'}
                                                                aria-disabled="true"
                                                            >
                                                                <Lock size={11} className="text-faint flex-shrink-0" />
                                                                <span className={`truncate ${value ? 'text-heading' : 'text-faint'}`}>{value || '—'}</span>
                                                            </div>
                                                        </div>
                                                    ))}
                                                </div>
                                                ...
                                                {isDIVModel(serviceModels) && !isDegreeProgramModel(serviceModels) && (
                                                    ...
                                                    // puts it everywhere else. Inset to match the form
                                                    // directly below — px-1, since the old 70px margin went
                                                    // with the label-column layout that justified it.
                                                    <div className="px-1">
                                                        <FieldRow
                                                ...
                                                {/* Placement Training — phases, one course per phase, batches
                                                    on the course. Every value still lives here and arrives
                                                    through props; the form adds no state of its own beyond
                                                    which phase is unfolded. The course picker is fed the
                                                    names already stored under this service's own category,
                                                    resolved here so the form does no matching of its own. */}
                                                {isPlacementCourses && (
                                                    <PlacementStructureForm
                                                        courseOptions={serviceCourseCategory?.courseNames || []}
                                                        courseCategoryName={serviceCourseCategory?.categoryName || placementTrainingName(serviceModels)}
                                                        // "no category of that name" and "a category with
                                                        // nothing in it" are different problems with
                                                        // different fixes, and an empty list alone cannot
                                                        // tell them apart.
                                                        courseCategoryFound={Boolean(serviceCourseCategory)}
                                                        coursesLoading={isLoadingCategories}
                                                        courseName={configCourseName}
                                                        onCourseName={setConfigCourseName}
                                                        academicBatch={configAcademicBatch}
                                                        onAcademicBatch={setConfigAcademicBatch}
                                                        semester={configSemester}
                                                        onSemester={setConfigSemester}
                        ...
                        {/* Footer — Cancel sits alone on the far left (the "abandon"
                            ...
                            while stepping). The right-hand group runs from least to most
                            committing: Back and Next just move between steps and write
                            nothing; Save & Next is the one that persists, so it is the
                            only filled button and it sits last. */}
                        <div className="px-5 py-3 border-t border-hairline flex items-center justify-between gap-2 flex-shrink-0 bg-surface">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {/* Next SKIPS AHEAD — it writes nothing. It used to be the
                                    solid brand button while the one that actually saves sat
                                    beside it in an outline, so the footer offered two
                                    primaries and the louder one was the throwaway. It now
                                    reads as navigation, like Back, and the titles say in
                                    words which one commits. */}
                                {!isLastStep && (
                                    <button
                                        type="button"
                                        onClick={() => goToStep(stepIndex + 1)}
                                        disabled={isSaving}
                                        title="Go to the next step without saving"
                                        className="flex items-center gap-1.5 h-9 text-sm font-medium text-ink-700 hover:text-heading border border-hairline-strong rounded-control px-4 hover:bg-row-hover transition-colors disabled:opacity-50"
                                    >
                                ...
                                {/* Save & Next persists what's filled so far AND moves on —
                                    ...
                                    THE step's primary action, on every step: solid, and last
                                    in the row where the eye finishes. */}
                                <button
                                    type="button"
                                    data-tour="sm-save"
                                    onClick={async () => {
                                    ...
                                    disabled={isSaving}
                                    title={isLastStep
                                        ? 'Save the mapping, then preview exactly what was written'
                                        : 'Save what is filled in so far, then go to the next step'}
                                    className="flex items-center gap-1.5 h-9 text-sm font-semibold px-5 rounded-control bg-brand-700 text-white hover:bg-brand-800 shadow-sm active:scale-[0.98] disabled:opacity-50 transition-colors"
                                >
                                    {isSaving ? (
        ...
        {open && confirmClose && (
            <div
                ...
                onClick={() => { if (!isSaving) blinkConfirm() }}
            >
                <div ref={confirmPanelRef} className="w-full max-w-sm rounded-xl bg-surface border border-hairline p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
                                ...
                                {m.serviceModels?.length ? (
                                    m.serviceModels.map((model, mi) => (
                                        <span key={mi} className="text-2xs font-medium bg-brand-wash text-brand-700 border border-brand-200 rounded-full px-2 py-0.5">
                                            {formatServiceModel(model, m.service)}
                                        </span>
                                    ))
                                ) : (
...
function ClientMappingsModal({
    ...
    // Overlay clicks blink the panel instead of closing it; the Escape handler
    // below and the header X remain the ways out.
    ...
    // Filter labels only: a model this client runs under B2B alone reads in
    // capitals, as it does in the table. One also used under another business
    // model keeps its stored spelling — capitals would misstate it there.
    const b2bOnlyModels = useMemo(() => {
        const b2b = new Set<string>()
        const other = new Set<string>()
        row?.mappings.forEach((m) => (m.serviceModels || []).forEach((sm) => {
            if (sm) (isB2BService(m.service) ? b2b : other).add(String(sm))
        }))
        return new Set([...b2b].filter((sm) => !other.has(sm)))
    }, [row])
                            ...
                            <div className="mb-3 flex flex-shrink-0 flex-nowrap items-center gap-2">
                                ...
                                <Select value={modelFilter} onValueChange={setModelFilter}>
                                    ...
                                    >
                                        ...
                                        {allModels.map((m) => <SelectItem key={m} value={m}>{b2bOnlyModels.has(m) ? m.trim().toUpperCase() : m}</SelectItem>)}
                                    </SelectContent>
                                </Select>
                                                                    ...
                                                                    <span className="flex flex-wrap gap-1.5">
                                                                        {m.serviceModels.map((sm, si) => (
                                                                            <span key={si} className={`text-sm ${modelFilter === sm ? 'font-semibold text-brand-700 dark:text-brand-400' : 'text-body'}`}>
                                                                                {formatServiceModel(sm, m.service)}
                                                                            </span>
                                                                        ))}
                                                                    </span>
                                                                ) : (
...
function DeleteConfirmModal({
    ...
    // Overlay clicks blink instead of cancelling — a destructive confirm should
    // not be dismissible by a stray click. Escape and Cancel still cancel.
    const { ref: panelRef, blink } = useModalBlink<HTMLDivElement>()
...
function DeleteMappingsModal({
    ...
    // Overlay clicks blink instead of cancelling; Escape and Cancel still close.
    const { ref: panelRef, blink } = useModalBlink<HTMLDivElement>()
        ...
        <AnimatePresence>
            {open && (
                <motion.div
                    ...
                    exit="exit"
                    className="fixed inset-0 bg-ink-900/40 flex items-center justify-center z-overlay p-4"
                    onClick={(e) => { if (e.target === e.currentTarget && !isLoading) blink() }}
                >
                    <motion.div
                        ref={panelRef}
                        variants={modalVariants}
...
function ServiceMappingView({ embedded = false }: { embedded?: boolean }) {
    // Overlay clicks blink the export client-picker instead of closing it; its
    // header X is the way out.
    const { ref: pickerPanelRef, blink: blinkPicker } = useModalBlink<HTMLDivElement>()
    // What a fresh mapping leaves behind for the "set the course up now?"
    // prompt. Filled by handleSave on a CREATE only — an edit is not the moment
    // to offer course setup, the course is already there — and read once the
    // wizard has closed, so the prompt never lands on top of the wizard's own
    // preview step.
    const [setupPrompt, setSetupPrompt] = useState<{ mappingId: string; courseId: string; label: string } | null>(null)
    const { ref: setupPromptRef, blink: blinkSetupPrompt } = useModalBlink<HTMLDivElement>()
    ...
    // The year filter's Range checkbox: off asks for one year, on for a span.
    const [yearRange, setYearRange] = useState(false)
    const [serviceModelsFilter, setServiceModelsFilter] = useState<string[]>([])
    ...
    // Every client ticked and no client filter at all select the same rows, but
    // the first would put every id in the query string — past both a sane URL
    // length and the 1000-id guard the API rejects on. The dropdown still shows
    // them all ticked; only the request collapses. The count rides in a ref
    // because the option list comes from `facets`, which is itself a product of
    // this very request — reading it directly here would tie the knot. Nothing
    // can be ticked before a render that has already filled the ref in.
    const clientOptionCount = useRef(0)
    ...
    const pageFilters = useMemo(() => ({
        ...
        yearFrom: yearFilter,
        yearTo: yearToFilter,
        serviceModels: serviceModelsFilter,
        clients: collapseAll(clientFilter, clientOptionCount.current),
        businessModels: businessModelFilter,
    ...
    }), [debouncedSearch, statusFilter, yearFilter, yearToFilter, serviceModelsFilter, clientFilter, businessModelFilter, sortKey, sortDir])
    ...
    const filterSig = JSON.stringify({ ...pageFilters, pageSize, viewMode })
    ...
    const clientsPage = useClientGroupsPage(
        pageFilters, currentPage, pageSize, viewMode === 'table' && pageSizeReady,
    )
    ...
    const activePageQuery = viewMode === 'table' ? clientsPage : mappingsPage
    const isLoadingMappings = activePageQuery.isLoading || activePageQuery.isPlaceholderData || (viewMode === 'table' && !pageSizeReady)
    const isPageBusy = isLoadingMappings || activePageQuery.isFetching || search.trim() !== debouncedSearch
    const displayPage = activePageQuery.data?.page ?? currentPage
    const displayLimit = activePageQuery.data?.limit ?? pageSize
    ...
    // What the picked clients ACTUALLY have. Null means "do not narrow", which
    // covers the two cases where narrowing would be wrong: no client picked (an
    // empty filter means no filter everywhere else here) and a backend that
    // predates the byClient facet. In both, the institution-wide lists stand.
    const clientScope = useMemo(() => scopeByClients(facets, clientFilter), [facets, clientFilter])
    const availableYears = clientScope ? clientScope.years : (facets?.years ?? [])
    const availableBusinessModels = clientScope
        ? BUSINESS_MODELS.filter((model) => clientScope.businessModels.includes(model))
        : BUSINESS_MODELS
    const reportCatalogue = useMemo(() => BUSINESS_MODELS.map((businessModel, index) => ({
        businessModel,
        models: [...new Set([
            ...(servicesList[index]?.serviceModals || []).map((model) => model.name).filter((name) => name && !isAllowedService(name)),
            ...(facets?.modelsByBusiness?.find((group) => group.businessModel === businessModel)?.models || []),
        ])].sort((a, b) => a.localeCompare(b)),
    })), [servicesList, facets?.modelsByBusiness])
    const availableServiceModels = [...new Set(reportCatalogue
        .filter((group) => !businessModelFilter.length || businessModelFilter.includes(group.businessModel))
        .flatMap((group) => group.models)
        .filter((model) => !clientScope || clientScope.serviceModels.includes(model)))].sort()
    // Filter labels only: a model offered under B2B alone reads in capitals, as
    // it does in the table. One shared with another business model keeps its
    // stored spelling — capitals would misstate it for the others.
    const b2bCatalogueModels = new Set((reportCatalogue.find((group) => group.businessModel === 'B2B')?.models || [])
        .filter((model) => !reportCatalogue.some((group) => group.businessModel !== 'B2B' && group.models.includes(model))))
    const availableClients: [string, string][] = useMemo(
        () => (facets?.clients ?? []) as [string, string][],
        [facets]
    )
    clientOptionCount.current = availableClients.length

    // A selection the new scope no longer offers would stay active while
    // vanishing from its own dropdown — an invisible predicate that empties the
    // table with nothing left on screen to un-tick. pruneToScope hands back the
    // same array when there is nothing to drop, so the state setters bail out
    // and this settles in a single pass rather than looping.
    useEffect(() => {
        if (!clientScope) return
        setBusinessModelFilter((current) => pruneToScope(current, availableBusinessModels))
        setServiceModelsFilter((current) => pruneToScope(current, availableServiceModels))
        setYearFilter((current) => (current && !clientScope.years.includes(current) ? '' : current))
        setYearToFilter((current) => (current && !clientScope.years.includes(current) ? '' : current))
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [clientScope])
    ...
    const hasActiveFilters = Boolean(
        search.trim() || statusFilter || yearFilter || yearToFilter || serviceModelsFilter.length || clientFilter.length || businessModelFilter.length
    )
    // Drawer filters only (search has its own box) — badges the Filter button.
    const activeFilterCount =
        (clientFilter.length ? 1 : 0) + (businessModelFilter.length ? 1 : 0) + (yearFilter || yearToFilter ? 1 : 0) + (serviceModelsFilter.length ? 1 : 0)
    ...
    const activeTotalPages = viewMode === 'table'
        ...
        : totalPages
    useEffect(() => {
        if (activePageQuery.data && !activePageQuery.isPlaceholderData && !activePageQuery.isFetching) {
            setCurrentPage((p) => Math.min(p, activeTotalPages))
        }
    }, [activeTotalPages, activePageQuery.data, activePageQuery.isPlaceholderData, activePageQuery.isFetching])
    ...
    // scroll (page or table). Constants below match what DataTable actually
    // draws: a 33px header (h-8 cells + 1px border) and 41px body rows (h-10
    // cells + 1px border), plus the TableFooter's ~44px footprint. They used
    // to assume 40px / 48px and hold back half a row on top, which left room
    // for two more rows standing empty. The small SAFETY absorbs sub-pixel
    // rounding; a row that wraps taller makes the table scroll rather than
    // hiding it (DataTable's Y axis scrolls). Recomputes on every element
    ...
    const handleSave = async (
        ...
        try {
            ...
            // Whether this save belongs to a mapping the user is creating right
            // now — either this call made it, or an earlier partial save in the
            // same wizard run did. Only then is "set the course up?" a sensible
            // thing to ask; an edit's course already exists.
            const createdHere = !mappingId || setupPrompt?.mappingId === mappingId
            if (mappingId) {
            ...
            openedFromClientMgmt.current = false
            // Offer Course Setup once the wizard is out of the way. Armed with
            // the course id when there is one, so "Do now" opens that course's
            // actions; without one it lands on the mapping's hierarchy.
            if (createdHere && savedMappingId) {
                setSetupPrompt({
                    mappingId: savedMappingId,
                    courseId: savedCourseId,
                    label: [payload.service, (payload.serviceModels || []).join(', ')].filter(Boolean).join(' · '),
                })
            }
...
<body>${clientPages}</body>
        ...
        <EmptyState
            ...
            primaryAction={
                ...
                (hasActiveFilters || canMap) ? (
                    ...
                    >
                        {hasActiveFilters ? 'Clear filters' : '+ Add Service'}
                    </button>
                ) : undefined
            }
        />
    ...
    const tableMaxH = `calc(100dvh - ${showReportFilters ? 24 : 18}rem)`

    // ── Render ──
    // Primary CTA lives at the top-right of the workspace tab strip so
    // Service Mapping matches Client Management's "+ Add Client" pattern.
    // Permission-gated on `canMap` — hidden when the signed-in user
    // doesn't hold the Map Service grant.
    const addServiceButton = canMap ? (
        <button
            type="button"
            onClick={() => openCreate()}
    ...
    return (
        <>
            {!embedded && <WorkspaceActionSlot>{addServiceButton}</WorkspaceActionSlot>}
            ...
            <motion.div variants={pageEnter} initial="hidden" animate="visible" className="flex h-full flex-col min-h-0 min-w-0">
                <div className="flex flex-1 min-h-0 flex-col px-4 sm:px-6 md:px-8 pt-3 pb-3">
                    <ServiceMappingOverview counts={facets?.counts} isError={activePageQuery.isError} />

                    {/* Filter row — search + Show/Hide filters only. The
                        primary "+ Add Service" CTA moved out of this row
                        to the workspace tab strip's top-right slot (see
                        `addServiceButton` below), matching how Client
                        Management surfaces "+ Add Client". */}
                    <div className="no-print mt-3 flex shrink-0 items-center gap-2 flex-wrap min-w-0">
                        <div className="relative min-w-0 basis-full sm:min-w-[220px] sm:basis-auto sm:flex-1">
                        ...
                        <div className="ml-auto flex items-center gap-1.5">
                            <Button type="button" variant="outline" size="sm" aria-expanded={showReportFilters} aria-controls="service-mapping-filters" onClick={() => setShowReportFilters((value) => !value)} className="text-xs">
                    ...
                    {showReportFilters && <div id="service-mapping-filters" className="no-print mt-3 shrink-0 rounded-xl border border-hairline bg-canvas/40 p-3">
                        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
                            {/* Order: business model → service model → year → client. */}
                            <div className="min-w-0"><p className="mb-1.5 text-xs font-medium text-subtle">Business models</p><MappingMultiFilter label="Business models" options={availableBusinessModels.map((value) => ({ value, label: businessModelDisplayName(value) }))} value={businessModelFilter} onChange={(value) => { setBusinessModelFilter(value); setServiceModelsFilter([]) }} emptyLabel={clientScope ? 'No business models for the selected clients' : undefined} /></div>
                            <div className="min-w-0"><p className="mb-1.5 text-xs font-medium text-subtle">Service models</p><MappingMultiFilter label="Service models" options={availableServiceModels.map((value) => ({ value, label: b2bCatalogueModels.has(value) ? value.trim().toUpperCase() : value }))} value={serviceModelsFilter} onChange={setServiceModelsFilter} emptyLabel={clientScope ? 'No service models for the selected clients' : undefined} /></div>
                            <MappingYearFilter label="Service providing year" from={yearFilter} to={yearToFilter} years={availableYears} range={yearRange} onRangeChange={setYearRange} onChange={(from, to) => { setYearFilter(from); setYearToFilter(to) }} />
                            <div className="min-w-0"><p className="mb-1.5 text-xs font-medium text-subtle">Clients</p><MappingMultiFilter label="Clients" options={availableClients.map(([value, label]) => ({ value, label }))} value={clientFilter} onChange={setClientFilter} /></div>
                        </div>
                    ...
                    {/* ── Content: table (one row per CLIENT) or hierarchy cards ──
                        ...
                        Manage Services modal (opened via the row's button). The
                    ...
                    {viewMode === 'table' ? (
                        ...
                        <div ref={tableCardRef} aria-busy={isPageBusy} className="mt-2 flex flex-1 min-h-0 flex-col">
                            <DataTable<ClientRow>
                                ...
                                emptyAction={(hasActiveFilters || canMap) ? (hasActiveFilters ? 'Clear filters' : '+ Add Service') : undefined}
                                ...
                                columns={[
                                    {
                                        ...
                                        skeletonWidth: '20px',
                                        render: (_row, i) => (displayPage - 1) * displayLimit + i + 1,
                                    },
                                    {
                                        // The human-readable Client ID ("CLT-000023")
                                        // from Client Management — the number people
                                        // quote, so it leads the row. Legacy clients
                                        // saved before ids existed show a dash.
                                        key: 'clientId',
                                        label: 'Client ID',
                                        sortKey: 'clientId',
                                        className: 'w-[13%] px-3 text-left align-middle',
                                        skeletonWidth: '70%',
                                        render: (row) => row.client.clientId
                                            ? <span className="block truncate font-mono text-xs tabular-nums text-body" title={row.client.clientId}>{row.client.clientId}</span>
                                    ...
                                    },
                                    {
                                        ...
                                        label: 'Client Name',
                                        sortKey: 'client',
                                        className: 'w-[33%] px-3 text-left align-middle',
                                        skeletonWidth: '60%',
                                    ...
                                    },
                                    {
                                        ...
                                        render: (row) => {
                                            ...
                                            return (
                                                <span className="block break-words text-body" title={businessModelDisplayName(bm)}>{businessModelDisplayName(bm)}</span>
                                            )
                                        },
                                    },
                                    {
                                        ...
                                        skeletonWidth: '64px',
                                        // "2 (2 Active)" as a blue link-like chip; a
                                        // click opens the client's services in a white
                                        // card, like Course Setup's Available Courses.
                                        // Counts stay live via aggServiceCount /
                                        // aggActiveCount (server aggregate when present,
                                        // else counted off the row's own mappings).
                                        render: (row, i) => (
                                            <div className="flex min-w-0 items-center" onClick={(e) => e.stopPropagation()}>
                                                <ServicesCountPopover
                                                    clientId={row._id}
                                                    total={aggServiceCount(row)}
                                                    active={aggActiveCount(row)}
                                                    // Lower-half rows open upward so the card
                                                    // stays inside the table's scroll box.
                                                    openUp={i >= 3 && i >= Math.ceil(currentClientAggregated.length / 2)}
                                                />
                                            </div>
                                        ),
                                    },
                                    {
                                        key: 'actions',
                                        label: 'Actions',
                                        className: 'w-[8%] no-print px-3 text-center align-middle',
                                        skeletonWidth: '28px',
                                        // One ⋮ menu instead of two buttons — the cell
                                        // stays narrow and the row stays quiet. Each
                                        // item keeps its own permission gate.
                                        render: (row, i) => {
                                            if (!canMap && !canView) {
                                            ...
                                            return (
                                                <div className="flex justify-center" onClick={(e) => e.stopPropagation()}>
                                                    <RowActionsMenu
                                                        label={`Actions for ${row.client.clientCompany}`}
                                                        // The last rows open upward so the table's
                                                        // scroll box cannot clip the menu.
                                                        openUp={i >= 2 && i >= currentClientAggregated.length - 2}
                                                        items={[
                                                            ...(canView ? [{ key: 'manage', label: 'Manage Services', icon: <ListTree className="h-3.5 w-3.5 text-subtle" />, onSelect: () => setDetailsClientId(row._id) }] : []),
                                                            ...(canMap ? [{ key: 'add', label: 'Add Services', icon: <Plus className="h-3.5 w-3.5 text-subtle" />, onSelect: () => openCreate(row._id) }] : []),
                                                        ]}
                                                    />
                                                </div>
                                            )
                            ...
                            <div className="min-h-11">
                                <TableFooter
                                    from={currentClientAggregated.length === 0 ? 0 : (displayPage - 1) * displayLimit + 1}
                                    to={currentClientAggregated.length === 0 ? 0 : (displayPage - 1) * displayLimit + currentClientAggregated.length}
                                    total={clientTotal}
                                    pageSize={pageSize}
                        ...
                        <div className="mt-4">
                            ...
                            />
                            {(isPageBusy || totalRows > 0) && (
                                <div className="mt-3 bg-surface rounded-xl border border-hairline shadow-xs overflow-hidden">
                                    <TableFooter
                                        from={currentRows.length === 0 ? 0 : (displayPage - 1) * displayLimit + 1}
                                        to={currentRows.length === 0 ? 0 : (displayPage - 1) * displayLimit + currentRows.length}
                                        total={totalRows}
                                        pageSize={pageSize}
            ...
            {/* After a NEW mapping is saved: the success note, and the offer to
                go and set the course up. "Later" just dismisses — the course is
                already created and reachable from Course Management whenever
                they want it. */}
            <AnimatePresence>
                {setupPrompt && !wizard.open && (
                    <motion.div
                        initial={{ opacity: 0 }}
                        animate={{ opacity: 1 }}
                        exit={{ opacity: 0 }}
                        ...
                        onClick={(e) => { if (e.target === e.currentTarget) blinkSetupPrompt() }}
                    >
                        <motion.div
                            ref={setupPromptRef}
                            role="dialog"
                            aria-modal="true"
                            aria-labelledby="service-added-title"
                            initial={{ opacity: 0, scale: 0.97, y: 12 }}
                            animate={{ opacity: 1, scale: 1, y: 0 }}
                            exit={{ opacity: 0, scale: 0.97, y: 12 }}
                            transition={{ duration: 0.18, ease: 'easeOut' }}
                            className="flex w-full max-w-md flex-col rounded-xl border border-hairline bg-surface shadow-2xl"
                        >
                            <div className="flex items-start gap-3 px-5 pt-5">
                                <span aria-hidden className="inline-flex size-9 shrink-0 items-center justify-center rounded-full bg-success-50 text-success-700">
                                    <CheckCircle2 className="size-5" />
                                </span>
                                <div className="min-w-0">
                                    <h2 id="service-added-title" className="text-sm font-semibold tracking-[-0.01em] text-heading">
                                        Service added successfully
                                    </h2>
                                    {setupPrompt.label && (
                                        <p className="mt-0.5 truncate text-xs font-medium text-brand-strong" title={setupPrompt.label}>
                                            {setupPrompt.label}
                                        </p>
                                    )}
                                    <p className="mt-2 text-xs leading-5 text-subtle">
                                        In Course Management you can start setting up your course — structure, resources and learners.
                            ...
                            <div className="mt-5 flex items-center justify-end gap-2 border-t border-hairline px-5 py-3">
                                ...
                                <Button
                                    ...
                                    className="text-xs"
                                    onClick={() => {
                                        // openMappingId opens the mapping's hierarchy;
                                        // openCourseId asks it to open that course's
                                        // Course Actions straight away.
                                        const query = new URLSearchParams({ openMappingId: setupPrompt.mappingId })
                                        if (setupPrompt.courseId) query.set('openCourseId', setupPrompt.courseId)
                                        setSetupPrompt(null)
                                        router.push(`/lms/pages/coursestructure?${query.toString()}`)
                                    }}
                                >
                                    Do now
            ...
            <AnimatePresence>
                {pickerMode && (
                    <motion.div
                        ...
                        transition={{ duration: 0.14 }}
                        className="fixed inset-0 z-50 flex items-center justify-center bg-ink-900/40 backdrop-blur-[2px] p-4"
                        onClick={(e) => { if (e.target === e.currentTarget) blinkPicker() }}
                    >
                        <motion.div
                            ref={pickerPanelRef}
                            initial={{ opacity: 0, scale: 0.97, y: 12 }}
...
export default function ServiceMappingPage() {
    ...
    // boundary above it for Next's static/streaming render. Content-only
    // skeleton: the workspace tab strip lives above this route and must not
    // be covered by a fallback.
    return (
