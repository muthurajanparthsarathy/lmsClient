...
import { ClientAvatar, HierarchyChain, modelTone, fmtDate, formatServiceModel, type MappingRowVM } from "./workspaceShared";
          ...
          >
            ...
            <div className="mt-3 pt-3 border-t border-hairline flex items-center justify-between gap-3">
              <div className="flex items-center gap-1.5 flex-wrap min-w-0">
                {vm.models.slice(0, 2).map((m) => <span key={m} className={`inline-flex items-center h-[22px] px-2 rounded-chip text-2xs font-medium whitespace-nowrap ${modelTone(m)}`}>{formatServiceModel(m, vm.service)}</span>)}
