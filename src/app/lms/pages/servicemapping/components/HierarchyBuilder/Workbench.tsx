            ...
            <div className="lg:pl-5 flex flex-col min-h-0 overflow-hidden">
                ...
                {!active ? (
                    ...
                    <>
                        ...
                        {/* The stray vertical scrollbar on this row came from two
                            things together. `overflow-x-auto` alone makes the OTHER
                            axis compute to auto as well (CSS resolves a `visible`
                            paired with a scrolling axis to `auto`), and each tab's
                            `-mb-px` — the pixel that laps its bottom border over the
                            row's — put its margin box one pixel past the content
                            box. One pixel of overflow on an axis that was quietly
                            scrollable is a full scrollbar.
                            `pb-px` gives that pixel somewhere to live, so the tabs
                            still lap the row border and nothing overflows;
                            `overflow-y-hidden` makes the axis explicit so a future
                            nudge cannot bring the bar back. Horizontal scrolling
                            stays, for a degree with more semesters than fit. */}
                        <div
                            data-tour="sm-semtabs"
                            className="flex items-end gap-1 border-b border-hairline-strong overflow-x-auto overflow-y-hidden pb-px flex-shrink-0"
                        >
