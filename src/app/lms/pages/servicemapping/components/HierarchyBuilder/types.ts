...
// The tree's rows have no category of their own, so the per-category name list
// is not passed down: a row picks from `allCourseNames`, one flat catalogue.
// Course Management still owns the categories; nothing UNDER a semester asks
// which one a course came from.
export type CourseApi = {
    /** Every course name across every category — deduped and sorted. */
    ...
    errorAt: (path: string, index: number, field: 'courseName') => string | undefined
