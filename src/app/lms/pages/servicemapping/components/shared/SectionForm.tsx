...
import { FoldChevron, structureCardCls } from './StructureLayout'
...
export function Section({
    ...
    return (
        // The same outlined container the phase cards use: it closes around the
        // heading alone when folded and around the heading plus its fields when
        // open, so a section is a THING on the page rather than a run of rows
        // separated by a hairline.
        <section className={cn(structureCardCls, 'px-3')}>
            <div className="flex items-center gap-3">
                <button
                    type="button"
                    onClick={onToggle}
                    aria-expanded={open}
                    className="flex items-center gap-2.5 py-3 text-left group"
                >
            ...
            <AnimatePresence initial={false}>
                {open && (
                    ...
                    >
                        ...
                        {/* pl-1, not pl-6. The old indent was there to show
                            nesting when sections were flat rows; the card's own
                            border does that now, and 24px of dead margin inside a
                            box only pushed every field right and squeezed the
                            controls. */}
                        <div data-tour={tourAnchor} className="min-w-0 pb-4 pl-1">
                            {children}
                        </div>
                    </motion.div>
                )}
            ...
            <AnimatePresence initial={false}>
                {open && (
                    ...
                    >
                        ...
                        {/* One nesting device, not two: a left rule OR an
                            outline. The parent section is the outline, so the
                            rule stays here to mark the level below it — just
                            closer in, since the card already supplies the outer
                            margin. */}
                        <div className="min-w-0 ml-[9px] border-l border-hairline pl-3">
                            {children}
                        </div>
                    </motion.div>
                )}
...
export function FieldRow({
    ...
    return (
        <div
            ...
            // 150px + 16px, down from 200px + 24px. The label column has to
            // be fixed so every label in a section starts on the same x, but at
            // 200 a short one like "Degree" left a hand's width of nothing
            // between the word and its field.
            className="py-2 md:grid md:grid-cols-[150px_minmax(0,1fr)] md:gap-x-4 md:items-start"
        >
...
/**
 * The outer wrapper for a set of `Section`s — the spacing between the cards, and
 * nothing else.
 *
...
export function SectionShell({ children }: { children: React.ReactNode }) {
    return (
        // No Expand/Collapse-all control any more. Every section opens by default
        // and each one folds from its own heading, so the shell's only job is the
        // spacing between them.
        //
        // Space between the cards rather than a rule through them: each section
        // carries its own outline, and a divider on top of that would be a second
        // boundary in the same place.
        <div className="font-sans space-y-2.5">{children}</div>
    )
}
