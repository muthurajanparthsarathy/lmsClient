...
import { ClientAvatar, modelTone, fmtDate, formatServiceModel, type MappingRowVM } from "./workspaceShared";
            ...
            <div className="rounded-tile border border-hairline bg-canvas p-3 space-y-2.5">
              ...
              <div className="flex items-center justify-between gap-3">
                ...
                <div className="flex items-center gap-1.5 flex-wrap justify-end">
                  {vm.models.length ? vm.models.map((m) => <span key={m} className={`inline-flex items-center h-[22px] px-2 rounded-chip text-2xs font-medium ${modelTone(m)}`}>{formatServiceModel(m, vm.service)}</span>) : <span className="text-sm text-subtle">—</span>}
                </div>
              </div>
              <div className="flex items-center justify-between gap-3">
                ...
                {siblings.map((s) => (
                  ...
                  >
                    <div className="min-w-0">
                      ...
                      <p className="text-2xs text-subtle truncate">{s.models.map((m) => formatServiceModel(m, s.service)).join(", ") || "—"}{s.year ? ` · ${s.year}` : ""}</p>
                    </div>
