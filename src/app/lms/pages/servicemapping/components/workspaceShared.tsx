...
// B2B's service models are acronyms — TD, HTD, CSR, COE — so under a
// Business-to-Business service they read in full capitals, however they were
// typed into the master data. Display only: filtering and matching keep the
// stored value, and every other model is returned untouched. Same service test
// as the wizard's isBusinessToBusiness.
export const isB2BService = (service?: string) =>
  /business\s*to\s*business|b2b/i.test(service || "");
export const formatServiceModel = (model: string, service?: string) =>
  isB2BService(service) ? model.trim().toUpperCase() : model;
...
export function ClientAvatar({
  ...
  logoUrl?: string;
  /** Optional CSS object-position saved when the client's logo was
   *  cropped in the form ("50% 50%" default). Falls back to center. */
  logoPosition?: string;
}) {
    ...
    >
      {showImage ? (
        <img
          ...
          style={{ objectPosition: logoPosition || "50% 50%" }}
