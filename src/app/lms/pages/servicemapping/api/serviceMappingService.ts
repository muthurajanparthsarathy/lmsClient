...
export interface MappedClientRef {
  _id: string;
  /** Human-readable Client ID ("CLT-000023"); absent on legacy clients. */
  clientId?: string;
...
export interface ServiceMapping {
  ...
  // Placement Training's structure: one configuration per course, carrying its
  // phases and training batches. Derived on read for mappings saved before this
  // existed, so it is present either way.
  courseConfigurations?: CourseConfiguration[];
  courseId?: string;
  courseCode?: string;
...
// Mirrors the server's embedded subdocuments. Ids are the server's on the way
// OUT; on the way IN they may be any string the form used, because the server
// re-mints phase ids and re-points batches through the ids it issues.
//
// A TrainingBatch ("Batch I") is a group sitting the course. It is NOT the
// academic batch ("2026") on the configuration — that is an intake year.
...
export interface TrainingBatch {
  _id?: string;
  name: string;
  /** null / absent = belongs to the configuration itself, not to a phase. */
  phaseId?: string | null;
  order?: number;
}
...
export interface MappingFacets {
  ...
  businessModels: string[];
  modelsByBusiness?: { businessModel: string; models: string[] }[];
  /** [id, name] pairs, already sorted by name. */
  clients: [string, string][];
  /** What each client ACTUALLY has, so the filter dropdowns can narrow to a
   *  client selection instead of offering the whole institution. Optional:
   *  a backend predating this facet leaves it undefined and the dropdowns fall
   *  back to the institution-wide lists. */
  byClient?: {
    client: string;
    businessModel: string;
    serviceModels: string[];
...
export interface MappingSetupStats {
  ...
  configured: number;
  pending: number;
  /** Distinct clients holding at least one mapped course. */
  clients?: number;
  /** A course has no active flag of its own — it inherits its mapping's, so
   *  these split totalCourses by whether the mapping is still active. */
...
export const buildMappingPageParams = (
  ...
  (["status", "setupStatus", "year", "yearFrom", "yearTo", "client", "service", "serviceModel", "course",
  ...
  if (f.serviceModels?.length) q.serviceModels = JSON.stringify(f.serviceModels);
  if (f.clients?.length) q.clients = JSON.stringify(f.clients);
  if (f.businessModels?.length) q.businessModels = JSON.stringify(f.businessModels);
  if (f.sortKey) {
    q.sortKey = f.sortKey;
...
export const serviceMappingApi = {
  ...
  // Bounded, so a delete the server never answers surfaces as an error toast
  // instead of a confirm dialog stuck on "Deleting…". Generous, because the
  // cascade removes the mapping's courses and everything under them.
  remove: (id: string) =>
    api.del<MessageResponse>(`/service-mapping/delete/${id}`, { timeout: 90000 }),
  ...
  return useQuery({
    ...
    queryFn: async ({ signal }) => {
      const res = await serviceMappingApi.getPage(
        buildMappingPageParams(filters, page, limit, opts), signal
      );
...
export function useClientGroupsPage(
  ...
  return useQuery({
    ...
    queryFn: async ({ signal }) => {
      const res = await serviceMappingApi.getPage(
        buildMappingPageParams(filters, page, limit, { groupBy: "client" }), signal
      );
