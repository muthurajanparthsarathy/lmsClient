...
import { attendanceApi, type AttendanceRecord, type AttendanceSummary } from "@/app/lms/pages/attendancemanagement/api/attendanceApi";
...
/**
 * Attendance cells for a SET of students — the Report table's filtered rows.
 *
 * The Report grid renders one cell per (student x working day), so it asks for
 * exactly those students' rows instead of the whole course's. Disabled until
 * there is at least one id, so an empty selection fires no request.
 */
 ...
 * them in Mongo is what lets the grid above scope itself to a set of students.
 *
