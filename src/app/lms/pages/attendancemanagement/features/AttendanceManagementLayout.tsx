...
export default function AttendanceManagementLayout({ children }: { children: React.ReactNode }) {
    ...
    // Back-arrow context depends on how the user got here — the report page
    // returns to the course's marking sheet; otherwise LDC users go back to
    // their console and admin/trainer users to the course picker.
    const isReportPage = pathname.endsWith("/attendancemanagement/report");
    const isMarkingPage = pathname.endsWith("/attendancemanagement");
    const courseQuery = `courseId=${encodeURIComponent(courseId)}${fromLdc ? "&from=ldc" : ""}`;
    const openReports = () => router.push(`/lms/pages/attendancemanagement/report?${courseQuery}`);
    const backToPicker = () =>
        router.push(
            isReportPage
                ? `/lms/pages/attendancemanagement?${courseQuery}`
                : fromLdc ? `/lms/pages/lddashboard#attendance` : `/lms/pages/attendancemanagement`
        );
                    ...
                    <div className="bg-surface border-b border-hairline w-full px-6 flex-shrink-0">
                        <div className="flex items-center justify-between gap-4 h-12">
                            ...
                            <div className="flex items-center gap-1.5">
                                    ...
                                    {canReports && isMarkingPage && (
                                        <button
                                            onClick={openReports}
                                            className="inline-flex items-center gap-1.5 h-7 px-2.5 rounded-control border border-info-500/30 bg-surface hover:bg-info-50 text-info-700 text-xs font-semibold transition-colors"
                                            title="Open the attendance report for this course"
                                        >
                                            <BarChart3 className="h-3.5 w-3.5" /> Reports
                                        </button>
                                    )}
                                    {canReports && !isReportPage && (
                                        <>
                                            <button
