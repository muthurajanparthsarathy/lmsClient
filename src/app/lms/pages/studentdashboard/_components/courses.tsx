                    ...
                    <thead>
                        <tr className="text-left">
                            {['Course', 'Track', 'Progress', 'Last Opened', ''].map((h, i) => (
                                <th
                                    key={h || i}
                                    className="border-b border-[#EEF0F4] px-1 pb-2 text-[10.5px] font-semibold uppercase tracking-[0.05em] text-slate-400 dark:border-gray-800"
                                >
                                    {h}
                                </th>
                            ))}
                            ...
                            >
                                <td className="px-1 py-3">
                                    <div className="flex items-center gap-2.5">
                                        ...
                                        <div className="min-w-0">
                                            <p className="max-w-[170px] truncate text-[12.5px] font-semibold text-slate-800 group-hover:text-indigo-600 dark:text-slate-100">
                                                {c.name}
                                            </p>
                                            <p className="truncate text-[10.5px] text-slate-400">{c.code || 'No course code'}</p>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-1 py-3">
                                ...
                                <td className="px-1 py-3">
                                    <div className="w-[92px]">
                                        <p className="mb-1 text-[11.5px] font-bold" style={{ color: progressTint(c.progress) }}>{c.progress}%</p>
                                        <Bar value={c.progress} color={progressTint(c.progress)} height={5} />
                                    </div>
                                </td>
                                <td className="px-1 py-3 text-[11.5px] text-slate-400">
                                    {c.lastAccessed ? timeAgo(c.lastAccessed) : 'Not opened'}
                                </td>
                                <td className="px-1 py-3 text-right">
                                    <span className="inline-flex items-center gap-1 rounded-lg bg-indigo-50 px-2.5 py-1.5 text-[11.5px] font-semibold text-indigo-600 transition-colors group-hover:bg-indigo-600 group-hover:text-white dark:bg-indigo-500/10">
                ...
                <>
                        ...
                        <Donut
                            ...
                            center={
                                <>
                                    ...
                                    <span className="mt-1 text-[10.5px] font-medium text-slate-400">Assigned</span>
                                </>
                            }
                        />
                    </div>
                    ...
                    <div className="mt-3 space-y-1.5">
                        {slices.map((sl) => (
                            <div key={sl.label} className="flex items-center gap-2">
                                <Dot color={sl.color} />
                                <span className="flex-1 truncate text-[12px] text-slate-600 dark:text-slate-300">{sl.label}</span>
                                <span className="text-[12px] font-semibold text-slate-800 dark:text-slate-100">{sl.value}</span>
                    ...
                    <div className="mt-4 grid grid-cols-2 gap-2.5">
                        <div className="rounded-[14px] bg-[#F8FAFC] p-3 text-center dark:bg-gray-800/60">
                            ...
                            <p className="mt-1.5 text-[10.5px] font-medium text-slate-400">Average score</p>
                        </div>
                        <div className="rounded-[14px] bg-[#F8FAFC] p-3 text-center dark:bg-gray-800/60">
                            <p className="text-[17px] font-bold leading-none tracking-[-0.02em]" style={{ color: C.success }}>
                                {s.onTimePct == null ? '—' : `${s.onTimePct}%`}
                            </p>
                            <p className="mt-1.5 text-[10.5px] font-medium text-slate-400">On-time submission</p>
                        </div>
                    </div>
                </>
            )}
                ...
                {deadlines.slice(0, 5).map((d, i) => {
                    ...
                    return (
                        <div key={`${d.id}-${i}`} className="relative flex items-start gap-3 py-2.5">
                            ...
                            >
                                <span className="text-[14px] font-bold leading-none" style={{ color: st.color }}>{d.date.getDate()}</span>
                            ...
                            <div className="min-w-0 flex-1 pt-0.5">
                                <div className="flex items-start justify-between gap-2">
                                    <p className="truncate text-[12.5px] font-semibold text-slate-800 dark:text-slate-100">{d.title}</p>
                                ...
                                <p className="mt-1 text-[11px] font-medium" style={{ color: st.color }}>
