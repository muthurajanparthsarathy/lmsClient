...
import { RerunButton } from '@/app/lms/pages/courses/reviewSubmission/components/rerun/RerunButton';
...
const isAutoEvaluated = isAutoEvalExercise(selectedExercise);
  ...
  // ── Breadcrumb ────────────────────────────────────────────────────────────
  // The full trail is Course ▸ Module ▸ SubModule ▸ Topic ▸ SubTopic ▸
  // Exercise ▸ Grading Console, and on real courses the middle titles are long
  // enough that it wrapped onto a second line and doubled the header height
  // ("Problem Solving using DSA- Phase II ▸ Fast & Slow Pointers, Linked List
  // Reversals (full, partial, k-group)" is ~110 characters on its own). So the
  // trail no longer wraps: the middle collapses to a "…" chip whose hidden
  // levels appear on hover, and the crumbs that stay visible truncate with an
  // ellipsis and carry their full title as a native tooltip.
  const BREADCRUMB_HEAD = 1; // always keep the course
  const BREADCRUMB_TAIL = 2; // always keep the exercise + the console itself
  const renderBreadcrumb = () => {
    const total = breadcrumb.length;
    // Only collapse when it actually buys something — hiding a single crumb
    // behind a "…" of the same width is strictly worse than showing it.
    const collapse = total > BREADCRUMB_HEAD + BREADCRUMB_TAIL + 1;
    const hiddenItems = collapse ? breadcrumb.slice(BREADCRUMB_HEAD, total - BREADCRUMB_TAIL) : [];
    type Slot =
      | { kind: 'crumb'; item: BreadcrumbItem; last: boolean }
      | { kind: 'ellipsis' };
    const slots: Slot[] = collapse
      ? [
          ...breadcrumb.slice(0, BREADCRUMB_HEAD).map((item): Slot => ({ kind: 'crumb', item, last: false })),
          { kind: 'ellipsis' },
          ...breadcrumb.slice(total - BREADCRUMB_TAIL).map((item, i): Slot => ({
            kind: 'crumb',
            item,
            last: i === BREADCRUMB_TAIL - 1,
          })),
        ]
      : breadcrumb.map((item, i): Slot => ({ kind: 'crumb', item, last: i === total - 1 }));

    return (
      <nav aria-label="Breadcrumb" className="flex items-center min-w-0 select-none">
            ...
            <li key={index} className="flex items-center min-w-0">
              ...
              {slot.kind === 'ellipsis' ? (
                <div className="relative group shrink-0">
                  <button
                    type="button"
                    aria-label={`Show ${hiddenItems.length} hidden breadcrumb level${hiddenItems.length > 1 ? 's' : ''}`}
                    title={hiddenItems.map((h) => h.title).join('  ›  ')}
                    className="flex items-center px-1.5 py-0.5 rounded text-slate-400 hover:bg-slate-100 hover:text-slate-700 transition-colors duration-150"
                  >
                    <MoreHorizontal className="h-3.5 w-3.5" />
                  </button>
                  {/* Hover reveal — the collapsed levels, one per line so a
                      long topic title stays readable instead of truncating
                      again inside the popover. */}
                  <div className="absolute left-0 top-full pt-1.5 z-[60] hidden group-hover:block">
                    <ul className={`min-w-[200px] max-w-[420px] rounded-lg border border-slate-200 bg-white shadow-lg py-1 ${inter.className}`}>
                      {hiddenItems.map((h, i) => (
                        <li key={i} className="flex items-start gap-1.5 px-2.5 py-1 text-[11px] font-medium text-slate-600">
                          <span className="shrink-0 mt-[2px] text-slate-400 [&>svg]:h-3 [&>svg]:w-3">{h.icon}</span>
                ...
                <div
                  className={`flex items-center gap-1 px-1.5 py-0.5 rounded min-w-0 transition-colors duration-150 ${slot.last ? 'text-indigo-600' : 'text-slate-400 hover:text-slate-700 cursor-pointer'}`}
                  title={slot.item.title}
                >
                  <span className="shrink-0 [&>svg]:h-3 [&>svg]:w-3">{slot.item.icon}</span>
                  <span className={`text-[11px] font-medium truncate max-w-[200px] ${inter.className} ${slot.last ? 'text-indigo-600' : 'text-slate-500'}`}>
                    {slot.item.title}
                  </span>
                </div>
              )}
    ...
    <div className="p-3 flex-1 flex flex-col min-h-0 overflow-y-auto overflow-x-hidden [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-slate-100 [&::-webkit-scrollbar-thumb]:bg-slate-300 [&::-webkit-scrollbar-thumb]:rounded-full" style={{ scrollbarWidth: 'thin' }}>
      ...
      <div className="flex flex-col min-h-0 flex-1">
          ...
          <div className={`mb-2 flex items-center justify-between gap-1.5 ${inter.className}`}>
            ...
            <div className="flex items-center gap-1.5">
              ...
              <Input
                ...
                inputMode="decimal"
                autoComplete="off"
                aria-label={`Question score out of ${maxScore}`}
                value={scoreDraft ?? String(score)}
                readOnly={!isScoreEditable}
                onChange={(e) => {
                  if (!isScoreEditable) return;
                  const raw = e.target.value;
                  // Only accept text that can still grow into a score: digits
                  // with at most one decimal point. Anything else (letters, a
                  // sign, a second dot) is ignored rather than parsed to NaN,
                  // so a stray keystroke can't blank the field.
                  if (raw !== '' && !/^\d*\.?\d*$/.test(raw)) return;
                  const parsed = Number.parseFloat(raw);
                  const value = Number.isFinite(parsed) ? parsed : 0;
                  const clamped = clampScore(value);
                  setScore(clamped);
                  // Keep the typed text while it is within range; once it goes
                  // over the max, show the clamp immediately instead of letting
                  // the field disagree with the score being saved.
                  setScoreDraft(value === clamped ? raw : String(clamped));
                }}
                // Leaving the field resolves whatever is in it — "", "8." or a
                // leading zero — to the committed number.
                onBlur={() => setScoreDraft(null)}
        ...
        {/* ── Evaluation Breakdown card ───────────────────────────────────
            ...
            Hidden on MANUAL exercises. A submission can still carry an
            `evaluationBreakdown` on a manual exercise — the code-editor runs
            test cases on Run/Submit regardless, and an exercise switched from
            testcase/ai to manual keeps the breakdown written by the earlier
            method. Either way the trainer is the grader here, so surfacing a
            "Test Case Auto-Score" badge next to the score input only implies
            a machine score that does not apply. Exercises with no stored
            method (legacy) are left alone: if a breakdown exists there, it
            really was produced by the auto-evaluation pipeline. */}
        {!isQuestionMCQ(selectedQuestion) && submissionQuestion && (submissionQuestion as any).evaluationBreakdown && !isManualEvalExercise(selectedExercise) && (() => {
