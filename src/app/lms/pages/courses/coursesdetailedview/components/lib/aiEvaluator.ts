...
export type BreakdownTestCase = {
  ...
  source: 'question' | 'ai';
  /**
   * Trainer flagged the SOURCE question test case `isHidden`. AI-generated
   * cases are never hidden (the trainer never authored them). The student's
   * Test Result panel conceals a hidden case's input/expected exactly the way
   * Test Case scoring does — AI mode must not become a way to read them out.
   */
  hidden: boolean;
  input: string;
...
const cleanTestCase = (tc: any) => ({
  ...
  // Carried through to the breakdown so the student editors can conceal it.
  // Never sent to Gemini — the prompt only lists input/expected.
...
export async function evaluateWithAi(input: EvaluateWithAiInput): Promise<AiEvaluationResult> {
  ...
  // Hard ceiling at the question's own marks. The 50/50 split is already
  // bounded by construction, but a question worth 5 must never come back as
  // 5.01 through float drift or a future weighting change.
  const totalScore = Math.min(maxMarks, Math.max(0, round2(criteriaPortion + testCasePortion)));
