...
import { runInteractivePython, isInteractiveTerminalSupported, type InteractiveHandle } from "@/app/lms/pages/courses/reviewSubmission/components/rerun/pyodideRunner"
import {
...
export default function MultiFileCodeEditor({
  ...
  // The step-through trace is built by instrumenting the source with
  // sys.settrace (lib/pythonTracer.ts), so it only exists for Python. On a
  // Java/C/JS assignment the button used to render and then dead-end into a
  // "Python only" toast — gate the button on the language instead so it is
  // simply absent unless the student is working in Python. selectedLanguage
  // is always clamped to availableLanguages, so this also covers exercises
  // whose trainer never enabled Python at all.
  const isPythonWorkspace = selectedLanguage === "python"

  // How the trainer configured this exercise to be evaluated. Manual means
  // nothing is auto-run and nothing is auto-scored, so there is no Run
  // Testcase button and no Test Result panel — Submit simply files the code
  // for the trainer to grade on Review. Test Case and AI both keep the full
  // Run Testcase + Test Result surface.
  const evalMethod = useMemo(
    () => resolveEvaluationMethod(exercise, category).method,
    [exercise, category],
  )
  // Manual is a plain workspace: a Run button and a Terminal. The student
  // pastes a test case into stdin, runs, and reads the raw output — nothing
  // is compared, counted or scored. Test Case and AI instead get Run Testcase
  // and the Test Result panel.
  ...
  useEffect(() => {
    ...
    // Fresh terminal state. stdin is NOT reset here — the seed effect below
    // owns it, because that one also runs on the first render (this teardown
    // deliberately skips it) and the first question needs its seed too.
    setTermLines([])
    setRunning(false)
    setLastRuntime(null)
    ...
    setBottomTab(isManualEval ? 'terminal' : 'test-result')
  ...
  // Seed the Terminal's stdin with this question's first VISIBLE example
  // input, on mount and on every question switch. Under Manual grading the
  // Terminal is the whole workspace, so opening with an empty stdin box would
  // make the first Run useless. Hidden cases are never seeded — that's the
  // trainer's held-back check, not a starting point. Freely editable after.
  useEffect(() => {
    setStdin(firstSampleInput(currentQuestion))
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentQuestion?._id])
  ...
  const postSubmission = async (
    ...
    // know it's outdated).
    //
    // Skipped entirely for a Manual exercise: nothing is evaluated, so there
    // is no result panel to route to (it isn't rendered at all).
    if (showsTestResult) {
  ...
  // panel. It is a dry run: no answer is recorded. Under AI the grading is
  // client-side, so nothing is posted at all. Under Test Case the submit
  // endpoint is the only judge available, so it is still called — but the
  // question is never marked "Submitted ✓" and the draft save still runs so
  // the student's work isn't lost. Students can iterate freely.
        ...
        return (
          <div style={{
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, justifySelf: "end" }}>
              ...
              {/* Visualize — Python only. The trace harness is Python-specific,
                  so on any other language the button is hidden rather than
                  shown-and-rejected. */}
              {isPythonWorkspace && (
                <button
                  onClick={visualize}
              ...
              {/* Run — Manual only. It executes the project against whatever
                  sits in the Terminal's stdin box and prints the raw output;
                  it never validates anything. A graded exercise gets Run
                  Testcase instead (below), which is a different job. */}
              {exercise && isManualEval && !(running || interactiveActive) && (
                <button
                  onClick={runCode}
                  disabled={!ready}
                  ...
                  style={{
                    ...
                    border: "none",
                    background: !ready ? "#94A3B8" : "#12A765",
              ...
              {/* Run Testcase — evaluates against the trainer's testcases and
                  ...
                  Hidden on a Manual exercise: there is nothing to run and no
                  panel to paint — the student writes code and submits it. */}
              {exercise && showsTestResult && (
                <button
