    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          paddingInline: 'clamp(8px, 0.9vw, 14px)',
            ...
            <table className="w-full border-collapse text-sm table-fixed">
              ...
              <tbody>
                {pagedExercises.map((exercise, idx) => {
                  ...
                  // Startable rows keep the plain white surface. The pale-
                  // orange fill + persistent orange rail used to paint every
                  // row that had a Start button, so "highlighted" signalled
                  // nothing — the Status chip and the orange Start button
                  // already carry that. Only hover tints now.
                  const railShadow = isHovered ? 'inset 3px 0 0 #F97316' : 'none'
                  return (
                    <tr key={exercise._id}
                      style={{
                      ...
                      {/* Name — the primary label in the row, so it takes the
                          medium weight + heading colour + slight size bump the
                          We_Do Assignment Name uses. Description subtitle
                          dropped so the row height stays uniform. */}
                      <td className="px-3 h-11 align-middle text-[13.5px] min-w-0">
                        <span className="block truncate font-medium text-heading" title={exercise.exerciseInformation.exerciseName}>
                          {exercise.exerciseInformation.exerciseName}
                        </span>
                      </td>
                      ...
                      <td className="pl-2 pr-4 h-11 align-middle">
                        <div className="flex items-center justify-end gap-1.5">
                          {availability.canStart && !limitReached && (
                            <button
                              type="button"
                              onClick={e => handleStartClick(exercise, e)}
                              // Same footprint (h-9 / 128px / 13px semibold) and
                              // the same brand orange as the We_Do assignment
                              // list's Start button. The four per-type tints
                              // (amber / violet / peach / teal) each read as a
