...
// Evaluation Method wiring — resolves the exercise's stored evaluationMethod
// (with the per-tab legacy fallback), and calls Gemini for AI grading. Both
// helpers are shared with the You_Do editor and multi-file editor so the
// three surfaces evaluate identically for the same setting.
...
// Test Result panel shared with the multi-file workspace and the You_Do
// editor — same component, so all three surfaces read identically.
import { TestResultView, type SubmitStatus, type TestResultCase, type TestResultState } from './multi-file/BottomPanel';
import { buildAiTestResultState } from '@/app/lms/pages/courses/coursesdetailedview/components/lib/aiTestResult';
import { firstSampleInput } from '@/app/lms/pages/courses/coursesdetailedview/components/lib/sampleInput';
...
export default function CodeEditor({
    ...
    // --- Security State ---
    // We Do component — security is not used here; only the You Do component runs proctoring.
    const [isAssessmentMode, setIsAssessmentMode] = useState(false);
    const [showSecurityAgreement, setShowSecurityAgreement] = useState(false);
    const [hasAgreedToSecurity, setHasAgreedToSecurity] = useState(true);
    ...
    // ── Bottom panel: Console (Run output) + Test Result (evaluation) ──
    // The Console tab is always there — running code is not evaluating it.
    // The Test Result tab only exists when the exercise is actually evaluated.
    const [bottomTab, setBottomTab] = useState<'console' | 'results'>('console');
    const [testResult, setTestResult] = useState<TestResultState | null>(null);
    const [selectedCaseIndex, setSelectedCaseIndex] = useState<number>(0);
    // Run Testcase spins independently of Submit so the two buttons can show
    // their own loading state.
    const [isRunningTestCases, setIsRunningTestCases] = useState(false);
    // Terminal stdin. Under Manual grading the Console IS the deliverable
    // surface: the student pastes a test case here, presses Run, and reads
    // the raw output. Seeded from the question's own example so the first
    // Run does something useful; freely editable after that.
    ...
    // Last score this student actually earned per question, keyed by question
    // id. The whole-exercise Submit reuses it instead of posting 0 — a plain
    // `status: 'submitted'` write OVERWRITES the stored score server-side, so
    ...
    useEffect(() => {
        ...
        fetch(`${API_ORIGIN}/exercise/${exercise._id}`, {
    ...
    // We Do component does not run proctoring. Force empty settings so every
    // `if (securitySettings.X)` branch below is inert. Security lives in YouDo.
...
function solve() {
    ...
    // --- Toast System ---
    // How the trainer configured this exercise to be evaluated. Manual means
    // nothing is auto-run and nothing is auto-scored: no Run Testcase button
    // and no Test Result tab — Submit just files the code for the trainer to
    // grade on Review. Test Case and AI both keep the full surface.
    const evalMethod = useMemo(
        () => resolveEvaluationMethod(exercise, category).method,
        [exercise, category],
    );
    // Manual is a plain workspace: Run + Console, no evaluation of any kind.
    // Test Case and AI get Run Testcase + the Test Result tab instead.
    ...
    const fetchUserProgress = useCallback(async (questionId?: string) => {
        ...
        try {
            ...
            const response = await fetch(`${API_ORIGIN}/courses/answers/single?${params.toString()}`, {
    ...
    const saveRecording = async () => {
        try {
            ...
            try {
                ...
                const saveResponse = await fetch(`${API_ORIGIN}/assessment/recording`, {
                    method: 'POST',
                    headers: {
    ...
    const handleTermination = useCallback(async (reason: string, status: 'completed' | 'terminated' = 'terminated') => {
        ...
        try {
            ...
            await fetch(`${API_ORIGIN}/exercise/lock`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`
                },
    ...
    // The Test Result belongs to the question that produced it — leaving it
    // up while the student reads a different question reads as that question's
    // result. Same reset the multi-file and You_Do editors do.
    useEffect(() => {
        setTestResult(null);
        setSelectedCaseIndex(0);
        setBottomTab('console');
        setCustomInput(firstSampleInput(exercise?.questions?.[currentProblemIndex]));
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [currentProblemIndex, exercise]);
    ...
    const runCode = async () => {
        ...
        try {
            // Whatever is in the terminal's input box is what gets piped to
            // the program. It starts as the question's example input, so this
    ...
    // ─── Evaluation-Method-aware grading helpers ─────────────────────────────
    // These mirror the You_Do implementation so a We_Do assignment set to
    // Test Case auto-scores identically to a You_Do assessment. All three
    // helpers are silent (no terminal spam) except verifyTestCasesAndScore
    // which streams per-case status lines just like the You_Do version does.

    const normalizeOutput = (s: string): string =>
        (s || '').replace(/\r\n/g, '\n').split('\n').map(l => l.replace(/\s+$/, '')).join('\n').trim();

    // Silent Piston call — same shape as executeCode but with no terminal logs,
    // used to grade one test case at a time. Compile errors surface as `error`.
    const runCodeForGrading = async (input: string): Promise<{ output: string; error?: string }> => {
        try {
            const pistonLang = getPistonLanguage(selectedLanguage);
            const currentCode = editorInstanceRef.current ? editorInstanceRef.current.getValue() : code;
            ...
            return { output: '', error: e?.message || 'Execution error' };
        }
    };

    // Max marks for the current question — mirrors the mark-badge logic in the
    // You_Do editor so Test Case scoring produces the same total for the same
    // question shape (Score-Type: evenMarks / levelBasedMarks / question specific).
    const getCurrentQuestionMaxMarks = (): number => {
        const currentQ: any = (exercise as any)?.questions?.[currentProblemIndex] ?? currentQuestion;
        const ss: any = (exercise as any)?.questionConfiguration?.programmingQuestionConfiguration?.scoreSettings;
    ...
    // Test-case auto-scoring — runs every test case on the current question and
    ...
    const submitProgressToBackend = async (
        ...
        evaluationBreakdown: any = null, // per-question Test Case / AI breakdown; null for Manual + nav-saves
        ...
        try {
            ...
            // Evaluation breakdown — stringified because FormData is text-only.
            // The controller parses + sanitises server-side (see answer.js).
    ...
    // ── Evaluate + submit ────────────────────────────────────────────────
    // ONE path behind both the Run Testcase and the Submit Question buttons,
    // so a question is scored the same way whichever the student presses.
    // `record: false` (Run Testcase) is a dry run: it skips the "Submitted ✓"
    // bookkeeping, the auto-advance, AND — for AI — the POST itself, so
    // nothing is stored until the student actually presses Submit. The
    // evaluation and the Test Result panel are identical either way.
    // Test Case is the one method a dry run still has to POST for: the submit
    // endpoint is also the judge (it re-runs the trainer's hidden cases
    // server-side), so no verdict exists without calling it.
    //
    // Per evaluation method:
    //   • testcase — the server judge re-runs the trainer's authoritative
    //     test cases (including hidden ones) and stamps score + breakdown.
    //   • ai       — Gemini grades client-side against the selected criteria;
    //     the resulting score (never more than the question's marks) is
    //     posted and stored.
    //   • manual   — nothing runs, score stays 0, no breakdown, no panel.
    const runEvaluation = async (opts: { record: boolean; isTestSubmission?: boolean }) => {
        const { record, isTestSubmission = false } = opts;
        const maxMarks = getCurrentQuestionMaxMarks();

        // Route to the Test Result tab and mark the previous result stale.
        // Skipped for Manual — that tab isn't rendered at all.
        if (showsTestResult) {
        ...
        try {
            ...
            if (method === 'testcase') {
                // Server-side judge. The submit endpoint re-runs the code
                // against the trainer's authoritative testCases and stamps
                // score + breakdown itself; the response carries the per-case
                // rows the Test Result panel renders.
                addTerminalLog('system', '🧪 Running test cases on server judge…');
            } else if (method === 'ai') {
                addTerminalLog('system', '🤖 Running AI evaluation…');
                const liveQ: any = liveQuestion ?? currentQuestion;
                const currentCode = editorInstanceRef.current ? editorInstanceRef.current.getValue() : code;
                // Per-question mode: pass the specific question so the
                // resolver picks its `aiTestCasesCount` (falling back to the
                // exercise's count for legacy questions without the field).
                const { getAiTestCasesCountFor } = resolveEvaluationMethod(exercise, category);
                const aiTestCasesCount = getAiTestCasesCountFor(liveQ);
                // Cached generated cases from an earlier student's run — Gemini
                // REUSES these rather than regenerating so everyone sees the
                // same set. `aiGeneratedTestCases` lives on the question doc.
                const cachedGen = Array.isArray(liveQ?.aiGeneratedTestCases) ? liveQ.aiGeneratedTestCases : [];
                const aiResult = await evaluateWithAi({
                    code: currentCode,
                    language: selectedLanguage,
                    question: {
                        title: liveQ?.title || liveQ?.questionTitle || '',
                        description: typeof liveQ?.description === 'string' ? liveQ.description : liveQ?.description?.text || '',
                        testCases: liveQ?.testCases || [],
                    },
                    criteria: aiCriteria,
                    maxMarks,
                ...
                submitStatus = 'submitted'; // AI never auto-marks 'solved' — trainer decides on Review
                submitBreakdown = aiResult.breakdown;
                // First run on this question generated the cases fresh —
                // persist them so every later student is judged against the
                // same set. Fire-and-forget: a failure just re-triggers
                // generation on the next attempt.
                if (aiResult.newlyGeneratedTestCases && aiResult.newlyGeneratedTestCases.length > 0 && liveQ?._id) {
                    const token = getToken() || localStorage.getItem('token') || '';
                    fetch(`${API_ORIGIN}/courses/answers/persist-ai-test-cases`, {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
                        body: JSON.stringify({
                            exerciseId: exercise?._id,
                            questionId: liveQ._id,
                            courseId,
                            nodeId,
                            nodeType,
                            category: category || 'We_Do',
                    ...
                    }).catch(() => { /* silent — question is still gradable next time */ });
                }
                if (aiResult.failed) {
                    showToast({
                        type: 'warning',
                        title: 'AI grader unavailable',
                        message: aiResult.errorMessage || (record
                            ? 'AI grader failed. Your code was saved — trainer will grade manually.'
                            : 'AI grader failed. Nothing was submitted — try again, or press Submit to file it for the trainer.'),
                        duration: 6000,
                    });
                    addTerminalLog('warning', `⚠️ ${aiResult.errorMessage || 'AI grader failed.'}`);
                } else {
                    const b = aiResult.breakdown.ai;
                    addTerminalLog('success', `🏁 AI score: ${submitScore}/${maxMarks} — ${b.passedTestCases}/${b.totalTestCases} test cases passed, ${b.criteria.length} criteria evaluated`);
                }
            }
            // method === 'manual' → submitScore stays 0, no breakdown, no auto-eval.

            // Run Testcase (`record: false`) is a DRY RUN — it must not store
            // anything. AI grading already happened client-side above, so the
            // POST would be a pure write; skip it and paint the panel from the
            // local result. Test Case is different: the submit endpoint IS the
            // judge (it re-runs the trainer's hidden cases server-side), so
            // there is no way to get a verdict without calling it.
            const isDryRun = !record && method === 'ai';
            const submitResponse = isDryRun
                ? null
            ...
            // The server's score wins when it judged (Test Case); otherwise the
            // client's own (AI, or 0 for Manual) is what was stored. A dry run
            // stored nothing, so it leaves the submitted-score bookkeeping
            // alone — otherwise a later Submit Test would file a score the
            // student never submitted.
            if (liveQuestionId && !isDryRun) {
                submittedScoresRef.current[liveQuestionId] =
                    typeof srvScore === 'number' ? srvScore : submitScore;
            }

            // ── Paint the Test Result panel ──────────────────────────────
            if (showsTestResult) {
                let nextResult: TestResultState;
                if (method === 'ai' || (srvBreakdown?.method === 'ai' && srvBreakdown.ai)) {
                    // Prefer the local evaluator result (richest); fall back to
                    // the server's sanitised echo.
                    nextResult = buildAiTestResultState({
                        ai: aiEval?.breakdown?.ai ?? srvBreakdown?.ai,
                        score: typeof srvScore === 'number' ? srvScore : submitScore,
                        maxMarks,
                        failed: aiEval?.failed,
                        errorMessage: aiEval?.errorMessage,
                    });
                } else if (srvBreakdown?.method === 'testcase' && srvBreakdown.testcase) {
                    ...
                    const cases: TestResultCase[] = rawCases.map((c) => ({
                        ...
                        hidden: !!c.hidden,
                        passed: !!c.passed,
                        // Server marks hidden cases progressively as they
                        // unlock; anything without the flag from an older
                        // response falls back to !hidden.
                    ...
                    nextResult = {
                        ...
                        score: typeof srvScore === 'number' ? srvScore : null,
                        maxMarks,
                    };
                } else {
                    // Test Case exercise whose question carries no test cases —
                    // nothing was judged, so say so rather than showing a
                    // green "Accepted" the student didn't earn.
                    nextResult = {
                        status: 'accepted',
                        cases: [],
                        score: typeof srvScore === 'number' ? srvScore : null,
                        maxMarks,
                        message: 'Submission recorded — no test cases are configured for this question.',
                    };
                }
            ...
            // Terminal mirror of the server's per-case verdicts, kept so the
            // Console tab still tells the whole story.
            ...
            if (!record) return submitResponse;
            ...
            if (showsTestResult) {
                setTestResult({
                    ...
                    cases: [],
                    message: record ? 'Submission failed' : 'Run failed',
                    errorDetail: error?.message || 'Your answer was not evaluated.',
                });
            }
            return null;
        }
    ...
    const submitCode = async () => {
        ...
        clearTerminal();
        try {
            // Per-QUESTION submit only. Finishing the whole exercise stays with
    ...
    // Run Testcase — evaluates the current code and paints Test Result without
    // recording the question as "Submitted ✓". Students can iterate freely.
    const runTestCases = async () => {
        if (isRunningTestCases || isRunning) return;
        setIsRunningTestCases(true);
        clearTerminal();
        try {
            await runEvaluation({ record: false });
        } finally {
    ...
    useEffect(() => {
        const checkExerciseStatus = async () => {
            ...
            try {
                const token = getToken() || '';
                const response = await fetch(`${API_ORIGIN}/exercise/status?courseId=${courseId}&exerciseId=${exercise._id}&category=You_Do&subcategory=${subcategory}`, {
                    headers: { 'Authorization': `Bearer ${token}` }
                });
        ...
        >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap', flex: 1, minWidth: 0 }}>
                            ...
                            {category && (
                                <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                    <ChevronRight style={{ width: 12, height: 12, color: theme === 'dark' ? '#4b5563' : '#c0c4cc', flexShrink: 0 }} />
                                    <button onClick={() => setPendingNavLevel('category')} style={{ fontFamily: FONT, fontSize: 12.5, fontWeight: 500, color: theme === 'dark' ? '#FDBA74' : '#F97316', background: 'none', border: 'none', cursor: 'pointer', padding: 0, whiteSpace: 'nowrap' }}>
                        ...
                        {/* {exercise && (
                            <button
                                ...
                                style={{
                                    ...
                                    borderRadius: 6,
                                    border: `1px solid ${showExerciseInfo ? '#FB923C' : (theme === 'dark' ? '#374151' : '#d1d5db')}`,
                                    background: showExerciseInfo ? (theme === 'dark' ? '#7C2D12' : '#FFEDD5') : (theme === 'dark' ? '#1f2937' : '#fff'),
                                    color: showExerciseInfo ? (theme === 'dark' ? '#FED7AA' : '#F97316') : (theme === 'dark' ? '#9ca3af' : '#6b7280'),
                                    cursor: 'pointer',
                                    fontFamily: FONT,
                                    fontSize: 12,
                                    fontWeight: 500,
                                ...
                                onClick={async () => {
                                    ...
                                    try {
                                        if (currentQuestion) {
                                            ...
                                            const prior = submittedScoresRef.current[qId];
                                            if (prior != null) {
                                                // Already evaluated this question — reuse that score
                                                // and just flip the exercise to submitted. Re-running
                                                // would double-charge Gemini / re-run the judge for
                                                // no new signal.
                                                await submitProgressToBackend(qId, 'submitted', prior, true);
                                            } else {
                                                // Never evaluated — run the exercise's configured
                                                // evaluation now so the last question isn't recorded
                                                // as a blind 0.
                                                await runEvaluation({ record: false, isTestSubmission: true });
                                            }
                                        }
            ...
            <div className={`flex items-center justify-between p-2.5 border-b ${theme === 'dark' ? 'border-gray-700' : 'border-gray-300'}`}>
                ...
                <div className="flex items-center gap-2" style={{ justifyContent: 'flex-end' }}>
                    ...
                    {/* Language Selector — hidden for link questions (no editor) */}
                    {!(exercise?.questions?.[currentProblemIndex] as any)?.isLinkQuestion && (
                    <select
                    ...
                    {/* CENTER — Submit + Run */}
                    {/* CENTER — Run only (hidden for link questions — no code to run) */}
                    {!(exercise?.questions?.[currentProblemIndex] as any)?.isLinkQuestion && (
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                {/* ── Link questions: the external page IS the question — one
                    iframe replaces the question panel + compiler entirely.
                    (Read from exercise.questions, never from `problem`:
                    convertExerciseToProblems strips unknown fields.) ────── */}
                {(() => {
                    ...
                    <div className="flex flex-col flex-1 min-h-0" style={{ height: `${100 - rightPanelSplit}%` }}>
                        ...
                        <div className={`flex items-center justify-between gap-0 border-b flex-shrink-0 ${theme === 'dark' ? 'border-gray-700 bg-gray-800' : 'border-gray-300 bg-gray-50'}`}>
                            {/* Left side - Tabs. Console (Run output) is always
                                ...
                                exercise auto-scores nothing, so there is no
                                result to show. */}
                            <div className="flex items-center gap-0">
                                <button
                                    ...
                                    className={`flex items-center gap-1.5 px-3 py-1.5 border-b-2 ${bottomTab === 'console'
                                        ? `border-orange-500 ${theme === 'dark' ? 'text-orange-400' : 'text-orange-600'}`
                                ...
                                {showsTestResult && (
                                    <button
                                        ...
                                        className={`flex items-center gap-1.5 px-3 py-1.5 border-b-2 ${bottomTab === 'results'
                                            ? `border-orange-500 ${theme === 'dark' ? 'text-orange-400' : 'text-orange-600'}`
                                            : `border-transparent ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}`}
                                    >
                                        <CheckCircle2 className="w-3.5 h-3.5" />
                                        <span className="text-xs font-medium">Test Result</span>
                                        {isRunningTestCases && <Loader2 className="w-3 h-3 animate-spin" />}
                                    </button>
                                )}
                            </div>
                            ...
                            <div className="flex items-center gap-2 mr-2">
                                {/* Run Testcase — evaluates the current code and
                                    ...
                                    workspace and the You_Do editor carry, in the
                                    same order of commitment. Hidden on a Manual
                                    exercise: nothing is evaluated there. */}
                                {showsTestResult && (
                                    <button
                                ...
                                {/* Submit Question — runs the exercise's configured
                                    evaluation (server test-case judge / AI /
                                    nothing for Manual), stores the score and
                                    marks the question submitted. This used to
                                    POST score:0 directly and skip evaluation
                        ...
                        {/* Terminal input — Manual only. Nothing here is ever
                            compared to an expected output; it is simply the
                            stdin the next Run pipes into the program. */}
                        {isManualEval && (
                            <div
                                className={`flex-shrink-0 px-3 py-2 border-b ${theme === 'dark' ? 'border-gray-700 bg-gray-900' : 'border-gray-200 bg-gray-50'}`}
                            >
                                <label
                                    htmlFor="console-stdin"
                                    className={`block mb-1 text-2xs font-semibold uppercase tracking-wide ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}
                                    style={{ fontFamily: FONT }}
                                >
                                ...
                                <textarea
                                    ...
                                    className={`w-full resize-y rounded px-2 py-1.5 font-mono text-xs outline-none border ${theme === 'dark' ? 'bg-gray-950 border-gray-700 text-slate-300' : 'bg-white border-gray-300 text-gray-800'}`}
                                    style={{ minHeight: 40, maxHeight: 160 }}
                                />
                            </div>
                        )}

                        {/* Test Result — the shared panel from the multi-file
                            workspace, so the same submission reads identically
                            on every programming surface. */}
                        {showsTestResult && bottomTab === 'results' && (
                            <div className={`flex-1 min-h-0 ${theme === 'dark' ? 'bg-gray-950' : 'bg-white'}`}>
                                <TestResultView
                                    state={testResult}
                                    selectedIndex={selectedCaseIndex}
                                    onSelect={setSelectedCaseIndex}
                                    onRetry={runTestCases}
                                    isSubmitting={isRunning || isRunningTestCases}
                                />
                            </div>
                        )}
                                ...
                                return (
                                    <div style={{ padding: '20px', background: theme === 'dark' ? '#431407' : '#FFF7ED', borderBottom: `1px solid ${theme === 'dark' ? '#7C2D12' : '#FED7AA'}` }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                                            <span style={{ fontSize: 16 }}>🎯</span>
                                            <span style={{ fontSize: 12, fontWeight: 700, color: theme === 'dark' ? '#FED7AA' : '#EA580C', letterSpacing: '0.6px', textTransform: 'uppercase' }}>Marking Scheme</span>
                                        </div>
                                        <div style={{
                                            display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                                            padding: '10px 14px', borderRadius: 8,
                                            background: theme === 'dark' ? '#7C2D12' : '#FFEDD5',
                                            border: `1px solid ${theme === 'dark' ? '#F97316' : '#FED7AA'}`,
                                        }}>
                                            <span style={{ fontSize: 12, color: theme === 'dark' ? '#FED7AA' : '#EA580C', fontWeight: 500 }}>Marks per question</span>
            ...
            <div className="fixed top-4 right-4 z-50 flex flex-col gap-2">
                {toasts.map((toast) => (
                    <div
                        key={toast.id}
                        className={`flex items-start gap-3 p-4 rounded-lg shadow-lg max-w-sm transition-all duration-300 ${toast.type === 'success' ? 'bg-green-50 border border-green-200' : toast.type === 'error' ? 'bg-red-50 border border-red-200' : toast.type === 'warning' ? 'bg-yellow-50 border border-yellow-200' : 'bg-orange-50 border border-orange-200'}`}
                    >
                        <div className={`flex-shrink-0 ${toast.type === 'success' ? 'text-green-500' : toast.type === 'error' ? 'text-red-500' : toast.type === 'warning' ? 'text-yellow-500' : 'text-orange-500'}`}>
