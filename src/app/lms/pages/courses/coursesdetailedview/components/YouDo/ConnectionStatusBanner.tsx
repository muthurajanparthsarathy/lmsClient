...
import { NetStatus } from "@/app/lms/pages/courses/coursesdetailedview/components/YouDo/useAttemptSession";

interface Props {
  netStatus: NetStatus;
  queueCount: number;
  /** Suppress the sync/pending strips and only surface the offline warning.
   *  Used by the code editor, where the strip flashed on every submit. */
  offlineOnly?: boolean;
}

export default function ConnectionStatusBanner({ netStatus, queueCount, offlineOnly = false }: Props) {
  ...
  const visible = isOffline || (!offlineOnly && (isSyncing || isConfirmingSync));
