  ...
  const onSecTabSwitch = useCallback((count: number, max: number) => {
    ...
    if (count >= max) {
      if (ASSESSMENT_LOCK_DISABLED) {
        toast.warn('Tab switch limit reached — ignored (test mode).', { toastId: 'tab-terminate' });
        return;
      }
...
const sendFinalSubmission = async () => {
  ...
  try {
    ...
    await fetch(`${API_ORIGIN}/courses/answers/submit`, {
      method: 'POST',
  ...
  useFaceProctor({
    ...
    onLimitReached: (reason) => {
      if (ASSESSMENT_LOCK_DISABLED) {
        toast.warn(`${reason} — ignored (test mode).`, { toastId: 'face-terminate' });
        return;
      }
  ...
  useEffect(()=>{
    const fetch_=async()=>{
      try{
        ...
        const token=getToken()||localStorage.getItem('token')||'';
        const res=await fetch(`${API_ORIGIN}/exercise/${finalId}`,{ method:'GET',headers:{'Authorization':`Bearer ${token}`,'Content-Type':'application/json'} });
  ...
  const prefillPreviousAnswers = async (qs: MCQQuestion[], exId: string) => {
    try {
      ...
      for (const q of qs) {
        try {
          const res = await fetch(`${API_ORIGIN}/courses/answers/previous-submission?courseId=${fCId}&exerciseId=${exId}&questionId=${q._id}&category=${fCat}`, { headers: { Authorization: `Bearer ${token}` } });
          if (!res.ok) continue;
          const data = await res.json();
