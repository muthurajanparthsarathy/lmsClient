...
// ─── Testing/dev kill-switch ──────────────────────────────────────────────────
// When NEXT_PUBLIC_DISABLE_ASSESSMENT_LOCK is 'true' (or '1'), proctoring
// violations (tab switch, face proctor, etc.) no longer TERMINATE / LOCK or
// auto-submit the assessment, and a previously-locked attempt won't show the
// "Access Terminated" screen — so an assessment can be run end-to-end while
// testing. Violations still surface a warning toast; everything else is
// unchanged. Leave this UNSET in production so proctoring behaves normally.
export const ASSESSMENT_LOCK_DISABLED =
  process.env.NEXT_PUBLIC_DISABLE_ASSESSMENT_LOCK === 'true' ||
  process.env.NEXT_PUBLIC_DISABLE_ASSESSMENT_LOCK === '1';
