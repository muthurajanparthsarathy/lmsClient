...
// Test Result panel shared with the multi-file workspace — the assessment
// editor renders the exact same component so both surfaces read identically.
...
// detail. Identical to what the multi-file and We_Do editors render.
...
const convertExerciseToProblems = (exercise: Exercise): ProblemData[] => {
    ...
    return exercise.questions.map((question, index) => ({
        ...
        // ALL non-hidden testCases show as examples — the authoring forms only
        // flag the first case isSample, so filtering on it hid every added case.
...
export default function CodeEditor({
    ...
    // Test Result panel state — same shape the multi-file editor keeps, so the
    ...
    // presses Run, and reads the raw output. Nothing is compared or scored.
    const [customInput, setCustomInput] = useState('');
    // The shared Terminal renders TermLine[]; this editor keeps LogEntry[].
    // 'warning' has no TermLine equivalent — it rides as 'info', which is the
    // same non-fatal tone.
    const termLines = useMemo<TermLine[]>(
        () => terminalLogs.map(l => ({
            id: l.id,
            kind: (l.type === 'warning' ? 'info' : l.type) as TermLine['kind'],
            text: l.content,
        })),
        [terminalLogs],
    );
    ...
    // Autosave notes on change (debounced 500ms) so the scratchpad survives a
    // reload without a Save button. Same question-scoped key as the load.
    useEffect(() => {
        const qId = currentQuestion?._id;
        if (!qId) return;
        const key = `lms_notes_${exercise?._id || 'ex'}_${qId}`;
        const t = setTimeout(() => {
    ...
    useEffect(() => {
        if (exercise?.questions && exercise.questions.length > 0) {
            ...
            // a Manual student's first Run does something useful.
            setCustomInput(firstSampleInput(question));

            // Load this question's notes. Miss = blank scratchpad. The key
            // includes both ids so notes on different questions don't collide.
            try {
                const notesKey = `lms_notes_${exercise?._id || 'ex'}_${question?._id || 'q'}`;
                const raw = typeof window !== 'undefined' ? localStorage.getItem(notesKey) : null;
                setNotesText(raw ?? "");
            } catch { setNotesText(""); }
...
function solve() {
    ...
    const handleTermination = useCallback(async (reason: string, status: 'completed' | 'terminated' = 'terminated') => {
        // Testing/dev kill-switch: don't terminate or lock — just warn and let
        // the assessment continue. (NEXT_PUBLIC_DISABLE_ASSESSMENT_LOCK)
        if (ASSESSMENT_LOCK_DISABLED) {
            showToast({
                type: 'warning',
                title: 'Proctoring disabled (test mode)',
                message: `Ignored termination: ${reason}`,
                duration: 4000,
            });
            return;
        }
...
`;
    ...
    // ── rows → TestResultState (the shape BottomPanel renders) ──
    // Hidden cases unlock ONE BY ONE and only once every visible case has
    // passed: reveal the first hidden case, keep revealing while each one
    // passes, and stop AT the first failing hidden case — which IS revealed so
    // the student can debug it. Same contract the server sends the multi-file
    // editor; computed here because You-Do judges locally via Piston.
    const buildTestResultState = (
        rows: typeof testResults,
        extra?: { score?: number | null; maxMarks?: number | null; message?: string },
    ): TestResultState => {
        let unlockNext = rows.filter(r => !r.isHidden).every(r => r.status === 'passed');
        const cases: TestResultCase[] = rows.map((r) => {
            const passed = r.status === 'passed';
            if (!r.isHidden) {
                return {
                    index: r.caseNo - 1, hidden: false, passed, unlocked: true,
                    input: r.input ?? '', expectedOutput: r.expected ?? '', actualOutput: r.actual ?? '',
                };
            }
            const unlocked = unlockNext;
            if (unlocked && !passed) unlockNext = false;
            return {
                index: r.caseNo - 1, hidden: true, passed, unlocked,
                // Trainer input/expected stay concealed until the case unlocks;
                // the student's own output is always safe to show.
                input: unlocked ? (r.input ?? '') : '',
                expectedOutput: unlocked ? (r.expected ?? '') : '',
        ...
        const status: SubmitStatus =
            totalCount > 0 && passedCount === totalCount ? 'accepted'
            : passedCount === 0 ? 'wrong-answer'
            : 'partial';
        const runtimeMs = Math.round(rows.reduce((acc, r) => acc + (parseFloat(r.time) || 0) * 1000, 0));
        return {
            status, cases, passedCount, totalCount,
            runtimeMs: runtimeMs || null,
            memoryKb: null,
            score: extra?.score ?? null,
            maxMarks: extra?.maxMarks ?? null,
            message: extra?.message,
        };
    };

    // Paint the "Evaluating…" state, keeping the previous result visible but
    // marked stale — exactly what the multi-file editor does on submit.
    const beginEvaluating = () => {
        // Manual exercises render no Test Result panel — nothing to route to.
        if (!showsTestResult) return;
    ...
    const evaluateCurrentQuestionWithAi = async (): Promise<{
        ...
        // its `aiTestCasesCount` (falling back to the exercise's count for
        // legacy questions without the field).
        const { aiCriteria, getAiTestCasesCountFor } = resolveEvaluationMethod(exercise, category);
        const maxMarks = getCurrentQuestionMaxMarks();
        const currentCode = editorInstanceRef.current ? editorInstanceRef.current.getValue() : code;
        const aiTestCasesCount = getAiTestCasesCountFor(liveQuestion);
        // Cached generated cases from an earlier student's run — Gemini REUSES
        // these rather than regenerating, so everyone is judged on the same set.
        const cachedGen = Array.isArray(liveQuestion?.aiGeneratedTestCases)
            ? liveQuestion.aiGeneratedTestCases : [];
        const aiResult = await evaluateWithAi({
            code: currentCode,
            language: selectedLanguage,
            question: {
                title: liveQuestion?.title || liveQuestion?.questionTitle || '',
                description: typeof liveQuestion?.description === 'string'
                    ? liveQuestion.description
                    : liveQuestion?.description?.text || '',
        ...
        // First run on this question generated the cases fresh — persist them
        // so every later student is judged against the same set.
        // Fire-and-forget: a failure here just re-triggers generation later.
        if (aiResult.newlyGeneratedTestCases && aiResult.newlyGeneratedTestCases.length > 0 && liveQuestion?._id) {
            const token = getToken() || localStorage.getItem('token') || '';
    ...
    //   • Test Case — runs the trainer's cases locally via Piston.
    //   • AI        — runs the AI evaluation client-side and shows the score.
    //     It used to also POST that score through submitProgressToBackend,
    //     which stored an answer the student never submitted (and spun the
    //     Submit button, because it shared `isRunning`). Both are gone.
    //   • Manual    — the button isn't rendered at all.
    const runTestCases = async () => {
        ...
        if (evalMethod === 'ai') {
            ...
            beginEvaluating();
            try {
                // evaluateCurrentQuestionWithAi paints the Test Result panel
                // itself, so the score is on screen without any round trip.
                const ai = await evaluateCurrentQuestionWithAi();
                showToast({
                    type: ai.failed ? 'warning' : 'success',
                    title: ai.failed ? 'AI grader unavailable' : 'AI evaluated',
                    message: ai.failed
                        ? 'Could not grade this run — try again, or submit for the trainer to grade.'
                        : `Score ${ai.score}/${ai.maxMarks} — not submitted yet`,
                    duration: 3500,
                });
            } catch (err: any) {
                setTestResult({
                    ...
                    errorDetail: err?.message || 'Could not evaluate the code.',
                });
            } finally {
        ...
        try {
            ...
            setTestResults(rows);
            setTestResult(buildTestResultState(rows, { maxMarks: getCurrentQuestionMaxMarks() }));
            setSelectedCaseIndex(0);
        } catch (err: any) {
            setTestResult({
                status: 'submission-failed',
                cases: [],
                message: 'Run failed',
                errorDetail: err?.message || 'Could not run the code.',
            });
        } finally {
            setIsRunningTestCases(false);
        }
    ...
    const handleSubmitCode = async () => {
        ...
        try {
            ...
            if (method === 'testcase') {
                ...
                // test cases failed, not just the count.
                submitBreakdown = {
                    method: 'testcase',
                    testcase: {
                        passed, total,
                ...
                // method === 'manual' — never run tests or AI, post code with
                // score:0. No Test Result panel is rendered for a Manual
                // exercise, so there is nothing to paint; the toast below is
            ...
            setTestResult({
                ...
                errorDetail: err?.message || 'Your answer was not evaluated.',
            });
        ...
        try {
            if (currentQuestion) {
                ...
                if (solvedQuestions.has(currentProblemIndex) && submittedScoresRef.current[qId] != null) {
                    ...
                    setShowTerminal(true);
                    const { method } = resolveEvaluationMethod(exercise, category);
                    let submitScore = 0;
                    let submitStatus: 'submitted' | 'solved' = 'submitted';
                    let submitBreakdown: any = null;
                    if (method === 'testcase') {
    ...
    const handleTimeUpAutoSubmit = async () => {
        ...
        try {
            if (currentQuestion) {
                const liveQuestion = exercise?.questions?.[currentProblemIndex];
                const qId = liveQuestion?._id || currentQuestion._id;
                // Only a Test Case exercise auto-scores on timeout. Manual must
                // never be given a mark the trainer didn't award, and an AI
                // exercise keeps whatever score its last evaluation stored
                // (re-running Gemini here would double-charge for no new
                // signal, and the student is out of time anyway).
                if (evalMethod === 'testcase') {
                    ...
                    await submitProgressToBackend(
                        ...
                        score,
                        true,
                        { method: 'testcase', testcase: { passed, total, cases } },
                    );
                    submittedScoresRef.current[qId] = score;
                } else if (evalMethod === 'ai') {
                    // Already scored this question — reuse it rather than
                    // re-charging Gemini for the same code. Never scored (the
                    // student ran out of time before pressing anything) — grade
                    // it now, so their work isn't recorded as a blind 0.
                    ...
                    if (prior != null) {
                        ...
                        try {
                            const ai = await evaluateCurrentQuestionWithAi();
                            await submitProgressToBackend(qId, 'submitted', ai.score, true, ai.breakdown);
                            submittedScoresRef.current[qId] = ai.score;
                        } catch {
                            // Grader unreachable at the buzzer — file the code
                            // for manual grading rather than losing the answer.
                            await submitProgressToBackend(qId, 'submitted', 0, true);
                            submittedScoresRef.current[qId] = 0;
                        }
                    }
                } else {
                    // Manual — the trainer grades on Review; a timeout must
                    // never stamp a mark of its own.
                    await submitProgressToBackend(qId, 'submitted', 0, true);
                    submittedScoresRef.current[qId] = 0;
                }
            }
    ...
    const submitProgressToBackend = async (
        ...
        try {
            ...
            const response = await fetch(`${(process.env.NEXT_PUBLIC_API_URL || 'https://lmsserver-yeve.onrender.com')}/courses/answers/submit`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` },
                body: formData,
            });
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    if (data.success) {
                        ...
                        // → show the Access Terminated screen. Suppressed while the
                        // testing kill-switch is on so a stale lock doesn't block entry.
                        if ((isLocked || status === 'terminated') && !ASSESSMENT_LOCK_DISABLED) {
                            setIsLocked(true);
                            setIsTerminated(true);
                            setHasStarted(false);
    ...
    return (
        ...
        >
            ...
            {/* GLOBAL HEADER — one compact toolbar, matched to the multi-file
                workspace: left = product glyph + sidebar toggle + Previous /
                N of M / Next paginator; right = the primary actions. The old
                Exercise-info / Score strip that sat above this row is gone —
            ...
            >
                <div className="flex items-center" style={{ gap: 10, minWidth: 0 }}>
                    ...
                    <button
                        ...
                        style={{
                            display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                            width: 36, height: 36, borderRadius: 8, flexShrink: 0,
                            border: `1px solid ${theme === 'dark' ? '#374151' : '#D9E1EA'}`,
                            background: theme === 'dark' ? '#1f2937' : '#fff',
                            color: theme === 'dark' ? '#d1d5db' : '#172033',
                            cursor: 'pointer',
                        }}
                    >
                    ...
                    {problems.length > 1 && (() => {
                        const prevDisabled = currentProblemIndex === 0 && !onCrossPrev;
                        const nextDisabled = (currentProblemIndex === problems.length - 1 && !onCrossNext) || (!isFreeFlow && !isQuestionSolved(currentProblemIndex));
                        const nextLabel = currentProblemIndex === problems.length - 1 && onCrossNext ? (crossNextLabel || 'Next Section') : 'Next';
                        return (
                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginLeft: 4 }}>
                                <button
                                    ...
                                    style={{
                                        ...
                                        border: `1px solid ${theme === 'dark' ? '#374151' : '#D9E1EA'}`,
                                        background: theme === 'dark' ? '#1f2937' : '#fff',
                                        color: prevDisabled ? '#C6D0DA' : (theme === 'dark' ? '#d1d5db' : '#172033'),
                                ...
                                <span
                                    aria-label={`Problem ${currentProblemIndex + 1} of ${problems.length}`}
                                    style={{
                                        display: 'inline-flex', alignItems: 'center', justifyContent: 'center',
                                        minWidth: 44, height: 32, padding: '0 8px',
                                        fontFamily: FONT, fontSize: 13, fontWeight: 700,
                                        color: theme === 'dark' ? '#f9fafb' : '#101828',
                                ...
                                <button
                                    ...
                                    style={{
                                        ...
                                        border: `1px solid ${theme === 'dark' ? '#374151' : '#D9E1EA'}`,
                                        background: theme === 'dark' ? '#1f2937' : '#fff',
                                        color: nextDisabled ? '#C6D0DA' : (theme === 'dark' ? '#d1d5db' : '#172033'),
                                        fontSize: 12, fontWeight: 600, fontFamily: FONT,
                                ...
                                {exercise?.questionBehavior?.allowSkip && (
                                    <button
                                        ...
                                        style={{
                                            ...
                                            opacity: currentProblemIndex === problems.length - 1 ? 0.5 : 1,
                                        }}
                                    >
                ...
                {/* Time Left — absolutely centred on the toolbar so it stays
                    dead-centre no matter how wide the left paginator or the
                    right action cluster grow. */}
                {exercise && exerciseTimeLeft !== null && (exData?.exerciseInformation?.totalDuration || 0) > 0 && (() => {
            ...
            <div className="flex flex-1 overflow-hidden relative">
                    ...
                    <div style={{
                        ...
                        <button
                            ...
                            style={{
                                ...
                                color: showNotesPanel ? '#0F5B5D' : (theme === 'dark' ? '#9ca3af' : '#667085'),
                        ...
                        <button
                            ...
                            style={{
                                display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
                                padding: '10px 4px', border: 'none', background: 'transparent', cursor: 'pointer',
                                color: showOverviewModal ? '#6957E5' : (theme === 'dark' ? '#9ca3af' : '#667085'),
                        ...
                        >
                            ...
                            <span style={{ fontSize: 11, fontWeight: 600, fontFamily: FONT }}>
                                {(exData as any)?.isGraded !== false ? 'Score' : 'Overview'}
                            </span>
                        </button>
                        <button
                            type="button"
                            ...
                            style={{
                                display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4,
                                padding: '10px 4px', border: 'none', background: 'transparent', cursor: 'pointer',
                                color: showDetailsModal ? '#0F9D94' : (theme === 'dark' ? '#9ca3af' : '#667085'),
                ...
                {/* Notes panel — private per-question scratchpad. Slides in beside
                    the rail and autosaves on every keystroke, so a reload never
                    loses the student's own notes. Kept intentionally simple so it
                ...
                {showNotesPanel && (
                    <div
                        ...
                        style={{
                            ...
                            borderColor: theme === 'dark' ? '#374151' : '#E4E7EC',
                        }}
                    >
                        <div style={{
                            ...
                            padding: '10px 14px',
                            borderBottom: `1px solid ${theme === 'dark' ? '#374151' : '#E4E7EC'}`,
                            background: theme === 'dark' ? '#292524' : '#FFFBF0',
                        }}>
                            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
                                <NotebookPen size={14} style={{ color: '#0F5B5D' }} />
                                <span style={{ fontFamily: FONT, fontSize: 13, fontWeight: 700, color: theme === 'dark' ? '#f9fafb' : '#101828' }}>
                                    Notes
                                </span>
                            </div>
                            <button
                        ...
                        <div style={{ padding: '6px 14px 4px', fontFamily: FONT, fontSize: 11.5, color: '#667085' }}>
                        ...
                        <textarea
                            ...
                            style={{
                                ...
                                borderRadius: 8,
                                border: `1px solid ${theme === 'dark' ? '#374151' : '#E4E7EC'}`,
                                background: theme === 'dark' ? '#0f172a' : '#fff',
                                fontFamily: FONT, fontSize: 13,
                                color: theme === 'dark' ? '#f1f5f9' : '#101828',
                                lineHeight: 1.55, outline: 'none',
                            }}
                            onFocus={(e) => { e.currentTarget.style.borderColor = '#0F5B5D'; }}
                            onBlur={(e) => { e.currentTarget.style.borderColor = theme === 'dark' ? '#374151' : '#E4E7EC'; }}
                        />
                    </div>
                )}
                ...
                {/* The vertical "resize handle" that used to sit here has been
                    removed. It never resized anything — `setLeftPanelWidth` was
                    never called — and because it was absolutely positioned at
                    `leftPanelWidth%` of a row that also holds the 72px nav rail,
                    it landed ~72px INSIDE the description panel and painted a grey
                ...
                <div className="flex flex-col flex-1 min-w-0 h-full" style={{ width: `${100 - leftPanelWidth}%` }}>
                    ...
                    {/* ── Bottom panel (resizable) ──
                        ...
                        Which surface it IS follows the exercise's evaluation
                        method: Test Case / AI get Test Result (status summary,
