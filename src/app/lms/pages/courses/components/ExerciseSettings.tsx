...
import { D, FONT, injectFonts } from '../uploadcourseresources/components/youdo/assessments/shared/tokens';
import {
    ...
    return (
      <div className="es-step" style={{ padding: '18px 22px', display: 'flex', flexDirection: 'column', gap: 13 }}>
        ...
        {formData.programmingConfig.questionConfigType === '' ? (
          ...
          <div className="es-2col" style={{ display: 'grid', gridTemplateColumns: formData.isGraded !== false ? '1fr 1fr' : '1fr', gap: 12, alignItems: 'start', marginTop: 12 }}>
            {formData.isGraded !== false && (
            <div style={SPEC_CARD}>
              <div className="es-card-b" style={SPEC_CARD_B}>
                <EvaluationMethodConfig
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center z-50 p-4" style={{ background: 'rgba(30,41,59,0.55)', backdropFilter: 'blur(6px)', fontFamily: FONT }}
      ...
      <style>{`
        ...
        /* Location trail. It scrolls sideways on a deep path, but a visible
           scrollbar under a 15px-tall strip is all bar and no content — and
           on Windows it steals height from the row. Hidden in every engine;
           the strip still scrolls by wheel, trackpad and keyboard. */
        .es-crumbs { scrollbar-width: none; -ms-overflow-style: none; }
        .es-crumbs::-webkit-scrollbar { display: none; }
      ...
      <div className="es-main flex overflow-hidden"
        ...
        <section className="es-right flex-1 flex flex-col min-w-0" style={{ background: '#FFFFFF' }}>
          ...
          <div style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              {/* Location trail — Course › Module › … › We Do › Assignment.
                  The wizard fills 96vw and hides the page behind it, so
                  without this there is nothing on screen saying WHICH node's
                  assignment is being authored. Display-only: every step here
                  belongs to one node, so a clickable crumb would have to
                  discard the form to navigate.

                  One line, no wrapping — it sits above the step title and
                  must not push it down as the path deepens. Long paths
                  scroll sideways instead, with the tail (the part that
                  identifies the activity) kept visible. */}
              {breadcrumbs.length > 0 && (
                <nav
                  ...
                  style={{
                    ...
                    marginBottom: 5, overflowX: 'auto', whiteSpace: 'nowrap',
                ...
                >
                  {breadcrumbs.map((crumb, i) => {
                    ...
                    return (
                      <React.Fragment key={`${crumb.type}-${i}`}>
                        {i > 0 && (
                          <ChevronRight size={11} aria-hidden style={{ flexShrink: 0, color: '#CBD5E1' }} />
                        )}
                        <span
                          title={`${titleiseKey(crumb.type)}: ${crumb.name}`}
                          style={{
                            flexShrink: 0, maxWidth: 190,
                            overflow: 'hidden', textOverflow: 'ellipsis',
                            fontWeight: last ? 700 : 500,
                            color: last ? '#FF5A0A' : '#667085',
                          }}
                        >
                          {crumb.name}
                        </span>
          ...
          <div className="es-foot flex items-center flex-shrink-0"
            ...
            {/* Next stays enabled while step 1 is unsaved — handleNext raises
                the "Save before continuing" notice instead. A disabled button
                here looked like a bug: the form was filled, the button was
            ...
            {!isLastStep && (
              <button onClick={handleNext} disabled={busy}
                ...
                title={isOnStep1 && !step1Unlocked ? 'Save your exercise details to continue' : undefined}
