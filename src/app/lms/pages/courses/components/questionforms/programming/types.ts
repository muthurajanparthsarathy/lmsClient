...
export interface ProgrammingQuestionFormProps {
  ...
  /**
   * Supplied by a host that opened this form FROM ITS OWN source chooser (the
   * Combined "Add a question" popup). When an auto-opened source modal is
   * dismissed with nothing picked and no unsaved work, the teacher backed out
   * of the SOURCE, not the question type — so hand them back to that chooser
   * instead of exiting the flow or dropping them into a blank manual editor.
   * Omitted by every other caller, which keeps their existing routing.
   */
  onSourceDismissed?: () => void;
}
