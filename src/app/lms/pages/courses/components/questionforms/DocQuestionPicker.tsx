              ...
              return (
                <button key={q._previewId} onClick={() => toggle(q)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {(tcs.length > 0 || typeof q.marks === 'number') && (
                      <div className="flex items-center gap-2 mt-0.5 text-[10px]" style={{ color: '#8b8b9e' }}>
                        {tcs.length > 0 && <span className="inline-flex items-center gap-1"><FlaskConical size={9} />{tcs.length} test case{tcs.length === 1 ? '' : 's'}</span>}
                        {hidden > 0 && <span className="inline-flex items-center gap-1"><EyeOff size={9} />{hidden} hidden</span>}
                        {/* Marks read off the paper's header line — informational; the
                            exercise's own scoring config still decides the saved score. */}
                        {typeof q.marks === 'number' && <span>{q.marks} mark{q.marks === 1 ? '' : 's'} on paper</span>}
                      </div>
                    )}
                  </div>
