...
import { QuestionApprovalBanner, RequestApproveButton, deriveApprovalContext } from "@/app/lms/pages/courses/components/questionforms/QuestionApprovalBanner";
...
const ProgrammingQuestionForm: React.FC<ProgrammingQuestionFormProps> = ({
  ...
  autoOpenSource, onSourceDismissed,
}) => {
  injectFonts();
...
/**
 * Remaining-slot pill for one row of the "Add Question via" menu.
 *
 * The menu already knew each source's remaining count — it just threw the
 * number away and kept the boolean, so every row read "Import from bank" and
 * you had to open a source to discover it had no room. The count is the
 * decision the reader is actually making, so it goes on the row.
 *
 * `shared` marks the two rows that bill the SAME Manual slice (Question Bank
 * and Upload via Document): without it, one quota shown twice reads as two.
 */
const AddViaQuotaPill: React.FC<{ remaining: number; accent: string; shared?: boolean }> = ({ remaining, accent, shared }) => {
  const full = remaining <= 0;
  return (
    <span
      title={full
        ? 'No slots left for this source'
        : `${remaining} slot${remaining === 1 ? '' : 's'} left${shared ? ' — shared with the other Manual option' : ''}`}
      className="ml-auto inline-flex items-center shrink-0 tabular-nums"
      style={{
        fontSize: 10.5, fontWeight: 700, lineHeight: 1,
        padding: '4px 8px', borderRadius: 999,
        color: full ? '#B42318' : accent,
        background: full ? '#FEF3F2' : `${accent}14`,
        border: `1px solid ${full ? '#FECDCA' : accent + '33'}`,
      }}
    >
    ...
    <div ref={addDropdownRef} className="relative" style={{ marginRight: 8 }} onClick={e => e.stopPropagation()}>
      ...
      {showAddDropdown && (
        ...
        >
          ...
          {bankViaScratch && (() => {
            const remaining = getSourceRemainingTotal('scratch');
          ...
          >
            ...
            <AddViaQuotaPill remaining={remaining} accent="#a855f7" shared />
          </button>
            );
          })()}
          ...
          {allowedSources.thirdParty && (() => {
            const remaining = getSourceRemainingTotal('thirdParty');
            const quotaFull = remaining <= 0;
            return (
          <button
            onClick={() => {
          ...
          >
            ...
            <AddViaQuotaPill remaining={remaining} accent="#0d9488" />
          </button>
            );
          })()}
          ...
          {allowedSources.ai && (() => {
            const remaining = getSourceRemainingTotal('ai');
            const quotaFull = remaining <= 0;
            return (
          <button
            onClick={() => {
          ...
          >
            ...
            <AddViaQuotaPill remaining={remaining} accent="#F27757" />
          </button>
            );
          })()}
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <div className="text-[10px]" style={{ color: quotaFull ? '#dc2626' : '#8b8b9e' }}>{quotaFull ? 'Manual quota full' : `Import from ${DOC_ACCEPT_LABEL}`}</div>
            </div>
            <AddViaQuotaPill remaining={remaining} accent="#0891b2" shared />
          </button>
            );
          })()}

          {/* Question Bank and Upload both stamp 'scratch', so they spend one
              shared Manual allowance. Two rows showing the same number looks
              like a bug until this says otherwise. */}
          {bankViaScratch && allowedSources.upload && (
            <div style={{
              borderTop: '1px solid #f0f0f7', padding: '8px 14px 10px',
              fontSize: 10, color: '#8b8b9e', lineHeight: 1.45,
            }}>
              Question Bank and Upload share one Manual allowance.
            </div>
          )}
        </div>
      )}
      {/* Hidden file input for "Upload via Document" (.txt / .docx / .pdf) */}
      <input ref={docInputRef} type="file" accept={DOC_ACCEPT} className="hidden" onChange={handleDocumentSelected} />
  ...
  <div className="fixed inset-0 z-[9999]">
    <QuestionBankSelector
      ...
      onClose={() => {
        ...
        // Host owns the source chooser (Combined "Add a question"): dismissing
        // the picker empty-handed means "not this source", not "not this type"
        // — so return to that chooser rather than exiting or silently landing
        // the teacher in the manual editor they never asked for.
        if (
          isBankLike
          ...
          onSourceDismissed();
          return; // form is unmounting — no re-route on top of it
        }
        if (
          isBankLike
          && sourceModalAutoOpenedRef.current
...
<GenerateProgFamilyAI
  ...
  onClose={() => {
    ...
    const manualSlotsRemain = allowedSources.manual && getSourceRemainingTotal('scratch') > 0;
    // Same host-chooser contract as the bank picker's onClose above.
    if (
      autoOpenSource === 'ai'
      && sourceModalAutoOpenedRef.current
      && !sourceModalAddedRef.current
