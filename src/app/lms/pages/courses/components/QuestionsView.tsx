...
import { DOC_ACCEPT, DOC_ACCEPT_LABEL } from './questionforms/docTextExtract';
import {
  AQ, QuotaFullChip, AddQuestionModalShell, AddQuestionSourceRow,
  AddQuestionLevelStep, AddQuestionLevelBar,
  AQIconScratch, AQIconDocument, AQIconAI, AQMostFlexibleTag, AQNewBadge,
} from './questionforms/AddQuestionChooserUI';
import QuestionPreview from './questionforms/QuestionPreview';
...
} from '../uploadcourseresources/components/youdo/assessments/questionsource/quotaModel';
...
const addBtnDisabled = isAddingQuestions || (() => {
  ...
  const AddQuestionOptions = () => {
    ...
    // Mockup palette + option row now live in AddQuestionChooserUI so the
    // You_Do assessment chooser renders the identical modal (it had drifted to
    // a pre-2026-09 popup). Semantics here — level picker, allocation panel,
    // per-source gating — are unchanged.
    const MK = AQ;
    const Row = AddQuestionSourceRow;

    return (
      <AddQuestionModalShell
        title={isPureProg ? 'Add a programming question' : isPureMCQ ? 'Add an MCQ question' : 'Add a question'}
        contextName={exercise?.exerciseInformation?.exerciseName || nodeName || null}
        contextSub={exercise?.exerciseInformation?.exerciseName ? (nodeName || null) : null}
      ...
      >
          ...
          {levelFirstDiffs.length > 0 && addFlowDiff === null && (
            <AddQuestionLevelStep levels={levelFirstDiffs} onPick={setAddFlowDiff} />
          )}
          ...
          <div className="space-y-2" style={{ padding: '4px 0 16px' }}>
            {addFlowDiff && (
              <>
                <AddQuestionLevelBar level={addFlowDiff} onChange={() => setAddFlowDiff(null)} />
                ...
                {(() => {
                  ...
                  return (
                    <div style={{ background: '#FAFAF7', border: '1px solid #EDEBE7', borderRadius: 14, padding: '14px 16px' }}>
                      <div className="text-[10.5px] font-bold uppercase" style={{ color: '#6B7280', letterSpacing: '.6px', marginBottom: 10 }}>
                        Allocation for this level
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {rows.map(({ bucket, label, dot }) => {
                          ...
                          // A bar makes "1 of 2" legible at a glance; the old
                          // rows were three columns of small text and you had
                          // to read every one to find the bucket with room.
                          const pctDone = st.configured > 0
                            ? Math.min(100, Math.round((st.used / st.configured) * 100))
                            : 0;
                          return (
                            <div key={bucket} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                              <span style={{ width: 8, height: 8, borderRadius: 999, background: notAllocated ? '#D1D5DB' : dot, flexShrink: 0 }} />
                              <span className="font-semibold" style={{ minWidth: 104, fontSize: 12.5, color, flexShrink: 0 }}>{label}</span>
                              {notAllocated ? (
                                <span style={{ fontSize: 12, color: '#9CA3AF' }}>Not allocated for this difficulty</span>
                              ) : (
                                <>
                                  <div style={{ flex: 1, minWidth: 54, height: 6, borderRadius: 999, background: '#ECEAE4', overflow: 'hidden' }}>
                                    <div style={{
                                      width: `${pctDone}%`, height: '100%', borderRadius: 999,
                                      background: isDone ? '#10B981' : dot,
                                      transition: 'width 400ms ease-out',
                                    }} />
                                  </div>
                                  <span className="tabular-nums" style={{ fontSize: 12, color, flexShrink: 0, minWidth: 96, textAlign: 'right' }}>
                                    <span style={{ fontWeight: 700 }}>{st.used}/{st.configured}</span>
                                    <span style={{ color: isDone ? '#10B981' : '#6B7280' }}>
                                      {isDone ? ' completed' : ` · ${st.remaining} left`}
                                    </span>
                                  </span>
                                </>
                              )}
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="text-[11.5px]" style={{ color: '#8b8b9e', marginTop: 11, paddingTop: 10, borderTop: '1px dashed #E5E2DC', lineHeight: 1.45 }}>
            ...
            {allowedSources.manual && okFor('scratch') && (() => { const full = fullFor('scratch'); return (
              <Row
                ...
                tag={!full ? AQMostFlexibleTag : undefined}
            ...
            {allowedSources.ai && (isPureMCQ || isCombined || isPureProg) && okFor('ai') && (() => { const full = fullFor('ai'); return (
              <Row
                ...
                badge={full ? <QuotaFullChip /> : AQNewBadge}
            ...
            {allowedSources.upload && (isPureMCQ || docProgEligible) && okFor('scratch') && (() => { const full = fullFor('scratch'); return (
              <Row
                ...
                badge={full && <QuotaFullChip />}
                sub={full ? 'Manual quota completed — shared with Scratch & Question Bank' : isPureProg ? `Upload a ${DOC_ACCEPT_LABEL} and pick which parsed questions to import.` : 'Bulk import from JSON · CSV · TXT · DOCX · PDF.'}
              />
            ); })()}
          </div>
          )}
