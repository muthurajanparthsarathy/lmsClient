...
import {
  ...
  AlertTriangle, X, Zap, CheckCircle, Edit3, BarChart3, Clock, ClipboardCheck,
  Laptop, Code2, Search,
...
import AddQuestionViaDocument from '../../components/AddQuestionViaDocument';
...
import CreateExerciseLauncher from '../../../../component/CreateExerciseLauncher';
...
import { getActiveBatchId } from '@/app/lms/pages/courses/api/resourceBatch';
import TableFooter from '@/app/lms/shared/listing/TableFooter';
import AddQuestionForm from "../../components/questionforms/AddQuestionForm";
import QuestionBankSelector from "../../components/questionforms/mcq/QuestionBankSelector";
          ...
          <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: T2.bg }}>
            ...
            <div style={{ flexShrink: 0, height: 40, borderBottom: `1px solid ${T2.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: T2.pageBg, padding: '0 8px 0 0' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                ...
                {/* Visualize — Python only, matching the student workspace,
                    where the step-through trace is Python-specific. */}
                {lang === 'python' && (
  ...
  const fetchSubmissionStatuses = async (exerciseList: Exercise[]) => {
    ...
    try {
      ...
      const resp = await fetch(
        `${API_ORIGIN}/analytics/exercise-submission-status?${params}`,
      ...
      <>
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-72" style={{ ...JKT, border: '1px solid #e4e4ed', boxShadow: '0 8px 32px rgba(26,26,46,0.14)' }}>
            ...
            {/* Grade — shown whenever the exercise is complete and enabled the
                moment submissionStatusMap[_id] flips true (the first time a
                student has an ExamSession row for this exercise); before that
                it stays visible-but-disabled. Grade deep-links into the Grades
                student list for this exercise, skipping the outer exercises
                picker; `returnTo` carries the current URL so Back returns to
                the assignment list.
                "Review Submissions" no longer lives here — it was promoted to
                its own "Review" column on the row (same gate), so reviewing
                doesn't cost a menu click. */}
            {exerciseComplete && (
              <>
                <DropdownMenuItem
                  onClick={() => {
          ...
          <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
            <div className="flex-shrink-0 bg-canvas border-b border-hairline">
              <table className="w-full border-collapse" style={{ tableLayout: 'fixed' }}>
                ...
                <thead>
                  <tr>
                    {['#', `${subLabelSingular} ID`, `${subLabelSingular} Name`, 'Type', 'Created', 'Status', 'Review', 'Actions'].map((label, index) => (
                      <th key={label}
                        className={`${index === 0 ? 'pl-4 pr-2' : index === 6 ? 'px-3 text-center' : index === 7 ? 'pl-2 pr-4 text-right' : 'px-3'} h-8 text-[10px] font-semibold uppercase tracking-wider text-subtle align-middle whitespace-nowrap`}>
                        {label}
                      </th>
                    ))}
                  </tr>
                ...
                <tbody>
                  {Array.from({ length: Math.max(6, Math.min(10, pagination.itemsPerPage)) }).map((_, idx) => (
                    <tr key={idx} className="border-b border-hairline bg-surface">
                      ...
                      <td className="h-11 px-3 align-middle">
                        <div className="mx-auto h-6 w-[74px] rounded-md bg-slate-100 animate-pulse" />
                      </td>
                ...
                <thead>
                  <tr>
                    {[
                      ...
                      { label: 'Review', align: 'center' as const, className: 'px-3' },
                    ...
                    return (
                      <tr key={ex._id}
                        ...
                        {/* Review — promoted out of the kebab into its own column.
                            Carries the exact gate the "Review Submissions" menu
                            entry used: the exercise must be fully configured AND
                            submissionStatusMap[_id] must be true (the server
                            flips it the first time a student has an ExamSession
                            row for this exercise). Until then there is nothing to
                            review, so the cell shows a neutral dash that occupies
                            the same centred slot as the button — the column stays
                            visually aligned whichever state a row is in. */}
                        <td className="h-11 px-3 align-middle text-[12px]">
                          <div className="flex items-center justify-center">
                            {isExerciseComplete(ex) && submissionStatusMap[ex._id] ? (
                              <button
                                type="button"
