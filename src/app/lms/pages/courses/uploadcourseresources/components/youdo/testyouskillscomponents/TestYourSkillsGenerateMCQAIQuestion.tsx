...
interface GenerateMCQAIQuestionProps {
  ...
  exerciseData?: ExerciseData; // Made optional
  onClose: () => void;
...
// ─── MAIN COMPONENT ────────────────────────────────────────────────────────
const TestYourSkillsGenerateMCQAIQuestion: React.FC<GenerateMCQAIQuestionProps> = ({
  breadcrumbs = [],
  exerciseData,
  onClose,
  onSave,
  ...
  // 🔥 KEY CHANGE: Max 20 questions for Test Your Skills
  ...
  const exerciseName = exerciseData?.exerciseName || exerciseData?.nodeName || '';

  // 🔥 KEY CHANGE: No limits - always can generate
  const getMaxAllowedQuestions = (): number => 20; // Allow up to 20 questions
...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (numberOfDuplicates > 0) setSettings(prev => ({ ...prev, numQuestions: Math.min(20, Math.max(1, numberOfDuplicates)) }));
    ...
    try {
      ...
      return parsed.map((q, index) => {
        ...
        const base: GeneratedQuestion = {
          ...
          points: 0, // 🔥 Points set to 0 initially - user can edit
  ...
  const createTemplateQuestion = (index: number, overrideType?: QuestionType): GeneratedQuestion => {
    ...
    const base: GeneratedQuestion = {
      ...
      difficulty: settings.difficulty,
      points: 0, // 🔥 Points set to 0 initially
  ...
  const handleGenerate = async () => {
    ...
    const count = Math.min(settings.numQuestions, getMaxAllowedQuestions());
    try {
      ...
      setGeneratedQuestions(final.map(q => ({ ...q, selected: true })));
    } catch {
      const fallback = Array.from({ length: count }, (_, i) => createTemplateQuestion(i+1));
      setGeneratedQuestions(fallback.map(q => ({ ...q, selected: true })));
  ...
  const handleAddMore = async () => {
    ...
    if (currentCount >= maxAllowed) {
      setApiError({ message: `Maximum ${maxAllowed} questions already generated. Remove some to add more.` });
      return;
    }
    
    const remainingSlots = maxAllowed - currentCount;
    const count = Math.min(settings.numQuestions, remainingSlots);
    
    setLoading(true); setApiError(null);
    try {
      const aiResponse = await callGeminiAI(buildPrompt(count, 'additional'));
      const parsed = parseAIResponse(aiResponse, count);
      const newQs = (parsed.length > 0 ? parsed.slice(0, count) : Array.from({ length: count }, (_, i) => createTemplateQuestion(generatedQuestions.length+i+1))).map((q, i) => ({ ...q, id: `gen-${Date.now()}-${i}-${Math.random().toString(36).substr(2,4)}`, selected: true }));
      setGeneratedQuestions(prev => [...prev, ...newQs]);
    } catch {
      setGeneratedQuestions(prev => [...prev, ...Array.from({ length: count }, (_, i) => ({ ...createTemplateQuestion(prev.length+i+1), selected: true }))]);
    } finally { setLoading(false); }
  };
  ...
  const toggleQuestionSelection = (questionId: string) => {
    setGeneratedQuestions(prev => prev.map(q => q.id === questionId ? { ...q, selected: !q.selected } : q));
  };

  const toggleSelectAll = () => {
    const newVal = !selectAll;
    setSelectAll(newVal);
    setGeneratedQuestions(prev => prev.map(q => ({ ...q, selected: newVal })));
  };
  ...
  const addCustomQuestion = () => {
    const maxAllowed = getMaxAllowedQuestions();
    if (generatedQuestions.length >= maxAllowed) {
      setApiError({ message: `Maximum ${maxAllowed} questions reached. Remove some to add more.` });
      return;
    }
    const q = { ...createTemplateQuestion(generatedQuestions.length+1), id: `custom-${Date.now()}-${Math.random().toString(36).substr(2,4)}`, isEditing: true, selected: true };
  ...
  const handleSave = () => {
    ...
    // 🔥 No limits - all selected questions are added
    const formattedSelected = selected.map(q => ({
      ...
      points: q.points || 0, // Preserve points if set, otherwise 0
    }));
  ...
  const renderQuestionEditor = (question: GeneratedQuestion) => {
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, padding: 12, background: 'var(--lms-violet-bg)', borderRadius: 'var(--lms-radius-md)', border: '1.5px solid var(--lms-violet-bdr)' }}>
        ...
        {/* Points field added for Test Your Skills */}
        <div>
          <label style={{ fontFamily: 'var(--lms-font)', fontSize: 10, fontWeight: 700, color: 'var(--lms-text-hint)', textTransform: 'uppercase', letterSpacing: '0.07em', display: 'block', marginBottom: 4 }}>Points / Marks</label>
          <input type="number" min="0" step="1" value={eq.points || 0} onChange={e => updateEditingQuestion('points', parseInt(e.target.value) || 0)} style={inputStyle} />
        </div>
      ...
      <div className="ai-sidebar-scroll"
        ...
        {showRightSidebar && (
          <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            {/* 🔥 KEY CHANGE: Questions to Generate - Allow up to 20 */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <input
                type="range" min="1" max="20" step="1" value={settings.numQuestions}
              ...
              <p style={{ fontFamily: 'var(--lms-font)', fontSize: 10.5, color: 'var(--lms-text-muted)' }}>
                Generate up to 20 questions for your quiz
              </p>
            </div>
          ...
          <div ref={modalRef} style={{ background: 'var(--lms-bg-white)', borderRadius: 'var(--lms-radius-lg)', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', width: '100%', maxWidth: 960, border: '1.5px solid var(--lms-border)', maxHeight: '92vh', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                    ...
                    <div style={{ width: '100%', maxWidth: 460, display: 'flex', flexDirection: 'column', gap: 16 }}>
                      ...
                      <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 6 }}>
                        ...
                        <div>
                          ...
                          <p style={{ fontFamily: 'var(--lms-font)', fontSize: 12, color: 'var(--lms-text-muted)' }}>Enter a topic and let AI create questions for your quiz</p>
                        </div>
                      </div>
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                        ...
                        <input type="text" value={topicInput} onChange={e => setTopicInput(e.target.value)}
                          ...
                          style={{ fontFamily: 'var(--lms-font)', fontSize: 13, padding: '10px 14px', border: '1.5px solid var(--lms-border)', borderRadius: 'var(--lms-radius-md)', outline: 'none', background: 'var(--lms-bg-surface)', color: 'var(--lms-text-main)', transition: 'all 0.15s', width: '100%', opacity: loading ? 0.6 : 1 }}
                      ...
                      {/* Generate button */}
                      <button onClick={handleGenerate} disabled={loading || !topicInput.trim() || !settings.questionType}
                        style={{ width: '100%', padding: '11px 0', background: 'var(--lms-orange)', color: 'white', border: 'none', borderRadius: 'var(--lms-radius-md)', fontFamily: 'var(--lms-font)', fontSize: 13.5, fontWeight: 700, cursor: loading || !topicInput.trim() || !settings.questionType ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, boxShadow: '0 2px 8px var(--lms-orange-glow)', opacity: loading || !topicInput.trim() || !settings.questionType ? 0.6 : 1, transition: 'all 0.15s' }}>
                        {loading ? (
                  ...
                  <div style={{ padding: 16, display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'var(--lms-info-bg)', border: '1.5px solid var(--lms-info-bdr)', borderRadius: 'var(--lms-radius-md)', padding: '8px 14px', marginBottom: 4 }}>
                      ...
                      <p style={{ fontFamily: 'var(--lms-font)', fontSize: 11, color: 'var(--lms-info)', fontWeight: 600 }}>
                        You can set points for each question by editing them
                      </p>
                    </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, padding: '10px 20px', borderTop: '1.5px solid var(--lms-border)', background: 'var(--lms-bg-surface)', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                ...
                {generatedQuestions.length >= getMaxAllowedQuestions() && generatedQuestions.length > 0 && (
                  <span style={{ fontFamily: 'var(--lms-font)', fontSize: 11, fontWeight: 700, color: 'var(--lms-success)', display: 'flex', alignItems: 'center', gap: 4 }}>
                    <CheckCircle size={12} />Max {getMaxAllowedQuestions()} questions reached
                  </span>
                )}
              </div>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {generatedQuestions.length > 0 ? (
                  <>
                    {/* Custom */}
                    <button onClick={addCustomQuestion} disabled={generatedQuestions.length >= getMaxAllowedQuestions()}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', border: '1.5px solid var(--lms-border)', background: 'var(--lms-bg-white)', color: generatedQuestions.length >= getMaxAllowedQuestions() ? 'var(--lms-text-hint)' : 'var(--lms-text-sec)', borderRadius: 'var(--lms-radius-md)', fontSize: 12, fontWeight: 600, cursor: generatedQuestions.length >= getMaxAllowedQuestions() ? 'not-allowed' : 'pointer', fontFamily: 'var(--lms-font)', transition: 'all 0.15s' }}>
                      <Plus size={12} /> Custom
                    </button>

                    {/* Add More */}
                    <button onClick={handleAddMore} disabled={loading || !topicInput.trim() || generatedQuestions.length >= getMaxAllowedQuestions()}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 12px', border: '1.5px solid var(--lms-violet-bdr)', background: 'var(--lms-violet-bg)', color: generatedQuestions.length >= getMaxAllowedQuestions() ? 'var(--lms-text-hint)' : 'var(--lms-violet)', borderRadius: 'var(--lms-radius-md)', fontSize: 12, fontWeight: 600, cursor: (loading || !topicInput.trim() || generatedQuestions.length >= getMaxAllowedQuestions()) ? 'not-allowed' : 'pointer', fontFamily: 'var(--lms-font)', opacity: (loading || !topicInput.trim() || generatedQuestions.length >= getMaxAllowedQuestions()) ? 0.5 : 1 }}>
                    ...
                    {/* Cancel */}
                    <button onClick={handleCloseWithConfirm} disabled={loading}
                      style={{ padding: '6px 12px', border: '1.5px solid var(--lms-border)', background: 'var(--lms-bg-white)', color: 'var(--lms-text-sec)', borderRadius: 'var(--lms-radius-md)', fontSize: 12, fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'var(--lms-font)', opacity: loading ? 0.5 : 1 }}>
                      Cancel
                    </button>

                    {/* Add Selected */}
                    <button onClick={handleSave} disabled={selectedCount === 0}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 16px', border: 'none', background: 'var(--lms-orange)', color: 'white', borderRadius: 'var(--lms-radius-md)', fontSize: 12, fontWeight: 700, cursor: selectedCount === 0 ? 'not-allowed' : 'pointer', fontFamily: 'var(--lms-font)', opacity: selectedCount === 0 ? 0.5 : 1, boxShadow: '0 2px 8px var(--lms-orange-glow)' }}>
                      <Save size={12} />
                  ...
                  <button onClick={handleCloseWithConfirm} disabled={loading}
                    style={{ padding: '6px 16px', border: 'none', background: 'var(--lms-border)', color: 'var(--lms-text-sec)', borderRadius: 'var(--lms-radius-md)', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: 'var(--lms-font)' }}>
                    Cancel
                  </button>
                )}
              </div>
...
export default TestYourSkillsGenerateMCQAIQuestion;