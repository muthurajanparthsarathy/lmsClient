...
import { extractDocumentText } from "@/app/lms/pages/courses/components/questionforms/docTextExtract";
import { parseQuestionsTxt } from "@/app/lms/pages/courses/components/questionforms/parseQuestionsTxt";
...
useEffect(() => {
  ...
  // ── File handling ──────────────────────────────────────────────────────────
  // json / csv / txt go to the server parser as-is. docx / pdf are read in the
  // browser first (see toServerFile) because the server's bulk parser only
  // understands those three formats — a Word paper would otherwise be rejected
  // outright, which is what trainers kept hitting.
  const DOC_EXTS = ["json", "csv", "txt", "docx", "pdf"];

  const acceptFile = (f: File) => {
    const ext = f.name.split(".").pop()?.toLowerCase();
    if (!DOC_EXTS.includes(ext || "")) {
      setParseError("Supported files: .json, .csv, .txt, .docx and .pdf.");
      return;
    }
    setFile(f);
    setParseError(null);
  ...
  /**
   * Hand the server something it can parse.
   *
   * A .docx / .pdf is extracted to plain text in the browser, then read with
   * the shared Q:/A)…D)/Answer: parser and re-emitted as the JSON rows the
   * bulk endpoint already accepts — so capacity, validation and the whole
   * preview/insert pipeline stay exactly where they were, server-side. If the
   * document isn't in that prose shape, its raw text is sent as .txt so the
   * server's pipe / tab / JSON-per-line formats still get their chance.
   */
  const toServerFile = async (f: File): Promise<File> => {
    const ext = f.name.split(".").pop()?.toLowerCase() || "";
    if (!["docx", "pdf"].includes(ext)) return f;

    const text = await extractDocumentText(f);
    const base = f.name.replace(/\.[^.]+$/, "");
    const parsed = parseQuestionsTxt(text);
    ...
    const rows = parsed.map((q) => ({
      questionTitle: q.description || q.title,
      options: q.options.map((o) => ({ text: o.text, isCorrect: o.isCorrect })),
      difficulty: q.difficulty,
      ...(q.explanation ? { explanation: q.explanation } : {}),
    }));
    return new File([JSON.stringify(rows, null, 2)], `${base}.json`, { type: "application/json" });
  };
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 16, padding: "20px 24px 24px" }}>
      ...
      >
        <input
          ref={fileInputRef}
          type="file"
          accept=".json,.csv,.txt,.docx,.pdf,application/json,text/csv,text/plain,application/pdf,application/vnd.openxmlformats-officedocument.wordprocessingml.document"
          style={{ display: "none" }}
        ...
        {file ? (
          ...
          <>
            ...
            <p style={{ ...JKT, fontSize: 11, color: "#bcbccc", margin: 0 }}>
              JSON · CSV · TXT · DOCX · PDF · Max 5 MB
            </p>
          </>
        )}
      </div>
