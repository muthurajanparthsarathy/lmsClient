...
import ProgrammingMockModal from '@/app/lms/component/ProgrammingMockModal';
...
import { FrontendCodeSetupSection, FrontendCode, EMPTY_FRONTEND, normalizeFrontendCode, isFrontendSolutionEmpty } from '@/app/lms/component/questionforms/CodeSetupSection';
// ─── FONT INJECTION ───────────────────────────────────────────────────────────
const injectFonts = (() => {
  let injected = false;
  return () => {
...
interface FlowQuestion {
  ...
  // Provenance tag persisted with the question — parity with Programming form.
  source?: string;
  isLinkQuestion?: boolean;
  questionLink?: string;
  // Code Setup — split HTML / CSS / JS for both editors.
...
const dbQuestionToFlow = (q: any): FlowQuestion => ({
  ...
  isDirty: false,
  isPreExisting: true,
  // Preserve DB source tag so an edited AI-generated Frontend question re-saves
  // as 'ai' instead of silently rebranding to 'scratch-manual'.
...
const FrontendQuestionForm: React.FC<FrontendQuestionFormProps> = ({
  ...
  // Question Source enforcement (see component/questionforms/ProgrammingQuestionForm for the rule)
  ...
  // Code Setup — HTML / CSS / JS bundles for both editors.
  ...
  const mkPayload = () => {
    ...
    // Preserve the current flow question's source tag (set by dbQuestionToFlow
    // for edits, by incoming-question handlers for AI/bank/doc imports); fresh
    // manual work defaults to 'scratch-manual'. Persisted so the source badge
    // in the manage-questions list and per-source quota narrowing both work
    // for Frontend questions — parity with MCQ / Programming (You_Do).
    const sourceTag = (q as any)?.source || 'scratch-manual';
    return {
  ...
  const handleIncomingProgQuestions = (incoming: any[]) => {
    ...
    flowQuestionsRef.current = newFlow; setFlowQuestions(newFlow);
    const startIdx = flow.length; currentIndexRef.current = startIdx; setCurrentIndex(startIdx);
  ...
  const handleDocumentSelected = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      handleIncomingProgQuestions(parsed as any[]);
    } catch (err: any) {
...
{/* Stats summary */}
      ...
      {showBankSelector && (
        <div className="fixed inset-0 z-[9999]">
          <QuestionBankSelector
            ...
            onSelect={(qs: any) => { setShowBankSelector(false); handleIncomingProgQuestions(qs as any[]); }}
      ...
      <GenerateProgFamilyAI
        ...
        exerciseName={exerciseName}
        onGenerated={(qs: any) => handleIncomingProgQuestions(qs as any[])}
      />
    </div>
  );
};
