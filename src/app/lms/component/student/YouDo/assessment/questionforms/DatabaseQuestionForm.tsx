...
const remainingMarksIncludingUnsaved = useMemo((): number => {
  ...
  const handleIncomingDBQuestions = useCallback((incoming: any[]) => {
    ...
    const mapped: DBQuestion[] = incoming.map((q) => {
      ...
      return {
        __localId: mkLocalId(),
        title: q.title || 'Untitled Database Question',
        description: toBlocks(q.description || ''),
        difficulty: diff,
        score: defaultScore,
        sampleQuery: q.sampleQuery || '',
        sampleResult: toBlocks(q.sampleResult || ''),
  ...
  const snapshotForm = useCallback((): DBQuestion => {
    ...
    return {
      ...
      hints: allHints,
      questionType: 'database',
      // Preserve existing tag on edit; new manual questions default to
