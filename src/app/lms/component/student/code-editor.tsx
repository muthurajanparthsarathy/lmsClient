...
function solve() {
    ...
    const submitCode = async () => {
        ...
        try {
            ...
            // ── Evaluation Method branch ─────────────────────────────────────
            // The stored `evaluationMethod` on the exercise decides what runs
            // now. Manual: nothing (post score:0). Test Case: run every test
            // case via Piston. AI: call Gemini with the selected criteria.
            // See lib/resolveEvaluationMethod for the per-tab legacy fallback.
            ...
            if (method === 'testcase') {
                ...
                // Cached generated cases from an earlier student's Submit — Gemini
                // will REUSE these rather than regenerating so everyone sees the
                ...
                // If this student is the FIRST to submit an AI-graded exercise,
                // Gemini generated the test cases fresh. Persist them on the
                // question so every subsequent student is judged against the
                // same set. Fire-and-forget — a failure here doesn't stop the
                    ...
                    <div className="flex flex-col flex-1 min-h-0" style={{ height: `${100 - rightPanelSplit}%` }}>
                        ...
                        <div className={`flex items-center justify-between gap-0 border-b flex-shrink-0 ${theme === 'dark' ? 'border-gray-700 bg-gray-800' : 'border-gray-300 bg-gray-50'}`}>
                            ...
                            <div className="flex items-center gap-0">
                                <div className={`flex items-center gap-1.5 px-3 py-1.5 border-b-2 border-orange-500 ${theme === 'dark' ? 'text-orange-400' : 'text-orange-600'}`}>
