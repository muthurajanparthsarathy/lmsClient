...
// Include submenu pages and workspace tabs so each stays searchable.
const flattenGroup = (group: NavGroup): PaletteEntry[] => {
    ...
    group.items.forEach((item) => {
        ...
        [...(item.children || []), ...(item.tabs || []).filter((tab) => tab.title !== item.title || tab.href !== item.href)].forEach((child) => {
            entries.push({
                title: child.title,
                href: child.href,
                icon: child.icon,
