...
export interface SidebarItem {
    ...
    // Extra routes that count as THIS item for the active highlight without
    // rendering as rows of their own — a flat entry whose section spans more
    // than one page (Business Management: Client Management + Service
    // Mapping). The sidebar reads these alongside `children`.
    tabs?: { href: string }[];
}
...
export const ADMIN_SIDEBAR_KEYS = [
    ...
    "externalassessment",   // ─┐ External › Assessment / Event. Same rule —
    "externalevent",        // ─┘ the section lands at the first key held.
    "dynamicfieldsettings", // ─┐ System Settings › Dynamic Settings / Audit
    "logs",                 //  │ Logs / Backup. Same rule again.
    "backup",               // ─┘
    "profile",              // Profile
];
...
export const buildSidebarItems = (permissions: UserPermission[], adminOnly: boolean): SidebarItem[] => {
    ...
    // ── Merged sections ──
    // A MergedSection collapses several separately-granted pages into ONE
    // expandable rail entry, in place, at the position of the first of its
    // children the account holds. Children are PERMISSION-DRIVEN: the submenu
    // carries only the pages actually granted, so ticking a child in the modal
    // is what puts that row in the rail.
    //
    // Two sections use it:
    //   Question Bank → Internal Questions / External Questions
    //   External      → Assessment / Event
    //
    // Both were hand-rolled copies of this block before; the Question Bank one
    // additionally used to pin BOTH children on whenever `questionbanks` was
    // held, regardless of what was granted.
    const applyMergedSection = (section: typeof QUESTION_BANK_SECTION) => {
        const granted = grantedSectionChildren(section, items.map(i => i.permissionKey));
        if (granted.length === 0) return;
        const idx = items.findIndex(i =>
            section.children.some(c => c.key === canonicalPermissionKey(i.permissionKey)),
        );
        if (idx === -1) return;
        const sectionColor = items[idx].color;
        const children: SidebarItem[] = granted.map(c => ({
            title: c.title,
            href: c.href,
        ...
        const merged: SidebarItem = {
            title: section.title,
            // Parent opens the FIRST child held, in spec order — for Question
            // Bank that is Internal, which every existing bookmark and in-app
            // link (Course Setup, exercise authoring, …) already targets.
            href: children[0].href,
            icon: getIconByName(section.iconName),
            iconName: section.iconName,
            color: sectionColor,
            hasChevron: false,
            permissionKey: section.parentKey,
            children,
        };
        const childKeys = section.children.map(c => c.key);
        const rest = items.filter(
            i => !childKeys.includes(canonicalPermissionKey(i.permissionKey)),
        );
    ...
    applyMergedSection(QUESTION_BANK_SECTION);
    applyMergedSection(EXTERNAL_SECTION);
    ...
    //
    // Course Management was briefly a third child here. It is its OWN
    // permission — `permissions.tree.ts` has always carried it as a
    // top-level `page("admin-course-manage", …)` rather than a child of the
    // `admin-businessmanagement` container — so nesting it on the rail made
    // the sidebar disagree with the Assign Permission modal it is granted
    // from: an account holding only Course Management saw a "Business
    // Management" heading it had no Business grant for. It now renders as a
    // sibling, in the `ADMIN_SIDEBAR_KEYS` slot right after this section.
    //
    // ── Business Management — SINGLE flat sidebar entry ──
    //
    // No dropdown, no subitems. The three routes it covers — Client
    // Management, Service Mapping and Reports — are switched via the tab
    // strip in the shared module layout (BusinessWorkspaceChrome), so a
    // second navigation on the rail would be duplicated chrome. The
    // sidebar's job is to land the user in the workspace; the tabs are
    // what move them between its sections.
    //
    // Landing route: Client Management if the user holds that grant,
    // otherwise Service Mapping. This way even an account granted only
    // one of the two lands somewhere usable — and the container-only
    // grant (businessmanagement) implies BOTH, so it lands on Client
    // Management like the fully-granted case.
    ...
    if (firstBusinessIdx !== -1) {
        ...
        const hasContainer = held.has("businessmanagement");
        const canClient = held.has("clientmanagement") || hasContainer;
        const canService = held.has("servicemapping") || hasContainer;

        if (canClient || canService) {
            const landing = canClient
            ...
            const merged: SidebarItem = {
                ...
                iconName: "Briefcase",
                color: sectionColor,
                // Explicitly no chevron and no children — this is a flat
                // link, not a disclosure. The Route switching lives in
                // the workspace tabs inside the page.
                hasChevron: false,
                permissionKey: "businessmanagement",
                // Both workspace routes keep this entry lit, not just the
                // landing one — otherwise opening Service Mapping left the
                // rail with nothing selected. Only the routes this user
                // actually holds.
                tabs: [
                    ...(canClient ? [{ href: "/lms/pages/clientmanagement" }] : []),
                    ...(canService ? [{ href: "/lms/pages/servicemapping" }] : []),
                ],
            };
            // Strip every underlying key + any old top-level Business
            // Management entry, then splice the flat entry in at the
...
const SYSTEM_SETTINGS_KEYS = ["dynamicfieldsettings", "logs", "backup"];
function systemSettingsMerge(items: SidebarItem[]): SidebarItem[] {
    ...
    const backup = items.find(i => (i.permissionKey || "").toLowerCase() === "backup");
...
const GROUP_BY_KEY: Record<string, string> = {
    ...
    // External → Assessment / Event. Learning rather than Management: these
    // are assessments and events people sit, just for an audience outside the
    // LMS — so they belong beside Question Bank and Grade, not beside Client
    // Management. `external` is the merged parent's own key.
    external: "Learning",
    externalassessment: "Learning",
    externalevent: "Learning",
    feedback: "Learning",
    grades: "Learning",
    calendar: "Learning",
    schedule: "Learning",
