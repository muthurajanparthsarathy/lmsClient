...
export interface SidebarItem {
    ...
    /** Pages reached through workspace tabs, without a sidebar submenu. */
    tabs?: SidebarItem[];
}
...
export const ADMIN_SIDEBAR_KEYS = [
    ...
    "clientmanagement",     // ─┐ Client Management. The merger splices the
...
export const buildSidebarItems = (permissions: UserPermission[], adminOnly: boolean): SidebarItem[] => {
    ...
    // Client Management is one direct sidebar link. Its two pages are tabs
    // in the workspace; retain their grants for active-route matching and
    // command-palette shortcuts. Course Management stays a separate item.
    ...
    if (firstBusinessIdx !== -1) {
        ...
        // Build tabs only for the keys this user actually holds.
        ...
        const businessTabs: SidebarItem[] = [];
        ...
        if (businessTabs.length > 0) {
            const merged: SidebarItem = {
                // The GROUP is named for what it holds — Clients and Services —
                // not for its first tab. Calling the parent "Client Management"
                // as well as the tab inside it made Service Mapping look like it
                // lived under Clients, and repeated the words either side of one
                // click. Matches this section's own permission key,
                // `businessmanagement`.
                title: "Business Management",
                // Open Clients by default, or Services if that is the only grant.
                href: businessTabs[0].href,
                icon: getIconByName("Building"),
                iconName: "Building",
                color: sectionColor,
                hasChevron: false,
...
const SYSTEM_SETTINGS_KEYS = ["dynamicfieldsettings", "logs", "reportsettings"];
function systemSettingsMerge(items: SidebarItem[]): SidebarItem[] {
    ...
    const reportSettings = items.find(i => (i.permissionKey || "").toLowerCase() === "reportsettings");
    ...
    if (reportSettings) {
        children.push({
            ...
            href: "/lms/pages/reportsettings",
            icon: getIconByName("FileCog"),
            iconName: "FileCog",
            color: sectionColor,
            permissionKey: "reportsettings",
        });
    }
