...
export function DataTable<T>({
    ...
    // body row share the same width for each column, no matter what the
    // cells contain. Only meaningful with `fixedLayout: true`. Anything
    // that goes past the container's width triggers horizontal scroll on
    ...
    return (
        ...
        <div
            ...
            //
            // `columnWidths` is a special case of fixedLayout: the widths are
            // explicit pixel values, and the total can exceed the container.
        ...
        >
            <table
                ...
                // is wider the table sits left-aligned; when it is narrower
                // the wrapper scrolls horizontally. Never `min-width: 100%`
                // here — that used to blow every column up to fill the
                // remaining container width, defeating the pixel widths.
                style={
                    columnWidths
            ...
            >
                {columnWidths && (
                    <colgroup>
                        ...
                        {columns.map((c, i) => (
                            <col key={c.key} style={{ width: columnWidths[i] }} />
                        ))}
                    ...
                    <tr>
                        ...
                        {columns.map((c) => {
                            ...
                            return (
                                <th
                                    key={c.key}
                                    aria-sort={c.sortKey ? (isSorted ? (sortDir === 'asc' ? 'ascending' : 'descending') : 'none') : undefined}
                            ...
                            return (
                                ...
                                >
                                    ...
                                    {columns.map((c) => (
                                        ...
                                        <td
                                            key={c.key}
                                            className={`${c.className || 'px-3'} h-10 align-middle text-[13px] text-body`}
                                        >
                                            {c.render(row, i)}
                                        </td>
                                    ))}
